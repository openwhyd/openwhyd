...
var PlayemWrapper = new (function() {
  ...
  var PLAY_TIMEOUT = 6000,
    timeout,
    players = [
      // defined in /js/playem-all.js (loaded in index.html)
      new BandcampPlayer({}),
  ...
  var parseFunctions = function(jsCode) {
    ...
    while ((match = FCT_DETECTOR.exec(jsCode))) {
      var fctDef = match[0],
        fctName = match[1];
      functions[fctName] =
        fctDef + parseFunctionImpl(jsCode.substr(match.index));
    }
    return functions;
  };
  return (function() {
    var fcts = {},
      all = parseFunctions(jsCode);
    for (var fctName in all) {
      if (fctName == 'include') scope.include = compileFct(all[fctName]);
...
function BookmarkletWrapper(srcElementId) {
  var detectors = (function() {
    ...
    var codeEl = document.getElementById(srcElementId).contentDocument
      .childNodes[0];
  ...
  (function nextUrl(i) {
    ...
    if (!url) nextUrl(i + 1);
      ...
      detectStream(url, detectors, function(detectorIndex, state) {
        ...
        if (arguments.length) handler(i, detectorIndex, state);
        else nextUrl(i + 1);
      });
    }
  })(0);
...
function HtmlTable(id, rows, columns) {
  ...
  table.innerHTML = Array.apply(null, Array(rows))
    .map(function() {
      return (
        '<tr>' +
  ...
  this.getCell = function(row, col) {
    return table.getElementsByTagName('td')[
      arguments.length == 2 ? row * columns + col : row
    ];
  };
}
...
document.getElementById('bookmarkletCode').onload = function() {
  ...
  readFileLines('urls.txt', function parseFileLines(lines) {
    ...
    [{ name: 'URL' }].concat(detectors).map(function(col, i) {
      table.getCell(0, i).innerHTML =
        "<p style='background:lightgray;'>" + col.name;
    });
    lines.map(function(url, i) {
      table.getCell(i + 1, 0).innerHTML =
        (url.charAt(0) == '#' ? '<b>' : '') + url;
    });
    // main process
