...
function initPostBox($box, onPostComplete) {
  ...
  function renderResult(r) {
    if (r.uNm)
      return (
        '<li class="whyd" data-pid="' +
        r.id +
        '">' +
        ' <a href="' +
        r.url +
        '" target="_blank" class="thumb-result">' +
        '  <img src="' +
        r.img +
        ...
        htmlEntities(r.name) +
        '</h3>' +
        '  <div class="avatar" style="background-image:url(/img/u/' +
        r.uId +
        ');"></div>' +
        '  <p>added by <strong>' +
        r.uNm +
      ...
      return (
        '<li class="' +
        (r.playerLabel || '').toLowerCase() +
        '">' +
        ' <a href="' +
        (r.src ? r.src.id : r.url) +
        '" target="_blank" class="thumb-result">' +
        '  <img src="' +
        r.img +
        '" alt="" width="76" height="76" />' +
  ...
  function selectTune(e) {
    ...
    checkTune(tune, function (embedRef) {
      ...
      if (embedRef) whydPost.setEmbedRef(embedRef);
        ...
        window.showMessage &&
          showMessage(
            "Sorry, we can't find that track anymore... Please pick another one.",
            true
          );
        ...
        function triggerSearch(current) {
          engines[current].query(query, function (results) {
            ...
            if (expected == current) {
              for (
                ;
                expected < engines.length && resultSet[expected];
                ++expected
              )
              ...
              if (lastQuery && !currentResults.length)
                display(
                  '<div class="noResults">' +
                    '<p>' +
                    query +
                    ' was not found on Youtube and Soundcloud</p>' +
