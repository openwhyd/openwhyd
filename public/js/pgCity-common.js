...
var initCityPage = function (urlPrefix, cityName) {
  ...
  var TEMPLATE_TRACKS = [
    ...
    '<div class="play"></div>',
    '</a>',
    '<a class="title no-ajaxy" target="_blank" href="' +
      urlPrefix +
      '/c/{_id}" onclick="{onclick}">{name}</a>',
    '<p class="author">',
  ...
  var TEMPLATE_PEOPLE = [
    ...
    '<div class="thumb" style="background-image:url(' +
      urlPrefix +
      '/img/u/{id}?width=100&amp;height=100);"></div>',
  ...
  var TEMPLATE_PLAYLISTS = [
    '<li data-id="{id}">',
    '<a class="no-ajaxy" target="_blank" href="' +
      urlPrefix +
      '/u/{uId}/playlist/{plId}#autoplay">',
    '<div class="thumb" style="background-image:url(' +
      urlPrefix +
      '/img/playlist/{id});"></div>',
  ...
  function startIndexPage() {
    ...
    fetch(
      urlPrefix + '/api/city/' + cityName + '/posts?limit=3',
      renderTracks(TEMPLATE_TRACKS, document.getElementsByClassName('posts')[0])
    );
    fetch(
      urlPrefix + '/api/city/' + cityName + '/peopleCurrent?limit=3',
      makeDivRendererPeople(
        TEMPLATE_PEOPLE,
        document.getElementsByClassName('people')[0]
      )
    );
  ...
  function startPeoplePage() {
      ...
      for (var i = 0; i < users.length; i++) {
        ...
        $.getJSON('/api/user?id=' + uid + '&includeTags=1', function (data) {
          ...
          $li
            ...
            .css(
              'background-image',
              'url(/img/u/' + data.id + '?width=100&amp;height=100)'
            );
          $li
            .find('.username')
            .html(
              '<a href="//openwhyd.org/u/' + data.id + '">' + data.name + '</a>'
            );
    ...
    loadTopFeatured(function () {
      ...
      fetch(
        urlPrefix + '/api/city/' + cityName + '/peopleRecent?limit=6',
        makeDivRenderer(
          TEMPLATE_PEOPLE,
          document.getElementById('page3').getElementsByClassName('posts')[0]
        )
      );
    });
  ...
  function startPlaylistsPage() {
    var TEMPLATE_PLAYLISTS_GRID = [
      ...
      '<div class="username"><a class="no-ajaxy" target="_blank" href="' +
        urlPrefix +
        '/u/{uId}"><span style="background-image:url(' +
        urlPrefix +
        '/img/u/{uId});"></span><p>{uNm}</p></a></div>',
      '<a class="no-ajaxy" target="_blank" href="' +
        urlPrefix +
        '/u/{uId}/playlist/{plId}#autoplay">',
      '<div class="thumb" style="background-image: -webkit-gradient(linear, left top, left bottom, color-stop(0, rgba(0, 0, 0, 0.7)), color-stop(0.20, transparent)), url(' +
        urlPrefix +
        '/img/playlist/{id});background-image: -moz-linear-gradient(top, rgba(0, 0, 0, 0.7) 0%, transparent 20%), url(' +
        urlPrefix +
        '/img/playlist/{id});background-image: -o-linear-gradient(top, rgba(0, 0, 0, 0.7) 0%, transparent 20%), url(' +
        urlPrefix +
        '/img/playlist/{id});background-image: linear-gradient(top, rgba(0, 0, 0, 0.7) 0%, transparent 20%), url(' +
        urlPrefix +
        '/img/playlist/{id});"><div class="play"></div></div>',
      '</a>',
      '<div class="genres">',
      '<ul>',
      '{genres}',
      ...
      '</div>',
      '<div class="infos">',
      '<p class="plName"><a href="' +
        urlPrefix +
        '/u/{uId}/playlist/{plId}#autoplay">{name}</a></p>',
    ...
    function makeDivRendererPlaylists(template, divOrCallback) {
      return function (res) {
        ...
        var html = alldata
          ...
          .join('\n');
        if (typeof divOrCallback == 'function') divOrCallback(html);
        else divOrCallback.innerHTML = html;
      };
    }
  ...
  function switchPage(link) {
    ...
    var autoplay = $(link).data('autoplay') ? true : false;
