...
(function() {
  ...
  var urlPrefix =
    findScriptHost('/js/bookmarklet.js') || 'https://openwhyd.org';
  ...
  function YoutubeDetector() {
    ...
    var regex = /(youtube\.com\/(v\/|embed\/|(?:.*)?[\?\&]v=)|youtu\.be\/)([a-zA-Z0-9_\-]+)/; ///https?\:\/\/(?:www\.)?youtu(?:\.)?be(?:\.com)?\/(?:(?:.*)?[\?\&]v=|v\/|embed\/|\/)?([a-zA-Z0-9_\-]+)/; //^https?\:\/\/(?:www\.)?youtube\.com\/[a-z]+\/([a-zA-Z0-9\-_]+)/
    ...
    return function(url, cb) {
      ...
      if (embed.id) {
        ...
        var url = encodeURIComponent(
          'http://www.dailymotion.com/embed/video/' + embed.id
        ); // "k7lToiW4PjB0Rx2Pqxt";
        include(
  ...
  function initWhydBookmarklet() {
    ...
    function detectEmbed(e, callback) {
      ...
      for (var p = 0; p < prov.length; ++p)
        addEmbedThumb(e, prov[p], function(embed) {
          ...
          if (0 == --remaining) callback && callback(detected);
        });
    }
      ...
      detectEmbed({ src: window.location.href }, function(found) {
        ...
        /*if (found)
          ...
          (function processNext() {
            if (!toDetect.length) whenDone();
            else detectEmbed(toDetect.shift(), processNext);
          })();
        }
      });
    }
    ...
    function genSearchLink(title) {
      return (
        '<p style="margin-top:15px;">Search for ' +
        '<a href="http://openwhyd.org/search?q=' +
    ...
    if (html && !dzTrackId) whenDone(html);
    else detectAllEmbeds();
  }
  /*
	(function loadNext(){
		if (toInclude.length)
