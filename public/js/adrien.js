...
function transformEchonestResults(cb) {
  return function(res) {
    ...
    var meta = {
      ...
      artistId: firstResult.artist_foreign_ids.map(function(f) {
        return f.foreign_id;
      }), //((firstResult.artist_foreign_ids || [])[0] || {}).foreign_id,
      trackId: firstResult.tracks.map(function(f) {
        return f.foreign_id;
      }), //firstTrack.foreign_id,
      albumId: firstResult.tracks.map(function(f) {
        return f.foreign_release_id;
      }), //firstTrack.foreign_release_id,
      albumName: firstResult.tracks.map(function(f) {
        return f.album_name;
      }) //firstTrack.album_name,
    };
    cb(meta);
  };
}
...
function queryEchonest(q, cb) {
  ...
  for (var i in q) finalQ[i] = q[i];
  $.getJSON(
    'http://developer.echonest.com/api/v4/song/search?bucket=id%3Adeezer&bucket=id:spotify-WW&bucket=tracks&callback=?',
    finalQ,
    cb
  );
}
...
function filterForeignIds(ids, prefix) {
  ...
  ids.map(function(id) {
    ...
    if (splitted.shift().indexOf(prefix) == 0) out.push(splitted.pop());
  });
  return out;
}
...
var trackFetcher = {
  deezer: function(trackId, cb) {
    $.getJSON(
      'http://api.deezer.com/track/' + trackId + '?output=jsonp&callback=?',
      function(res) {
        cb({
...
function pickBestForeignTracks(meta, source, cb) {
  ...
  (function next(i) {
    ...
    trackFetcher[source](trackId, function(res) {
      var distance =
        levenshteinenator(meta.artistName, res.artistName) +
...
function trackCss() {
  ...
  for (var i = 0; i < target.length; ++i) observer.observe(target[i], config);
}
...
function coucou() {
  ...
  function padZeroes(t, n) {
    var t = '' + t;
    for (var i = Math.min(t.length, n - t.length); i > 0; --i) t = '0' + t;
    return t;
  }

  function renderDate(t) {
    return (
      padZeroes(t.getDate(), 2) +
      '/' +
      padZeroes(t.getMonth() + 1, 2) +
      '/' +
      t.getFullYear() +
      ' ' +
      padZeroes(t.getHours(), 2) +
      ':' +
      padZeroes(t.getMinutes(), 2)
    );
  }
  /*
  ...
  function displayHiddenTrackData() {
    ...
    $post.append(
      $(
        ...
        .append(
          $("<div class='score'>score: " + $post.attr('data-score') + '</div>')
        )
    );
  }
  ...
  function displayTrackMetadata(elt) {
    ...
    $.getJSON('/api/track' + eid, function(res) {
      if (res && res.meta) {
        ...
        $elt.append(
          $('<button>lookup with deezer</button>').click(function() {
            getForeignId(res.meta.art, res.meta.tit, logMatches);
          })
        );
      } else
        $elt.append(
          $('<button>search with deezer</button>').click(function() {
            searchForeignId($elt.find('h2 > a').text(), logMatches);
          })
        );
      $elt.append(
        $('<button>refresh meta</button>').click(function() {
  ...
  if (window.user && window.user.lastFm)
    $('.posts')
      .bind('DOMNodeInserted', onEachPost)
      .children()
      .each(onEachPost);
