...
function renderCityPage(city, loggedUser, cb) {
  ...
  var whydPageParams = {
    ...
    pageImage:
      config.urlPrefix + templateParams.cityImgPrefix + 'fb-sharing.jpg',
    js: [],
    css: [],
    endOfBody: [
      '    <script>',
      ...
      '        initCityPage("' +
        config.urlPrefix.substr(config.urlPrefix.indexOf('//')) +
        '", "' +
        city +
        '");',
      '      }',
  ...
  memberCache.hasCity(city, function (members) {
    ...
    if (!actionFct)
      return renderCityPage(
        city,
        loggedUser,
        response.renderHTML.bind(response)
      );
    ...
    actionFct(result, function (res) {
      ...
      response.renderJSON(
        reqParams.callback
          ? snip.renderJsCallback(reqParams.callback, result)
          : result
      );
    });
  });
