  ...
  function joinContest(plC) {
    if (!plC || !plC.uri)
      return render({
        error:
          (plC || {}).error ||
          'hmm... are you sure that this contest still exists?'
      });
    if (loggedUser) {
      console.log('logged user', loggedUser);
      //response.temporaryRedirect("/u/"+loggedUser.id+"/playlist/create?name=" + encodeURIComponent(plC.title));
      userModel.hasPlaylistNameByUid(loggedUser.id, plC.title, function(
        playlist
      ) {
        if (playlist)
          response.temporaryRedirect(
            '/u/' + loggedUser.id + '/playlist/' + playlist.id
          );
        else
          userModel.createPlaylist(loggedUser.id, plC.title, function(
            playlist
          ) {
            response.temporaryRedirect(
              '/u/' + loggedUser.id + '/playlist/' + playlist.id
      ...
      console.log(
        'user is trying to join a contest without being logged in => rendering register page...'
      );
      var registerParams = {
  ...
  if (getParams && getParams.action == 'info' && getParams.uri && isAdmin)
    contestModel.fetchOne(getParams.uri, function(plC) {
      render(
        !plC || !plC.uri
          ? { error: (plC || {}).error }
          : wrap(contestTemplate.renderInfoPage(plC))
      );
    });
    ...
    contestModel.fetch(function(list) {
      ...
      render(
        !list || list.error
          ? { error: (list || {}).error }
          : wrap(contestTemplate.renderListPage(list))
      );
    });
  else if (getParams && getParams.uri)
    contestModel.fetchOne(getParams.uri, function(plC) {
      if (!plC || plC.error)
        render({ error: (plC || {}).error || 'Contest not found' });
      else response.redirect(plC.url);
    });
    ...
    contestModel.save(
      request.body,
      /*render*/ function(plC) {
        if (!plC || !plC.uri) render({ error: (plC || {}).error });
        else response.redirect('/playlistcontest/' + plC.uri + '/info');
      }
    );
