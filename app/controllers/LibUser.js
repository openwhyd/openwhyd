...
function fetchAndRender(options, callback) {
  ...
  const renderer = options.playlistId
    ? require('./LibUserPlaylist')
    : require('./LibUserProfile');
...
async function renderUserLibrary(lib, user) {
   ...
   *
   * Par @adrienjoly (twitter.com/adrienjoly)
   *
   * - Consultant Craft / Tech Lead @SHODO (shodo.io)
   * - Quand j'ai le temps, je maintiens @Openwhyd (openwhyd.org)
   */
  ...
  await new Promise((resolve) => renderResponse(feed, resolve));
}
