...
var PlayemWrapper = new function () {
  ...
  this.fetch = function (url, cb) {
    var player = getPlayer(url)
    if (!player || !player.fetchMetadata) { return cb() }
  ...
  this.play = function (url, cb) {
    ...
    if (streamUrlPos) { id += '#' + url.substr(streamUrlPos) }
...
var parseDetectors = function (jsCode) {
  ...
  function compileFct (fct) {
    ...
    for (var symbol in scope) { fct = fct.replace(new RegExp(symbol, 'g'), '__scope.' + symbol) }
    return new Function('__scope', 'return ' + fct)(scope)
  }
  function parseFunctionImpl (str) {
    var start = str.indexOf('{'),
      level = 1, levelIncr = { '{': 1, '}': -1 }
    for (var i = start + 1; level > 0 && i < str.length; ++i) { level += levelIncr[str.charAt(i)] || 0 }
  ...
  return (function () {
    ...
    for (var fctName in all) {
      if (fctName == 'include') { scope.include = compileFct(all[fctName]) } else if (fctName.indexOf('Detector') != -1) { fcts[fctName] = compileFct(all[fctName]) }
    }
    return fcts
  })()
}
...
function detectStream (url, detectors, handler) {
  (function next (i) {
    ...
    if (!detector) { return handler() } // no more detectors to check for this url
...
function detectStreams (urls, detectors, handler) {
  (function nextUrl (i) {
    if (i == urls.length) { return handler() }
    var url = urls[i].length && urls[i][0] != '#' && urls[i].split(/\s/)[0]
    if (!url) { nextUrl(i + 1) } else {
      ...
      detectStream(url, detectors, function (detectorIndex, state) {
        ...
        if (arguments.length) { handler(i, detectorIndex, state) } else { nextUrl(i + 1) }
      })
    }
  })(0)
}
...
document.getElementById('bookmarkletCode').onload = function () {
  ...
  readFileLines('urls.txt', function parseFileLines (lines) {
    var urls = 0
    for (var i in lines) { urls += (lines[i].length && lines[i][0] != '#') }
    ...
    detectStreams(lines, detectors, function (r, c, result) {
      if (arguments.length) { table.getCell(r + 1, c + 1).innerHTML = SYMBOLS[result] } else { console.log('done.') }
    })
  })
}
