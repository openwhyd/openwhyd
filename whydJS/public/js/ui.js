...
function WhydLogging () {
  ...
  this.validateField = {
    'name': function ($name) {
      if (!$name.val()/* .trim() */) { return /*! $name.each(error( */'Please enter your name. E.g. John Smith'/* )) */ } else { return null } // $name.each(ok);
    },
    'email': function ($email) {
      if (!emailCheck.test($email.val()/* .trim() */)) // trim not supported by IE8-
      { return /*! $name.each(error( */'Your email address looks wrong...'/* )) */ } else { return null } // $name.each(ok);
    },
    'password': function ($password) {
      var pwd = $password.val()/* .trim() */
      if (pwd.length < 4 || pwd.length > 32) { return /*! $password.each(error( */'Please enter a password between 4 and 32 characters'/* )) */ }
