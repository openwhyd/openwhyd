var postPage = {
  ...
  init: function () {
    var isDynamic = ($('.post').attr('data-pid') == '')
    this.eId = $('.thumb').attr('data-eid')
    if (isDynamic) {
      $('.ago, .author, .ext, .btnLike, .btnComment, .btnShare, .btnRepost, .postEdit, .btns span').hide()
    ...
    this.waitFor('whydPlayer', function (whydPlayer) {
      ...
      whydPlayer.fetchTrackByUrl(eId, function (track) {
        ...
        if (track && track.title) {
          $('.post h2 a').text(track.title).attr('href', track.url || eId)
          $('.btnRepost').attr('href', 'javascript:publishPost(' + JSON.stringify(track) + ');').show()
          cb(track.img)
        } else {
          cb()
        }
  ...
  imagesResolver: function (img) {
    if (img) {
      ...
      } else if (this.eId.indexOf('/dz/') == 0) { img = img.replace(/\/image$/, '/image?size=480x640') } else if (this.eId.indexOf('/ja/') == 0) { img = img.replace(/\/covers\/1\.200\.jpg$/, '/covers/1.600.jpg') }
      var i = new Image()
      i.onload = function () {
        if (i.height >= 120) {
