...
function ProgressBar (p) {
  ...
  $progressTrack.mousedown(function (e) {
    ...
    function moveCursor (e) {
      ...
      if (updateBarOnDrag) { $progressBar.css('width', 100 * (offset_x / width) + '%') }
...
function WhydPlayer () {
  ...
  function setPageTitlePrefix (symbol) {
    ...
    if (spacePos < 3) { window.document.title = window.document.title.substr(spacePos + 1) }
  ...
  function populateTracksFromPosts () {
    ...
    for (var i = 0; i < posts.length; ++i) { addTrackFromAnchor(posts[i].getElementsByTagName('a')[0]) }
  ...
  function playTrackFromFirstAlternativeSource (eId) {
    fetchTrackMetadata(eId, function (track) {
      ...
      for (var i in track.alt) {
        ...
        if (newEid != eId && shortcuts[newEid.substr(0, 4)]) { return playTrackFromAlternativeSource(newEid) } else { console.log('skipping', newEid, '...') }
      }
    })
  }
  ...
  var playemEventHandlers = {
    onError: function (e) {
      ...
      logTrackPlay()
      if (playem.getQueue().length > 1) { playem.next() } else { playem.stop() }
    },
    onReady: function () {
    ...
    loadMore: function (params, cb) {
      if (params || cb) { return loadMore(params, cb) } else if (!isShuffle) {
  ...
  for (var i in playemEventHandlers) { playem.on(i, wrapLogger(i, playemEventHandlers[i])) }
  ...
  for (var prefix in PLAYERS) { players[prefix] = playem.addPlayer(window[PLAYERS[prefix]], defaultDefaultParams) }
  ...
  var exports = {
    ...
    switchSource: function (sourceId) {
      ...
      fetchTrackMetadata(eId, function (res) {
        if (sourceId) { playTrackFromAlternativeSource('/' + sourceId + '/' + res.mappings[sourceId].id) } else if (res && res.mappings) {
          console.log('found the following mappings:')
          for (var src in res.mappings) { console.log('-', src, ':', Math.floor(res.mappings[src].c * 100) + '%') }
        }
      }, true)
    },
    ...
    playPause: function () {
      if (!currentTrack) { self.playAll() } else if (isPlaying) { self.pause() } else { playem.resume() }
    },
    next: function () {
      playem.next()
    },
    ...
    playAll: function (postNode) {
      ...
      if (currentTrack && currentTrack.metadata.post == postNode) { self.playPause() } else { playTrack(trackNumber) }
    },
    updateTracks: function () {
      populateTracksFromPosts()
    },
    like: function () {
      if (currentTrack.metadata) { toggleLovePost(currentTrack.metadata.post.dataset.pid) }
    },
    repost: function () {
      if (currentTrack.metadata) { publishPost(currentTrack.metadata.post.dataset.pid) }
    },
    comment: function () {
      if (currentTrack.metadata) { goToPage('/c/' + currentTrack.metadata.post.dataset.pid) }
    },
    refresh: function () {
      if (currentTrack) {
...
/* loader.whenReady */(function () {
  ...
  if (window.location.href.indexOf('#autoplay') != -1) { window.whydPlayer.playAll() }
