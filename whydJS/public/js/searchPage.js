...
if (window.location.href.indexOf("tab=") != -1) {
	...
	if (tab)
		$("#tab_"+tab).each(selectTab);
}

// merges results from youtube and soundcloud with currently displayed results
$(window).ready((function(query){
	var $body = $("body");
	$body.addClass('loading');
	function renderTrack(track){


		return '<div class="post external '+track.eId.substr(1,2)+'">'
		     +   '<a class="thumb" href="' + track.url+'" target="_blank" data-eid="' + track.eId + '"'
		     +     ' onclick="return playTrack(this);" style="background-image:url(\'' + track.img + '\');">'
		     +     '<img src="' + track.img + '">'
		     +     '<div class="play"></div>'
		     +   '</a>'
		     +   '<h2 ><a  onclick="window.goToPage(\''+ track.eId.split("#")[0] +'\');return false;"  target="_blank">' + htmlEntities(track.name) + '</a></h2>'
		     +   '</a><p class="author">'
		     +     '<span style="background-image:url(\'/images/icon-' + track.playerLabel.toLowerCase() + '.png\');"></span>'
		     +     'Powered by ' + track.playerLabel
		     +   '</p>'
		     +   '<div class="btns">'
		     +     '<a class="btnRepost" href="javascript:publishPost({eId:\'' + track.eId + '\'});">Add to</a>'
			 +   '</div>'
			 + '</div>';
	}
	...
	function displayDynamicSearchResults(){
		...
		document.getElementById("tracks").innerHTML += tracks.map(renderTrack).join("\n");
		trackCounter.innerHTML = trackCounter.innerHTML.replace(""+trackCount, trackCount + tracks.length);
