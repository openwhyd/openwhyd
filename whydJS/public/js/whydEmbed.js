...
(function(){
	...
	function forEachElement (elementName, handler) {
		var els = document.getElementsByTagName(elementName), i;
		for(i = 0; i < els.length; ++i)
			handler(els[i], i);
	}

	// extract parameters
	var urlPrefix, params = (function getScriptParams(fileName){
		...
		return (function parseQueryString(url){
			var params = {};
			url.split("?").pop().split("&").map(function(p){
				p = p.split("=");
				params[p[0]] = p[1];
			});
			return params;
	...
	var view = (function(){
		var that = {
			...
			populateTracks: function(tracks){
				this.tracklistElement.innerHTML = tracks.map(function(d, i){
					d.index = i;
					return t("<li data-wtn=\"{index}\"><span></span>{name}</li>", d);
				}).join("");
			},
			setCurrentTrack: function(metadata){
				this.videoElement.style.backgroundImage = "url("+metadata.img.replace(/[\"\'\(\)\<\>]/gi, "")+")";
			}
		};
		// private functions
		return that;
	...
	function fetchFeed(cb){
		...
		include((params.path.indexOf("//") + 1 ? "" : urlPrefix) + params.path + "?callback=" + cbName);
	}
	...
	function initPlayem(playerContainer, playerId, cb) {
		...
		var playem, PLAYERS = [ "Youtube", "SoundCloud", "Vimeo", "Dailymotion", "Deezer", "AudioFile", "Bandcamp" ],
			PLAYER_PARAMS = {
				playerId: playerId,
	...
	function loadSoundManager(cb){
		...
		include(urlPrefix + "/js/soundmanager2" + (DEBUG ? ".js" : "-nodebug-jsmin.js"), function(){
			soundManager.setup({debugMode: DEBUG, url: urlPrefix + "/swf/soundmanager2_xdomain.swf", flashVersion: 9, onready: function() {
