DeezerExport = (function() {
	...
	var INIT_PARAMS = {
		appId: '128545',
		channelUrl: window.location.href.replace("/mobile/", '/html/deezer.channel.html')
	};
	...
	function login(cb) {
		...
		DZ.login(function(response) {
			...
			if ((response.authResponse || {}).accessToken) {
				...
				console.log('Deezer: User cancelled login or did not fully authorize.');
			}
	...
	var methods = {
		...
		playTracks: function(trackIds, plName){
			checkLoginStatus(function(){
				DZ.api('user/me/playlists', 'POST', {title : plName || "whyd mobile playlist"}, function(response){
					console.log("Deezer: created playlist", response);
					var plId = response.id;
					DZ.api('playlist/'+plId+'/tracks', 'POST', {songs: trackIds}, function(response){
						console.log("Deezer: added tracks", response);
						window.location.href = "deezer://www.deezer.com/playlist/" + plId + "?referrer=whydMobileTrackFinder";
					});
				});
			});
		}
