...
(function ($) {
  ...
  methods = {
    init: function (popin, options) {
      ...
      // Close button event
      popin.find(settings.close).click((function (self) {
        return function (e) {
          e.preventDefault()
    ...
    open: function () {
      if (methods.openedPopin) {
        methods.closePopin.call(this, (function (self) {
          return function () {
    ...
    openPopin: function () {
      ...
      // 'Before' function
      if (typeof settings.beforeOpen === 'function') { settings.beforeOpen.call(this) }
      ...
      settings.popin.addClass('animate-on').fadeIn(settings.speed, function () {
        if (typeof settings.afterOpen === 'function') { settings.afterOpen.call(this) }
    ...
    closePopin: function (callback) {
      ...
      // 'Before' function
      if (typeof settings.beforeClose === 'function') { settings.beforeClose.call(methods.openedPopin) }

      window.popinObject = settings.popin

      settings.popin.addClass('animate-off').fadeOut(settings.speed, function () {
        if (typeof settings.afterClose === 'function') { settings.afterClose.call(methods.openedPopin) }
  ...
  // Custom console.log
  function log () {
    if (window.console && console.log) { console.log('[MaryPopin] ' + Array.prototype.join.call(arguments, ' ')) }
  }

  // Get viewport height
  function getViewportHeight () {
    if (typeof window.innerHeight !== 'undefined') { methods.windowHeight = window.innerHeight } else if (typeof document.documentElement !== 'undefined' &&
		typeof document.documentElement.clientHeight !==
		'undefined' && document.documentElement.clientHeight != 0) { methods.windowHeight = document.documentElement.clientHeight } else { methods.windowHeight = $(window).height() }
  }
  ...
  function globalInit () {
    ...
    methods.mask.click(function () {
      if (methods.openedPopin != undefined) {
        ...
        if (settings.maskClick === true) { methods.close.apply(methods.openedPopin) }
      } else {
    ...
    document.onkeydown = function (evt) {
      ...
      if (evt.keyCode == 27) { methods.close.apply(methods.openedPopin) }
    }
    ...
    $(window).resize(function () {
      if (methods.openedPopin) { positionPopin.apply(methods.openedPopin) }
    })
  }
