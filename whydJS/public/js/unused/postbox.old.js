...
function initPostBox ($box, onPostComplete) {
  ...
  function stopPreview () {
    ...
    if (previewedTrack && previewedTrack.embedType.stop) {
      ...
      previewedTrack = null
    } else if (scPlayer && scReady) { scPlayer.pause() }
  }
  ...
  function previewTune () {
    ...
    embedder.extractEmbedRef(url, function (embedRef) {
      if (embedRef && embedRef.embedType) {
        ...
        if (embedRef.embedType.play) { embedRef.embedType.play(embedRef) } else if (embedRef.id.indexOf('/sc/') == 0) {
  ...
  function checkTune (tune, setEmbedRef) {
    embedder.extractEmbedRef(tune.url, function (embedRef) {
      if (embedRef && embedRef.embedType) { setEmbedRef(embedRef) } else if (embedRef && tune.url.indexOf('http') == 0) {
        embedder.findTracksInPage(tune.name, tune.url, function (r) {
          ...
          if (firstEmbedRef && (firstEmbedRef.id || firstEmbedRef.type)) {
            if (firstEmbedRef.type && firstEmbedRef.type == 'mp3 file') { firstEmbedRef.id = tune.url + '#' + firstEmbedRef.url }
  ...
  function selectTune (e) {
    ...
    checkTune(tune, function (embedRef) {
      ...
      if (embedRef) { whydPost.setEmbedRef(embedRef) } else { window.showMessage && showMessage("Sorry, we can't find that track anymore... Please pick another one.", true) }
      if (tune.pid) { whydPost.setRepostPid(tune.pid) }
  ...
  function onSubmitPost (text) {
    if ($btnSubmit.hasClass('loading')) { return false }
  ...
  var quickSearch = new QuickSearch($box, {
    ...
    submitQuery: function (query, display) {
      ...
      if (query.indexOf('http://') > -1 || query.indexOf('https://') > -1) {
        ...
        // replace set to list
        for (var i in searchEngines) { engines.push(searchEngines[i]) }
        ...
        function triggerSearch (current) {
          engines[current].query(query, function (results) {
            ...
            if (expected == current) {
              for (; expected < engines.length && resultSet[expected]; ++expected) { appendResults(resultSet[expected]) }
