...
function WhydImgUpload (options) {
  ...
  var lastAvatarUrl = null, lastUploadUrl = null
  ...
  this.dispose = function () { // deleteTempAvatar()
    if (lastUploadUrl) {
      $.ajax({
        type: 'POST',
        url: uploadUrl,
  ...
  function onImageUploadComplete (data) {
    ...
    if (typeof data === 'string') {
      if (data.indexOf('{') == -1 && data.toLowerCase().indexOf('too large')) { data = {error: 'Your file is too large. Please resize it and try again.'} } else { data = JSON.parse(data.substring(data.indexOf('{'), data.lastIndexOf('}') + 1)) }
    }
    try {
      ...
      if (img.mime && img.mime.indexOf('image/') == 0) {
        ...
        options.onImageUploadComplete && options.onImageUploadComplete(lastAvatarUrl)
      } else { options.onError('Invalid image. Please try with another image!') }
    } catch (e) {
      options.onError(data.error || 'Upload did not work... Please try again!', e)
    }
  ...
  if (DndUpload) {
    ...
    $avatarDrop.on('dragleave', function (e) {
      if (!$(this).find(destination).size() && !$(this).find(event.target).size()) { $(this).removeClass('hover') }
    })
    var handlers = {
    ...
    var dndUpload = new DndUpload({
      ...
      handler: function (eventName, eventData) {
        ...
        if (handler) { handler(eventData) } else { console.log('DndUpload handler', eventName, eventData) }
      }
    })
  }
