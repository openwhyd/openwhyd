	...
	function onImageUploadComplete(data) {
		...
		if (typeof data == "string") {
			if (data.indexOf('{') == -1 && data.toLowerCase().indexOf("too large"))
				data = {error: "Your file is too large. Please resize it and try again."};
			else
		...
		try {
			...
			if (img.mime && img.mime.indexOf("image/") == 0) {
				...
				options.onError("Invalid image. Please try with another image!");
		}
		catch (e) {
		...
		var dndUpload = new DndUpload({
			...
			handler: function(eventName, eventData) {
				...
				if (handler)
					...
					console.log("DndUpload handler", eventName, eventData);
			}
		});
	}
