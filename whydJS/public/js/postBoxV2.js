...
function WhydPost (embedRef) {
  ...
  this.setEmbedRef = function (embedRef) {
    this.postData.eId = embedRef.eId || (embedRef.id + (embedRef.contentId ? '#' + embedRef.contentId : ''))
  ...
  this.setPlaylist = function (id, name, collabId) {
    ...
    if (collabId) { this.postData.pl.collabId = collabId } else { this.postData.pl.id = id }
  }
...
function WhydPlaylistSelector ($selPlaylist, defaultPlaylist, onSelect) {
  ...
  function bindItems () {
    if (defaultPlaylist) {
      ...
      if ($li.length == 0) { $li = addPlaylist(defaultPlaylist.name, defaultPlaylist.id) }
...
function initPostBox (params) {
  ...
  function initWhydPost (embedRef) {
    ...
    if (reposting) { whydPost.setRepostPid(params.pId) } else if (editingPost) {
  ...
  function tellMixpanel (whydPost) {
    try {
      var submitted = whydPost.postData, stored = whydPost.storedPost
      window.Whyd.tracking.log('Added track' + (addingFromBookmarklet ? ' using bookmarklet' : ''), stored._id)
      if ((submitted.pl || {}).id == 'create') { window.Whyd.tracking.log('Created playlist', stored.pl.id) }
    } catch (e) {
  ...
  function close () {
    if (window.avgrundClose) { avgrundClose() } else { window.close() }
  ...
  function displayConfirmationScreen (posted) {
    ...
    if (pId) {
      ...
      url = url.substr(0, url.indexOf('/', 10)).replace('https', 'http') + '/c/' + pId
      ...
      $confirm.find('.sharing').html('<a href="https://twitter.com/share" class="twitter-share-button"' +
				...
				' data-show-faces="false" data-href="' + htmlEntities(url) + '"></div>')
    }
  }
  ...
  function closeAndShowTrack (posted) {
    avgrundClose()
    if (!posted) { return (window.showMessage || alert)('Oops; an error occurred... Please try again!', true) }
    var url = '/u/' + posted.uId
    if (posted.pl) {
      if (posted.pl.collabId != null) { url = '/playlist/' + posted.pl.collabId } else { url += '/playlist/' + posted.pl.id }
    }
    if (window.showMessage) {
      var plName = posted.pl ? posted.pl.name : 'your tracks'
      showMessage("Successfully added track to <a target='_blank' href='" + url + "'>" + encodeHtmlEntities(plName) + '</a>')
    } else if (url) { window.location.href = url } else { window.location.reload() }
  }

  function onPostSuccess (postId, whydPost) {
    var posted = whydPost.storedPost
    if (!posted) { console.log('error from post api:', postId, whydPost) } else { tellMixpanel(whydPost) }
    if (addingFromBookmarklet) { displayConfirmationScreen(posted) } else { closeAndShowTrack(posted) }
  }
  ...
  function populateTrackUi (embedRef) {
    ...
    $('#contentThumb').css('background-image', 'url("' + (embedRef.img || '/images/cover-track.png') + '")')
    $('.whydSrcLogo').css('background-image', 'url("/images/icon-' + embedRef.playerLabel.toLowerCase().split(' ')[0] + '.png")')
    new WhydPlaylistSelector($('#selPlaylist'), whydPost.postData.pl, whydPost.setPlaylist.bind(whydPost))
  ...
  function makePlayemStreamDetector (eidSet) {
    ...
    function getPlayerId (url) {
      for (var i in players) {
        if (players[i].getEid(url)) { return i }
      }
    }
    ...
    return function detect (url, cb) {
      detectEid(url, function (eId, player) {
        ...
        if (!player.fetchMetadata) // e.g. MP3 files
        {
          return cb({
            eId: eId,
  ...
  function imageToHD (track) {
    if (track.img) {
      if (track.eId.substr(1, 2) == 'yt') {
        ...
        i.onload = function () {
          if (i.height >= 120) {
            document.getElementById('contentThumb').style.backgroundImage = 'url(' + img + ')'
          }
        }
        i.src = img
      } else if (track.eId.substr(1, 2) == 'sc') {
        track.img = track.img.replace('-large', '-t300x300')
      } else if (track.eId.indexOf('/dz/') == 0) { track.img = track.img.replace(/\/image$/, '/image?size=480x640') } else if (track.eId.indexOf('/ja/') == 0) { track.img = track.img.replace(/\/covers\/1\.200\.jpg$/, '/covers/1.600.jpg') }
    }
    return track
  }
  ...
  function init () {
    ...
    makePlayemStreamDetector()(params.embed.replace(/\&amp\;/g, '&'), function (track) {
      console.log('postBox detected track:', track)
      if ((track || {}).eId) {
        track.src = params.src
        track.name = $('#contentTitle').text() // || $("#contentTitleInput").val();
        if (!track.name || track.name == 'undefined') // weak equality necessary because of text()
