...
function WhydPost(embedRef) {
	...
	this.setPlaylist = function(id, name, collabId) {
		...
		if (collabId)
			this.postData.pl.collabId = collabId;
		else
	...
	this.submit = function(shareOnFb, onPostComplete) {
		...
		$.ajax({
			...
			success: function(post){
				...
				if (onPostComplete)
					onPostComplete(post._id, that/*postData*/);
				if (shareOnFb && fbAction)
					fbAction("add", "/c/" + post._id, "track");
			},
			error: function(cause){
				console.log("post error:", cause);
				that.storedPost = null;
				if (onPostComplete)
					onPostComplete(null, that);
			}
		});
	}
}
...
function WhydTextWithMentions(textArea) {
	...
	function submitSearchQuery(q, cb) {
		$.ajax({ type: "GET", url: "/search", data: q, complete: function(res, status) {
			try {
...
function initPostBox(params){
	...
	function initWhydPost(embedRef){
		...
		if (params.ctx)
			whydPost.setContext(params.ctx);
		if (reposting)
			whydPost.setRepostPid(params.pId);
		else if (editingPost) {
			whydPost.set_id(params.pId);
			if (params.pl)
				whydPost.setPlaylist(params.pl.id, params.pl.name);
		}
		return whydPost;
	}
	...
	function closeAndShowTrack(posted){
		...
		if (posted.pl) {
			if (posted.pl.collabId != null)
				url = "/playlist/" + posted.pl.collabId;
			else
	...
	function onPostSuccess(postId, whydPost) {
		...
		if (!posted)
			console.log("error from post api:", postId, whydPost);
		else
			tellMixpanel(whydPost);
		if (addingFromBookmarklet)
			displayConfirmationScreen(posted);
		else
			closeAndShowTrack(posted);
	}
	...
	function populateTrackUi(embedRef){
		...
		if (!(embedRef||{}).eId)
			return displayConfirmationScreen(false);
