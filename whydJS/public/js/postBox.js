...
function WhydPlaylistSelector (whydPost, $selPlaylist, defaultPlaylist) {
  ...
  function hideMenu (e) {
    ...
    if (!e || !($playlistHead[0] == e.target || $playlistHead[0] == e.target.parentNode ||
				$playlistMenu.find('form')[0] == e.target.parentNode)) { $playlistMenu.hide() }
  }
  function showMenu () {
    $playlistMenu.show()
  ...
  function selectPlaylist () {
    ...
    whydPost.setPlaylist($li.attr('data-plid'), $li.text(), $li.attr('data-collabid'))
  ...
  function bindItems () {
    if (defaultPlaylist) {
      var $li = $ul.find(defaultPlaylist.collabId ? "li[data-collabid='" + defaultPlaylist.collabId + "']"
        : "li[data-plid='" + defaultPlaylist.id + "']")
      if ($li.length == 0) { $li = addPlaylist(defaultPlaylist.name, defaultPlaylist.id, defaultPlaylist.collabId) }
...
function initPostBox (params) {
  ...
  function onTrack (embedRef) {
    ...
    if (reposting) { whydPost.setRepostPid(params.pId) } else if (editingPost) {
      ...
      if (params.pl) { whydPost.setPlaylist(params.pl.id, params.pl.name/*, "{{collabId}}" */) }
    }

    (new WhydPlaylistSelector(whydPost, null, whydPost.postData.pl)).bindItems()

    $('#contentSource').text('Source: ' + embedRef.playerLabel)
      .addClass(embedRef.playerLabel.toLowerCase())

    // init ui
    $('#contentThumb > div > div').html('<img src="' + (embedRef.img || '/images/cover-track.png') + '"/>')
    $('#contentTitle').text(embedRef.name)
    var $titleInput = $('#contentTitleInput').val(embedRef.name)

    new WhydTextWithMentions(
      document.getElementById('text'), document.getElementById('btnSubmit'), function (text) {
  ...
  $(function () {
    ...
    makePlayemStreamDetector()(params.embed.replace(/\&amp\;/g, '&'), function (track) {
      ...
      if (!(track || {}).eId) {
        ...
        $('#btnSubmit').hide()
        return (window.showMessage || alert)('Oops; an error occurred... Please try again!', true)
      }
