$(function () {
  ...
  $('#tabAccount form').submit(function (event) {
    ...
    (function next (ok) {
      ...
      if (step) { step(next) } else { showMessage(allOk ? 'Your changes were successfully applied' : 'Please fix your settings and try again', !allOk) }
    })(true)
  })
  ...
  (function () {
    ...
    function toggleLastFmConnection (connected) {
      ...
      if (connected && window.user.lastFm) { $('#lastFmProfile').text(window.user.lastFm.name).attr('href', 'http://lastfm.com/user/' + window.user.lastFm.name) }
    }
