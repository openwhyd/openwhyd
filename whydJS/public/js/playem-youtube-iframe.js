...
(function () {
  ...
  function Player (eventHandlers, embedVars) {
    ...
    window.addEventListener('message', function (e) {
      if (e.origin === IFRAME_HOST) {
        ...
        if (message.code == 'onApiReady') { that.safeCall('play', that.embedVars.videoId) } else { that.safeClientCall(message.code, param) }
      }
    })
    this.isReady = true
  ...
  Player.prototype.safeCall = function (fctName, param) {
    if (!this.iframeReady) { return console.warn('YT-iframe not ready => ignoring call to', fctName) }
    try {
