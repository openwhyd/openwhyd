...
function ContentEmbed () {
        ...
        $.getJSON(API_PREFIX + '/track/3/info?track_id=' + trackId + API_SUFFIX, function (data) {
          ...
          function fetchBandName (cb) {
            ...
            $.getJSON(API_PREFIX + '/band/3/info?band_id=' + data.band_id + API_SUFFIX, function (data) {
              ...
              if (bandName) { embedRef.name = bandName + ' - ' + embedRef.name }
              cb()
            })
          }
  ...
  var youtubeEmbed = {
    ...
    require: function (embedRef, callback) {
      ...
      $.getJSON('https://www.googleapis.com/youtube/v3/videos?id=' + embedRef.videoId + '&part=snippet&key=' + YOUTUBE_API_KEY + '&callback=?', function (data) {
        if (data && (data.items || []).length) { embedRef.name = (data.items[0].snippet || {}).title }
        callback(embedRef)
      })
    },
  ...
  var dailymotionEmbed = {
    ...
    require: function (embedRef, callback) {
      ...
      $.getJSON('https://www.dailymotion.com/services/oembed?format=json&url=' + url + '&callback=?', function (data) {
        ...
        if (embedRef.url) { embedRef.videoId = decodeURIComponent(embedRef.url).split('/').pop() }
        callback(embedRef)
      })
    },
    render: function (embedRef, options, callback) {
  ...
  return {
    ...
    renderEmbed: function (embedId, options) {
      if (options) {
        for (var i in embedDefaults) { options[i] = options[i] != undefined ? options[i] : embedDefaults[i] }
      }
