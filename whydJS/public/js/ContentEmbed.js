...
function ContentEmbed () {
	...
	var generalUrl = /\/\/[-A-Z0-9+&@#$*'()\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|]/ig; // \b(?:https?|ftp|file):
	...
	var jamendoEmbed = {
		...
		detect: function(url, cb) {
			cb(/jamendo.com\/.*track\/(\d+)/.test(url) ? {videoId: RegExp.$1} : null);
		},
		...
		require: function(embedRef, callback) {
			$.ajax({
				url: '//api.jamendo.com/v3.0/tracks?client_id=' + JAMENDO_CLIENT_ID + '&id=' + embedRef.videoId,
	...
	var bandcampEmbed = (function(){
		...
		function detect(url, cb) {
			...
			if (!matches || matches.length < 3)
				return cb();
			var bandUri = matches[1], trackUri = matches[2];
			$.getJSON(API_PREFIX + '/url/1/info?url=' + encodeURIComponent(url) + API_SUFFIX, function(data) {
				...
				$.getJSON(API_PREFIX + '/track/3/info?track_id=' + trackId + API_SUFFIX, function(data) {
					...
					if (!data.streaming_url)
						return cb();
					var embedRef = {
						id: "/bc/" + [bandUri, trackUri, trackId].join("/") + "#" + data.streaming_url,
						title: data.title,
					...
					function fetchBandName(cb) {
						if (!data.band_id)
							return cb();
						$.getJSON(API_PREFIX + '/band/3/info?band_id=' + data.band_id + API_SUFFIX, function(data) {
					...
					function fetchAlbumArt(cb) {
						if (embedRef.img || !data.album_id)
							return cb();
						$.getJSON(API_PREFIX + '/album/2/info?album_id=' + data.album_id + API_SUFFIX, function(data) {
							data = data || {};
			...
			detect: function(url, cb) {
				for (var i in regexs)
					if (regexs[i].test(url))
						return cb({url: url, videoId: RegExp.$1});
				cb();
			},
			getHref: function(embedRef) {
				return this.url;
	...
	var spotifyEmbed = oEmbed("sp", "Spotify", "//embed.spotify.com/oembed/?url=", [
		/spotify\.com\/track\/(\w+)/i,
		/spotify\:track\:(\w+)/i,
	]);
	...
	var youtubeEmbed = {
		...
		render: function(embedRef, options, callback) {
			...
			var url = "https://youtube.com/embed/" + embedRef.videoId + '?wmode=opaque&amp;controls=0;autoplay=' + (options.autoplay ? 1 : 0);
	...
	var soundcloudEmbed = {
		...
		detect: function(url, cb) {
			...
			if (url.indexOf("/sc/") == 0) // ex: /sc/silkmusik/raw-percussion-build#https://api.soundcloud.com/tracks/60886771				
				url = url.replace("/sc/", "//soundcloud.com/");
			if (url.match(this.regex) || url.match(this.regexShort))
				cb({ url: url });
		...
		require: function(embedRef, callback) {
			...
			var url = 'https://api.soundcloud.com/resolve.json?url='+encodeURIComponent("http://"+embedRef.url.split("//").pop())+'&client_id='+scClientId+'&callback=?';
			$.getJSON(url, function(data, status) {
				if (!data || !data.id)
					return callback();
				embedRef.name = data.title;
				embedRef.id = "/sc/" + data.permalink_url.split("soundcloud.com\/").pop() + "#" + data.uri; // includes data.id
				embedRef.img = data.artwork_url;
				embedRef.name = (data.title && data.user && data.title.indexOf(" - ") == -1 ? data.user.username + " - " : "") + data.title;
				callback(embedRef);
			});
		},
		render: function(embedRef, options, callback) {
			...
			var html = '<iframe src="'+url+'" width="'+options.width+'" height="166" scrolling="no" frameborder="0" kwframeid="4"></iframe>';
			if (callback) callback(html);
			return html;
		}
	};
	...
	var dailymotionEmbed = {
		...
		require: function(embedRef, callback) {
			...
			$.getJSON("https://www.dailymotion.com/services/oembed?format=json&url=" + url + "&callback=?", function(data){
				...
				if (embedRef.url)
					embedRef.videoId = decodeURIComponent(embedRef.url).split("/").pop();
				callback(embedRef);
			});
		},
		render: function(embedRef, options, callback) {
	...
	var embedDetectors = [];
	for(var i in embedTypes)
		embedDetectors.push(embedTypes[i]);
	...
	return {
		...
		findTracksInPage: function (title, url, cb) {
			...
			$.ajax({
				...
				success: function(r){
					if (r && r.error)
						console.log("findTracksInPage error", r.error);
					cb(r);
				}
			});
		}
