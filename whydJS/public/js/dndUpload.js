...
function DndUpload (options) {
  ...
  var handler = options.handler || function (eventName, eventData) {
    if (options[eventName]) { options[eventName](eventData, this) } else { console.log('DndHandler', eventName, eventData) }
  }

  if (!('draggable' in document.createElement('span') && !!window.FormData && 'upload' in new XMLHttpRequest())) { return handler('error', 'drag&drop, formdata and upload progress are not supported') }
  ...
  holder.ondrop = function (e) {
    ...
    if (formFields) {
      for (var i = formFields.length - 1; i >= 0; --i) {
        if (formFields[i].type != 'file') { formData.append(formFields[i].name, formFields[i].value) }
      }
    }
