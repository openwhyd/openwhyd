	...
	$.ajax({ type: "GET", url: "/api/follow",
		...
		success: function() {
			...
			if (subscribing)
				...
				window.Whyd.tracking.log("Unfollowed", uid);
		}
	});
}
...
function _fetchUserInfo(uid, action, callback) {
	$.ajax({
		type: "GET", url: "/api/user/"+uid+"/"+action,
...
function _fetchPostInfo(pid, action, callback) {
	$.ajax({
		type: "GET", url: "/api/post/"+pid+"/"+action,
		success: callback, dataType: "json"
	});
}
	...
	getPostById(pId, function($post){
		...
		function updateButton(result) {
			...
			if (result.loved) {
				...
				$button.removeClass("selected").text("Like");
...
function loadTop() {
	...
	if (window.prevPageUrl && window.prevPageUrl.split("before=")[1])
		...
		goToPage(window.location.href); //refresh the page (e.g. when posting a first track on a new playlist)
}
	...
	if (user.id) {
		if (window.user && window.user.id == user.id)
			...
			activateSubscribeButton(($("<span class='userSubscribe'>")
				...
				.toggleClass("subscribed", user.subscribed ? true : false)).appendTo($li));
	}
}
...
function updatePostStats($post, $ext){
	...
	var hasReposts = (parseInt($post.find(".nbReposts span").text()) == 0 )?false:true;
	var hasLikes = (parseInt($post.find(".nbLoves span").text()) == 0 )?false:true;
		...
		$ext.find("form").unbind("submit").submit(function(e){
			...
			$textField.mentionsInput('val', function(text) {
				...
				addComment(pId, text, function(c) {
					...
					if (c.error)
						showMessage("Error: " + c.error, true);
					else {
...
function modalRepostBox(trackOrPid, onPosted) {
	...
	if (typeof trackOrPid == "string")
		url += '/' + trackOrPid /*+'/add'*/; // ?pid='+pId; //postData.pId+'&embed='+postData.eId+'&text='+postData.text;
...
var refreshNotifCounter = function () {
	...
	if (total == 0)
		...
		$notifIcon.addClass("someNotif");
};

var fetchNotifs = function () {
	$.ajax({
		type: "GET", url: "/api/notif",
		cache: false,
...
var renderNotif = function (notif) {
	...
	var imgId;
	if (notif.lastAuthor.id)
		imgId = "/u/" + notif.lastAuthor.id;
	else if ((notif.pId || "").indexOf("/") == 0)
		imgId = notif.pId;
	else
		imgId = "/u/" + notif.pId.split("/")[0];
...
function onPageLoad() {
	var $body = $("body");
	if (window.location.href.indexOf("/welcome") == window.location.href.length - 8)
		showHelpOverlay();
	...
	*/
	...
	var keyUpShortcuts = {
		...
		37: function() { // left arrow => previous (playlist) page
			if (window.prevPageInList)
				goToPage(window.prevPageInList);
		},
		39: function() { // right arrow => next (playlist) page
			if (window.nextPageInList)
				goToPage(window.nextPageInList);
		},
	}
					...
					if (!foundTracks) {
						...
						window.searchExternalTracks(query, function(track){
							if (track){
								...
								display(prependExternalTracks(externalTracks.slice(0, 2), resultsHtml), false);
						});
					}
				});
			}
				...
				success: function(data, textStatus, jqXHR){
					...
					$scripts.each(function(){
						...
						if (src)
							...
							scriptNode.appendChild(document.createTextNode(/*$script.text()*/this.innerHTML));
