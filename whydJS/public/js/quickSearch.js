...
function toggleClass (elem, className, showOrHide) {
  ...
  elem.className = newClassName + (showOrHide || newClassName == className ? ' ' + className : '')
  return elem
}
...
function QuickSearch (searchForm, options) {
  ...
  var searchResults = searchForm.getElementsByClassName('searchResults')[0] || document.createElement('div')
  ...
  function unhoverItem () {
    ...
    for (var i = liArray.length - 1; i >= 0; --i) { toggleClass(liArray[i], 'hover', false) }
  }

  function hoverItem () {
    toggleClass(searchResults.getElementsByTagName('li')[selected], 'hover', true)
  }
  ...
  function onResults (html, stillSearching, destination) {
    ...
    if (searchClear && !stillSearching) { toggleClass(searchClear, 'loading', false) }
    ...
    if (options.onResultClick) {
      ...
      for (var i = anchors.length - 1; i >= 0; --i) { addEvent(anchors[i], 'click', makeHandler(anchors[i])) }
    }

    nbResults = anchors.length
    if (!nbResults || params.noDefaultSelection) { selected = -1 } else {
      if (selected == null || selected < 0 || selected >= nbResults) { selected = 0 }
      updateSelection(0)
    }
  }
  ...
  function onQueryChange (event) {
    ...
    if (event.keyCode == '13') { // return
      ...
      if (nbResults != null && nbResults > 0 && selected != null && selected > -1) {
        var a = searchResults.getElementsByTagName('a')[selected]
        if (a.onclick) { a.onclick(event) }
        if (options.onResultClick) { return options.onResultClick(a.href, a) } else { window.location.href = a.href }
      } else if (options.noMoreResultsOnEnter || params.noMoreResultsOnEnter) {} // do nothing
      else if (!params.noMoreResults) { window.location.href = '/search?q=' + encodeURIComponent(searchField.value) } else {
        cancelQuery()
        hideResults()
      }
      ...
      if (val.length == 0) {
        hideResults()
        searchClear && (toggleClass(searchClear, 'loading', false).style.display = 'none')
