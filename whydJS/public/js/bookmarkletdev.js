  ...
  var PAGE_DETECTORS = [
    ...
    function detectTrackFromTitle () {
      ...
      for (var i = 0; i < titleParts.length; ++i) {
        if (title.indexOf(titleParts[i]) > -1) { return [{name: title.replace(titleParts[i], '')}] }
      }
    }
  ]
  ...
  function detectTracks (ui, URL_DETECTORS) {
    ...
    var toDetect = new function ElementStack () {
      ...
      this.push = function (elt) {
        ...
        if (!elt || !url || url.indexOf('javascript:') == 0 || url.indexOf('#') == 0) { return }
        set[url] = elt
      }
    ...
    PAGE_DETECTORS.map(function (detect) {
      ...
      res.map(function (track) {
        if (track && track.name) { ui.addSearchThumb(track) } else if (track && track.src) { toDetect.push(track) }
      })
    })
