...
(function (API_KEY) {
  ...
  Player.prototype.playStreamUrl = function (url) {
    var self = this
    if (!url) { return self.clientCall('onError', self, {source: 'BandcampPlayer', code: 'no_stream'}) }
    url = 'http://' + url.split('//').pop()
  ...
  Player.prototype.play = function (id) {
    var self = this
    if (isStreamUrl(id)) { this.playStreamUrl(id) } else {
      fetchStreamUrl(id, function (err, url) {
        if (err || !url) { self.clientCall('onError', self, { source: 'BandcampPlayer', error: (err || {}).error_message }) } // e.g. "bad api key"
...
(function () {
  ...
  function Player (eventHandlers, embedVars) {
    ...
    window.onDailymotionStateChange = function (newState) {
      if (newState > 0 || !ignoreEnded) { that.safeClientCall(EVENT_MAP[newState], that) } else { --ignoreEnded }
      /* if (newState == 1) {
...
(function () {
  ...
  // pos: seconds
  Player.prototype.setTrackPosition = function (pos) {
    if (this.sound) { this.sound.setPosition(Math.round(pos * 1000)) } else { DZ.player.seek(Math.round(100 * pos / this.currentTrack.duration)) }
  }

  //= ===========================================================================
  // vol: float between 0 and 1
  Player.prototype.setVolume = function (vol) {
    if (this.sound) { this.sound.setVolume(Math.round(vol * 100)) } else { DZ.player.setVolume(Math.round(vol * 100)) }
  }
  ...
  function createSound (self, url) {
    return soundManager.createSound({
      ...
      whileplaying: function () {
        ...
        if (self.eventHandlers.onTrackInfo) { self.eventHandlers.onTrackInfo(self.currentTrack) }
      },
      onplay: function () {
...
(function () {
  ...
  Player.prototype.setVolume = function (vol) {
    if (this.widget && this.widget.setVolume && this.soundOptions) { soundManager.setVolume(this.soundOptions.id, 100 * vol) }
  }
...
(function () {
  ...
  function searchTracks (query, limit, cb) {
    function waitFor (objName, cb) {
      setTimeout(function () {
        if (window[objName]) { cb(window[objName]) } else { waitFor(objName, cb) }
      }, 200)
    }
  ...
  Player.prototype.fetchMetadata = function (url, cb) {
    ...
    fetchMetadata(url, function (data) {
      if (data && data.kind == 'track') {
        embed.id = '' + data.id
        embed.eId = '/sc/' + data.permalink_url.substr(data.permalink_url.indexOf('/', 10) + 1) /* "/" + data.id + */ +
          '#' + data.stream_url
        embed.img = data.artwork_url || embed.img
        embed.title = data.title
        if (embed.title.indexOf(' - ') == -1 && (data.user || {}).username) { embed.title = data.user.username + ' - ' + embed.title }
      }
      cb(embed)
    })
  }
...
(function () {
      ...
      if (data.player_id == this.embedVars.playerId) {
        if (data.method == 'onLoad') {
          ...
          setTimeout(function () {
            ...
            if (typeof eventHandler === 'function') { eventHandler.apply(that, [that].concat(data.data)) } else { console.warn('vimeo missing handler for event', data.method) }
          })
        }
      }
    } catch (e) {
  ...
  function Player (eventHandlers, embedVars) {
    ...
    this.isReady = false
    this.trackInfo = {}
    if (window.addEventListener) { window.addEventListener('message', onMessageReceived.bind(this), false) } else { window.attachEvent('onmessage', onMessageReceived.bind(this), false) }
  ...
  Player.prototype.stop = function () {
    ...
    if ((this.element || {}).parentNode) { this.element.parentNode.removeChild(this.element) }
    if ((this.otherElement || {}).parentNode) { this.otherElement.parentNode.removeChild(this.otherElement) }
  }
...
(function () {
  ...
  function Player (eventHandlers, embedVars) {
    ...
    window.onYoutubeStateChange = function (newState) {
      ...
      if (eventName && that.eventHandlers[eventName]) { that.eventHandlers[eventName](that) }
    }
  ...
  Player.prototype.fetchMetadata = function (url, cb) {
    var id = this.getEid(url)
    if (!id) { return cb() } else { fetchMetadata(id, cb) }
  }
