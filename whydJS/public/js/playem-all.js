  ...
  */
  ...
  function Player(eventHandlers, embedVars) {
    ...
    this.soundOptions = {
      ...
      ontimeout: function(e) {
        ...
        that.eventHandlers.onError &&
          that.eventHandlers.onError(that, {
            code: 'timeout',
            source: 'AudioFilePlayer'
          });
      }
    };
  ...
  Player.prototype.getTrackPosition = function(callback) {
    ...
    this.getTrackInfo(function() {
      ...
      that.eventHandlers.onTrackInfo &&
        that.eventHandlers.onTrackInfo(that.trackInfo);
    });
  };
  ...
  Player.prototype.setTrackPosition = function(pos) {
    this.widget &&
      this.widget.setPosition(
        Math.floor(Math.min(this.widget.duration, pos * 1000) - 2000)
      );
  };

  Player.prototype.embed = function(vars) {
    if (!vars || !vars.trackId) return;
    //console.log("AudioFilePlayer embed vars:", vars);
    this.embedVars = vars = vars || {};
    this.soundOptions.id = vars.playerId =
      vars.playerId || 'mp3Player' + new Date().getTime();
    ...
    this.eventHandlers.onTrackInfo &&
      this.getTrackInfo(this.eventHandlers.onTrackInfo);
    this.play();
  };
...
(function(API_KEY) {
  ...
  function makeEidFromUrl(url) {
    ...
    return (
      match && match[0] + '/' + match[1] + (streamUrl ? '#' + streamUrl : '')
    );
  }
  ...
  Player.prototype.playStreamUrl = function(url) {
    var self = this;
    if (!url)
      return self.clientCall('onError', self, {
        source: 'BandcampPlayer',
        code: 'no_stream'
      });
    url = 'http://' + url.split('//').pop();
  ...
  Player.prototype.play = function(id) {
    var self = this;
    if (isStreamUrl(id)) this.playStreamUrl(id);
    else
      fetchStreamUrl(id, function(err, url) {
        if (err || !url)
          self.clientCall('onError', self, {
            source: 'BandcampPlayer',
            error: (err || {}).error_message
          });
        // e.g. "bad api key"
        else this.playStreamUrl(url);
      });
  };
...
(function() {
  ...
  function Player(eventHandlers, embedVars) {
    ...
    window.onDailymotionError = function(error) {
      ...
      that.safeClientCall('onError', that, {
        source: 'DailymotionPlayer',
        data: error
      });
    };
    ...
    window.onDailymotionPlayerReady = function(playerId) {
      that.element = /*that.element ||*/ document.getElementById(
        playerId
      ); /* ytplayer*/
      that.element.addEventListener(
        'onStateChange',
        'onDailymotionStateChange'
      );
  ...
  Player.prototype.embed = function(vars) {
    ...
    paramsQS = Object.keys(swfParams)
      .map(function(k) {
        // query string
    ...
    embedAttrs = {
      ...
      data:
        ...
        '//www.dailymotion.com/swf/' +
        this.embedVars.videoId +
        '?' +
        paramsQS,
    ...
    if (USE_SWFOBJECT) {
      swfobject.embedSWF(
        embedAttrs.data,
        this.embedVars.playerId,
        embedAttrs.width,
        embedAttrs.height,
        '9.0.0',
        '/js/swfobject_expressInstall.swf',
        null,
        params,
        atts
      );
  ...
  function fetchMetadata(id, cb) {
    ...
    var url = encodeURIComponent(
        'http://www.dailymotion.com/embed/video/' + id
      ),
    ...
    loader.includeJS(
      '//www.dailymotion.com/services/oembed?format=json&url=' +
        url +
        '&callback=' +
        callbackFct
    );
...
(function() {
  ...
  function fetchMetadata(id, cb) {
    ...
    loader.includeJS(
      '//api.deezer.com/track/' + id + '?output=jsonp&callback=' + callbackFct
    );
  }
  ...
  // pos: seconds
  Player.prototype.setTrackPosition = function(pos) {
    if (this.sound) this.sound.setPosition(Math.round(pos * 1000));
    else DZ.player.seek(Math.round((100 * pos) / this.currentTrack.duration));
  };

  //============================================================================
  // vol: float between 0 and 1
  Player.prototype.setVolume = function(vol) {
    if (this.sound) this.sound.setVolume(Math.round(vol * 100));
    else DZ.player.setVolume(Math.round(vol * 100));
  };

  //============================================================================
  function loadSDK(cb) {
    var dz;
    if (window.DZ) return cb();
  ...
  Player.prototype.init = function(onload) {
    ...
    DZ.init({
      ...
      player: {
        onload: function() {
          if (window.location.protocol === 'https:') DZ.override_https();
  ...
  function hookHandlers(self) {
    DZ.Event.subscribe('player_position', function(eventObject) {
      ...
      if (duration - position <= 1.5 && onEndedHandler) onEndedHandler(self);
    });
    ...
    for (var e in EVENT_MAP) DZ.Event.suscribe(e, createHandler(e));
  }
  ...
  function createSound(self, url) {
    return soundManager.createSound({
      ...
      onplay: function() {
        if (self.eventHandlers.onPlaying) self.eventHandlers.onPlaying(self);
      },
      onresume: function() {
        if (self.eventHandlers.onPlaying) self.eventHandlers.onPlaying(self);
      },
      onfinish: function() {
        if (self.eventHandlers.onEnded) self.eventHandlers.onEnded(self);
      }
    });
  }
...
(function() {
  ...
  Player.prototype.getEid = function(url) {
    return /jamendo.com\/.*track\/(\d+)/.test(url) || /\/ja\/(\d+)/.test(url)
      ? RegExp.$1
      : null;
  };
  ...
  Player.prototype.embed = function(vars) {
    ...
    this.soundOptions.id = vars.playerId =
      vars.playerId || 'mp3Player' + new Date().getTime();
    this.soundOptions.url =
      '//api.jamendo.com/v3.0/tracks/file?client_id=' +
      JAMENDO_CLIENT_ID +
      '&action=stream&audioformat=mp32&id=' +
      vars.trackId;
    this.trackInfo = {};
    if (this.widget) {
      this.pause();
...
(function() {
  var EVENT_MAP = {
    ...
    ERROR_EVENTS = ['onerror', 'ontimeout', 'onfailure', 'ondataerror'],
  ...
  function Player(eventHandlers, embedVars) {
    ...
    function init() {
      ...
      try {
        ...
        console.warn(
          'warning: soundManager was not found => playem-soundcloud will not be able to stream music'
        );
        that.callHandler('onApiReady', that);
      }
    }
  ...
  Player.prototype.safeCall = function(fctName, param) {
    try {
      ...
      if (this.widget && this.widget[fctName]) this.widget[fctName](param);
    } catch (e) {
  ...
  function unwrapUrl(url) {
    return /(soundcloud\.com)\/player\/?\?.*url\=([^\&\?]+)/.test(url)
      ? decodeURIComponent(RegExp.lastParen)
      : url.replace(/^\/sc\//, 'http://soundcloud.com/');
  }
  ...
  Player.prototype.getEid = function(url) {
    ...
    } else if (/snd\.sc\/([\w-_]+)/.test(url)) return RegExp.lastMatch;
    // => returns:
  ...
  function searchTracks(query, limit, cb) {
    function waitFor(objName, cb) {
      setTimeout(function() {
        if (window[objName]) cb(window[objName]);
        else waitFor(objName, cb);
      }, 200);
    }
    ...
    function translateResult(r) {
      ...
      return {
        eId:
          '/sc' +
          r.permalink_url.substr(r.permalink_url.indexOf('/', 10)) +
          '#' +
          r.uri,
        img: r.img || r.artwork_url || '/images/cover-soundcloud.jpg',
        url: r.url || r.permalink_url + '#' + r.uri,
        title:
          (r.title.indexOf(' - ') == -1 ? r.user.username + ' - ' : '') +
          r.title,
  ...
  function fetchMetadata(url, cb) {
    ...
    method = !!navigator.userAgent.match(/Version\/[\d\.]+.*Safari/)
      ? 'loadJSONP'
      : 'loadJSON';
    if (trackId)
      loader[method](
      ...
      loader[method](
        RESOLVE_URL +
          ...
          SOUNDCLOUD_CLIENT_ID +
          '&url=' +
          encodeURIComponent('http://' + url.replace(/^(https?\:)?\/\//, '')),
        cb
      );
  }
  ...
  Player.prototype.fetchMetadata = function(url, cb) {
    var embed = {};
    if (!this.getEid(url)) return cb();
    fetchMetadata(url, function(data) {
      if (data && data.kind == 'track') {
        embed.id = '' + data.id;
        embed.eId =
          '/sc/' +
          data.permalink_url.substr(data.permalink_url.indexOf('/', 10) + 1) +
          /*"/" + data.id +*/ '#' +
          data.stream_url;
  ...
  Player.prototype.play = function(id) {
    ...
    if (id.indexOf('/tracks/') == 0) return playId(id);
...
(function() {
  ...
  Player.prototype.embed = function(vars) {
    ...
    loader.loadJSON(
      'https://api.spotify.com/v1/tracks/' + vars.trackId,
      function(data) {
        ...
        that.eventHandlers.onEmbedReady &&
          that.eventHandlers.onEmbedReady(that);
        that.eventHandlers.onTrackInfo &&
          that.getTrackInfo(that.eventHandlers.onTrackInfo);
        that.play();
      }
    );
  };
...
(function() {
    ...
    try {
      ...
      if (data.player_id == this.embedVars.playerId) {
        if (data.method == 'onLoad') {
          ...
          setTimeout(function() {
            var eventHandler =
              that.eventHandlers[EVENT_MAP[data.event]] ||
              EVENT_MAP[data.event];
            if (typeof eventHandler == 'function')
              eventHandler.apply(that, [that].concat(data.data));
            else console.warn('vimeo missing handler for event', data.method);
          });
      }
    } catch (e) {
      console.log('VimeoPlayer error', e, e.stack);
      this.eventHandlers.onError &&
        this.eventHandlers.onError(this, {
          source: 'VimeoPlayer',
          exception: e
        });
    }
  ...
  function Player(eventHandlers, embedVars) {
    ...
    else window.attachEvent('onmessage', onMessageReceived.bind(this), false);
  ...
  Player.prototype.post = function(action, value) {
    ...
    try {
      return this.element.contentWindow.postMessage(
        JSON.stringify(data),
        this.element.src.split('?')[0]
      );
    } catch (e) {
      console.log(e);
    }
  ...
  Player.prototype.getEid = function(url) {
    return (
      /(vimeo\.com\/(clip\:|video\/)?|\/vi\/)(\d+)/.test(url) &&
      RegExp.lastParen
    );
  };

  function fetchMetadata(id, cb) {
    loader.loadJSON('https://vimeo.com/api/v2/video/' + id + '.json', function(
      data
    ) {
      cb(
        !data || !data.map
  ...
  Player.prototype.embed = function(vars) {
    ...
    for (i in attributes) this.element.setAttribute(i, attributes[i]);
    this.embedVars.playerContainer.innerHTML = '';
    this.embedVars.playerContainer.appendChild(this.element);
    // TODO: wait for this.element.contentWindow.postMessage to be ready to be called
    if (this.eventHandlers.onEmbedReady) this.eventHandlers.onEmbedReady();
  };
  ...
  Player.prototype.stop = function() {
    if (this.element) this.post('unload');
    if ((this.element || {}).parentNode)
      this.element.parentNode.removeChild(this.element);
