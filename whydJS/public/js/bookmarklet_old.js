...
(function () {
  ...
  function forEachElement (elementName, handler) {
    ...
    var l = 0 + els.length
    var count = 0
    for (var i = 0; i < l; i++) { count += handler(els[i]) }
    return count
  }
  ...
  var urlPrefix = findScriptHost('/js/bookmarklet.js') || 'https://openwhyd.org'
  ...
  function getUrl (path) {
    if (path && path.length > 0 && path.indexOf('://') == -1) { return (path[0] == '/' ? pageRoot : pagePrefix) + path } else { return path }
  }
  ...
  function SoundCloudDetector () {
    ...
    return function (url, cb) {
      ...
      if (id) {
        id = id.pop()
        if (id.split('/').pop() == 'sets') { return cb() }
  ...
  function initWhydBookmarklet () {
    ...
    function detectEmbed (e, callback) {
      ...
      for (var p = 0; p < prov.length; ++p) {
        addEmbedThumb(e, prov[p], function (embed) {
          ...
          if (eid && !eidSet[eid]) { addThumb(detected = lastThumb = eidSet[eid] = embed) }
          if (--remaining == 0) { callback && callback(detected) }
        })
      }
    }
      ...
      detectEmbed({src: window.location.href}, function (found) {
        ...
        /* if (found)
          ...
          for (var i in elementNames) {
            ...
            for (var j = 0; j < elts.length; ++j) { elts[j] && toDetect.push(elts[j]) }
          }
          // detect tracks from URLs
          (function processNext () {
            if (!toDetect.length) { whenDone() } else { detectEmbed(toDetect.shift(), processNext) }
          })()
        }
      })
    }
    ...
    function genSearchLink (title) {
      return '<p style="margin-top:15px;">Search for ' +
				'<a href="http://openwhyd.org/search?q=' + encodeURIComponent(title) + '" target="_blank">' +
				title + '</a> on Whyd</p>'
    }
    ...
    function detectFromTitle () {
      ...
      for (var i in titleParts) {
        if (title.indexOf(titleParts[i]) > -1) { return genSearchLink(title.replace(titleParts[i], '')) }
      }
      return null
    }
    ...
    if (html && !dzTrackId) { whenDone(html) } else { detectAllEmbeds() }
  }
  /*
	(function loadNext(){
		if (toInclude.length)
