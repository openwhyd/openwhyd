...
var hotTracks = {
  ...
  loadGenre: function (genre_index) {
    ...
    genreList = hotTracks.genres.map(function (object, i) {
      ...
      return '<li class=' + cssClass + "><a  onclick='hotTracks.loadGenre(" + i + ")'>" + object.name + '</a></li>'
    })
  ...
  loadTracks: function (callback) {
    ...
    $.getJSON('/hot/' + hotTracks.current_genre.key + '?format=json&skip=' + hotTracks.skip + '&limit=' + hotTracks.limit, function (data) {
      var content = ''
  ...
  templating: function (template, item) {
    ...
    for (var p in item) {
      template = template.replace(new RegExp('{' + p + '}', 'g'), htmlEntities(item[p]))
    }

    return template.replace(/\{\w*\}/gi, '') // remove un-populated slots
  },

  template: '<div class="post {cssClass}" data-pid="{_id}" data-loved="{isLoved}" >' +
...
var PLAYERS = {
  ...
  'bc': {
    ...
    urlMaker: function (eId) {
      ...
      return parts.length < 2 ? undefined : '//' + parts[0] + '.bandcamp.com/track/' + parts[1]
    }
  },
  'ja': {
    name: 'Jamendo',
  ...
  if (eId && eId.length && eId[0] == '/') {
    ...
    if (player) {
      return {
        ...
        contentUrl:
          player.urlPrefix ? eId.replace('/' + playerId + '/', player.urlPrefix) : (
            (player.urlMaker ? player.urlMaker(eId, src)
...
timeAgoWithString = function (timestamp) {
  ...
  date = renderTime(date) + ' - '	+ date.getDate() + ' ' + MONTHS_SHORT[date.getMonth()] + ' ' + date.getFullYear()
