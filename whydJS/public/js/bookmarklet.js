...
if (undefined == window.console) { console = {log: function () {}, info: function () {}, error: function () {}, warn: function () {}} }
...
(window._initWhydBk = function () {
  ...
  function include (src, cb) {
    ...
    if (src.split(/[\#\?]/)[0].split('.').pop().toLowerCase() == 'css') {
      ...
      function check () {
        if (inc.readyState && (inc.readyState == 'loaded' || inc.readyState == 'complete' || inc.readyState == 4)) { inc.onload() }
      }
  ...
  function BkUi () {
    ...
    function elt (attrs, children) {
      ...
      for (var i = 0; i < (children || []).length; ++i) { div.appendChild(children[i]) }
      return div
    }
  ...
  function imageToHD (track) {
    if (track.img) {
      if (track.eId.substr(1, 2) == 'yt') {
        ...
        i.src = img
      } else if (track.eId.substr(1, 2) == 'sc') { track.img = track.img.replace('-large', '-t300x300') } else if (track.eId.indexOf('/dz/') == 0) { track.img = track.img.replace(/\/image$/, '/image?size=480x640') } else if (track.eId.indexOf('/ja/') == 0) { track.img = track.img.replace(/\/covers\/1\.200\.jpg$/, '/covers/1.600.jpg') }
    }
    return track
  }
  // Track detectors
  ...
  function makePlayemStreamDetector (eidSet) {
    ...
    return function detectPlayemStreams (url, cb) {
      detect(url, function (eid, player, playerId) {
        ...
        eidSet[parts[0]] = true
        eidSet[eid] = true
        if (!player || !player.fetchMetadata) { return cb({eId: eid}) } else {
  ...
  var DETECTORS = [
    ...
    function detectTrackFromTitle () {
      ...
      for (var i = 0; i < titleParts.length; ++i) {
        if (title.indexOf(titleParts[i]) > -1) { return [{searchQuery: title.replace(titleParts[i], '')}] }
      }
    },
    function extractBandcampTracks () {
      ...
      if (bandcampPageUrl.indexOf('bandcamp.com/track/') != -1) { toDetect.push({src: bandcampPageUrl}) } else {
        ...
        for (var j = 0; j < elts.length; ++j) { toDetect.push({href: bandcampPageUrl + elts[j].getAttribute('href')}) }
      }
  ...
  function detectTracks (ui) {
      ...
      (function processNext () {
        if (!urlDetectors.length) {
          ...
          urlDetectors.shift()(url, function (track) {
            ...
            if (track && track.id) { cb(track) } else { processNext() }
          }, element)
        }
      })()
    }
    ...
    function detectEmbed (e, cb) {
      ...
      detectTrack(url, function (track) {
        ...
        if (track.title) {
          ...
          if (track.sourceLabel) { track.sourceLogo = urlPrefix + '/images/icon-' + track.sourceLabel.split(' ')[0].toLowerCase() + '.png' }
    ...
    var toDetect = new function ElementStack () {
      ...
      this.getSortedArray = function () {
        ...
        for (var i = 0; i < keys.length; ++i) { (/\/..\//.test(keys[i]) ? eIds : urls).push(set[keys[i]]) }
    ...
    DETECTORS.map(function (detectFct) {
      ...
      results.map(function (result) {
        if ((result || {}).searchQuery) { ui.addSearchThumb(result) } else { toDetect.push(result) }
      })
    })
    ...
    (function processNext () {
      ...
      if (!elt) { whenDone() } else { detectEmbed(elt, processNext) }
    })()
  }
