...
window.$ = window.$ || new (function FakeJquery() {

	function loadJS(src, cb) {
		var inc = document.createElement("script");
		if (cb)
			inc.onload = inc.onreadystatechange = function() {
				if (inc.readyState == "loaded" || inc.readyState == "complete" || inc.readyState == 4)
					cb();
			};
		inc.src = src;
	...
	return {
		getJSON: function(url, cb) {
			if (url[0] == "/" || url.indexOf("=?") == -1) // local request
				loadJSON(url, cb);
			else {
...
function WhydTrackFinder() {
	...
	var searchEngines = {
		wd: function() {
			return {
				label: 'Whyd',
				query: function(q, cb) {
					var url = "/search?context=quick&max-results="+MAX_RESULTS_PER_ENGINE+"&q="+encodeURIComponent(q);
		...
		yt: function() {
			return {
				label: 'Youtube',
				query: function(q, cb) {
					var url = "http://gdata.youtube.com/feeds/api/videos?v=2&alt=jsonc&first-index=0&max-results="+MAX_RESULTS_PER_ENGINE+"&q="+encodeURIComponent(q)+"&callback=?";
					var that = this;
		...
		sc: function() {
			var URL = "http://api.soundcloud.com/tracks?limit="+MAX_RESULTS_PER_ENGINE+"&client_id="+SOUNDCLOUD_CLIENT_ID+"&format=json&callback=?";
			return {
				label: 'Soundcloud',
				query: function(q, cb) {
					var that = this;
					$.getJSON(URL + "&q=" + encodeURIComponent(q), function(json) {
						var items = json instanceof Array ? json : ((json || {}).data || {}).items;
						cb(!items ? json : {
							"Soundcloud": items.map(function(r){
								r.title = r.title || r.name;
								var permalink = (r.url || r.permalink_url).split("/");
								return {
									eId: "/sc/" + permalink.slice(permalink.length-2).join("/") + "#" + r.uri,
									img: r.img || r.artwork_url || "/images/cover-soundcloud.jpg",
									url: r.url || r.permalink_url + "#" + r.uri,
									name: (r.title.indexOf(" - ") == -1 ? (r.uploader || r.user.username) + " - " : "") + r.title,
									playerLabel: that.label
								};
							})
						});
	...
	for (var i in searchEngines)
		searchEngines[i] = new searchEngines[i]();
...
(function(){
	...
	function renderResults(array, name, q) {
		return [
			...
			"<ul>"
		].concat(array.map(function(item){
			var name = htmlEscape(item.name);
			if (q)
				name = name.replace(new RegExp(q, "gi"), "<b>$&</b>"); // highlight matching part
			return "<li class='"+(item.cssClass || "")+"' " //  class='"+(item.playerLabel||"")+"'
				...
				+ "<a href='"+htmlEscape(item.url)+"' target='_blank'>"
				+ "<div class='thumb' style='background-image:url("+htmlEscape(item.img)+")'></div>"
				+ name + "</a>"
	...
	function loadStream(url, id, cb) {
		...
		$.getJSON(url + "?format=json", function(r){
			if (r)
				document.getElementById(id).innerHTML = renderResults(r.map(renderLastPost), id);
			cb && cb(r);
		}, "json");
	}
	...
	function loadUserPlaylists(cb) {
		$.getJSON("/api/user", function(u){
			if (!u || !u.pl)
				return;
			var uid = u._id, i = 0, more = { cssClass: "showMore", name: "More..." };
			...
			document.getElementById("myPlaylists").innerHTML =
				renderResults(u.pl.map(renderPlaylist).concat(more), "myPlaylists"); 
	...
	*/
	...
	function onAddTrack(btn){
		...
		var postData = {
			...
			"src[id]": "http://whyd.com/mobile",
			"src[name]": "Whyd Mobile Track Finder",
		};
		...
		$.getJSON("/api/post?"+params.join("&"), function(post){
			...
			if (!post || post.error)
				alert("Sorry, we were unable to add this track\n" + ((post||{}).error || ""));
			else
				alert("Succesfully added this track!");
		});
	};
	...
	function fadeIn(nodeSet, liCondition) {
		...
		if (fadeQueue.length)
			var interval = setInterval(function() {
				...
				if (elt)
					elt.className = elt.className.replace("hidden", "fadeIn");
				else
					clearInterval(interval);
			}, 10);		
	}

	document.getElementById("searchPane").addEventListener("DOMNodeInserted", function (ev) {
