  ...
  function loadJS (src, cb) {
    ...
    if (cb) {
      inc.onload = inc.onreadystatechange = function () {
        if (inc.readyState == 'loaded' || inc.readyState == 'complete' || inc.readyState == 4) { cb() }
      }
    }
    inc.src = src
...
(function () {
  ...
  function loadStream (url, id, cb) {
    ...
    $.getJSON(url + '?format=json', function (r) {
      if (r) { document.getElementById(id).innerHTML = renderResults(r.map(renderLastPost), id) }
      cb && cb(r)
    }, 'json')
  }
  ...
  function onAddTrack (btn) {
    ...
    $.getJSON('/api/post?' + params.join('&'), function (post) {
      ...
      if (!post || post.error) { alert('Sorry, we were unable to add this track\n' + ((post || {}).error || '')) } else { alert('Succesfully added this track!') }
    })
  };
  ...
  var qS = new QuickSearch(document.getElementById('searchField'), {
    ...
    submitQuery: function (query, display) {
      ...
      function handleResults (res, engine) {
        --remaining
        if (!res || res.error || res.errors) { console.log('error(s)', engine.label, res) } else {
          for (var i in res) {
  ...
  function fadeIn (nodeSet, liCondition) {
    ...
    if (fadeQueue.length) {
      var interval = setInterval(function () {
        ...
        if (elt) { elt.className = elt.className.replace('hidden', 'fadeIn') } else { clearInterval(interval) }
      }, 10)
    }
  }
