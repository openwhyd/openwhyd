$(function() {
	function loading(toggle) {
		$("body").toggleClass("filtering", toggle)
	...
	var quickSearch = new QuickSearch($box, {
		...
		submitQuery: function(query, display) {
			...
			loading(true);
			submitSearchQuery({ q: query, uid: window.pageUser.id, format: "html" }, function(resultsHtml){
				if (!resultsHtml || typeof resultsHtml == "object")
					resultsHtml = '';// '<a href="#" class="noResults"><p>' + query + ' was not found</p></a>';
				applyFilter(true);
