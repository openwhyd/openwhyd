...
$playlistNameField.parent().unbind().submit(function (e) {
  e.preventDefault()
  ...
  $.ajax({
    ...
    complete: function (res, status) {
      ...
      try {
        ...
        if (isNewPlaylist) {
          try {
            window.Whyd.tracking.log('Created playlist using button', json.id)
          } catch (e) {
            console.log('error', e, e.stack)
          }
          goToPage(window.location.href.replace('/playlist/create', '/playlist/' + json.id))
        } else {
