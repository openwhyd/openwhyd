...
$(function(){
	...
	function renderUserList(users) {
		...
		for (var i in users)
			resultsHtml += '<li>'
				+ '<div class="thumb" style="background-image:url(' + users[i].img + ')"></div>'
				+ users[i].name
				+ '</li>';
		return resultsHtml;
	}
	...
	window.dlgPlContributors = function(options) {
		options = options || {};
		var url = (options.collabId ? '/playlist/'+options.collabId : window.location.href.split("?")[0]);
		console.log(url + "/contributors")
		openRemoteDialog(url + "/contributors", 'dlgPlContributors', function($dlg){
