...
function processFile (file, options, callback) {
  ...
  if (!file || !file.path) { return callback({error: 'Error during file upload, please try again.'}) } else if (!file.type || file.type.indexOf('image/') != 0) {
    ...
    if (thumbDims.length > 0) {
      ...
      for (var i in thumbDims) {
        ...
        var thumbWidth = thumbWidthHeight.length > 0 ? thumbWidthHeight[0] : null
        var thumbHeight = thumbWidthHeight.length > 1 ? thumbWidthHeight[1] : null
        genThumb(thumbWidth, thumbHeight, function (thumbFile, thumbWidth, thumbHeight, dims) {
          console.log('generated thumb', thumbDim, thumbFile)
          result.thumbs[/* dims */thumbDim] = uploadCtr.cleanFilePath(thumbFile)
          if (!options.keepOriginal) { result.path = result.thumbs[/* dims */thumbDim] }
...
exports.controller = function (req, requestParams, res) {
  ...
  if (postParams && postParams.id && postParams.action == 'delete') { return uploadCtr.deleteFile(settings.uploadDir + '/' + postParams.id) }
