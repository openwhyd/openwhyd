...
exports.resetPassword = function(request, reqParams, response)
{
	...
	if (!reqParams.resetCode || !reqParams.uid || !reqParams.password)
		return exports.renderPasswordPage(request, reqParams, response, "missing required parameters");
	
	if (reqParams.password == "password" || reqParams.password.trim() == "")
		return exports.renderPasswordPage(request, reqParams, response, "Please enter your new password twice");
	...
	exports.checkResetCode(request, reqParams, response, function(user) {
		...
		users.save({_id:user._id, pwd:md5(reqParams.password)}, function (storedUser) {
			if (!storedUser)
				return exports.renderPasswordPage(request, reqParams, response, "Oops, your request failed... Please try again!");
			
			console.log("password reset ok");
			notifEmails.sendPasswordUpdated(storedUser._id, storedUser.email);
			
			if (reqParams.redirect)
				return response.redirect(reqParams.redirect);
			else
				return response.render("Your password was successfully updated! You can now login there: " + config.urlPrefix);
...
exports.controller = function(request, getParams, response)
{
	...
	if (request.method.toLowerCase() === 'post')
		...
		exports[getParams && getParams.uid ? "renderPasswordPage" : "renderForgotPage"](request, getParams, response);
};