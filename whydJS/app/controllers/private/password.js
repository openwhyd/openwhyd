...
function renderTemplate1 (params, callback) {
  templateLoader.loadTemplate('app/templates/passwordForgot.html', function (template) {
...
exports.checkResetCode = function (request, reqParams, response, okCallback) {
  if (!reqParams.resetCode || !reqParams.uid) {
    ...
    response.render('Your password reset request is invalid. Please try to login on ' + process.appParams.urlPrefix)
    return false
  }
  ...
  users.fetchByUid(reqParams.uid, function (user) {
    if (user && user.pwd == reqParams.resetCode) {
      ...
      response.render('Your password reset request is invalid or expired. Please try to login on ' + process.appParams.urlPrefix)
    }
  })
}
...
exports.renderForgotPage = function (request, reqParams, response, error) {
  ...
  templateLoader.loadTemplate('app/templates/passwordForgot.html', function (template) {
    response.render(template.render(vars), null, {'content-type': 'text/html'})
  })
}
...
exports.renderPasswordPage = function (request, reqParams, response, error) {
  ...
  exports.checkResetCode(request, reqParams, response, function (user) {
    ...
    templateLoader.loadTemplate('app/templates/passwordSet.html', function (template) {
      response.render(template.render(vars), null, {'content-type': 'text/html'})
    })
  })
}
...
exports.resetPassword = function (request, reqParams, response) {
  request.logToConsole('register.resetPassword', reqParams ? {
    resetCode: reqParams.resetCode,
    uid: reqParams.uid,
    email: reqParams.email,
    redirect: reqParams.redirect } : null)

  reqParams = reqParams || {}

  if (!reqParams.resetCode || !reqParams.uid || !reqParams.password) { return exports.renderPasswordPage(request, reqParams, response, 'missing required parameters') }
  ...
  exports.checkResetCode(request, reqParams, response, function (user) {
    users.save({_id: user._id, pwd: md5(reqParams.password)}, function (storedUser) {
      if (!storedUser) { return exports.renderPasswordPage(request, reqParams, response, 'Oops, your request failed... Please try again!') }

      console.log('password reset ok')
      notifEmails.sendPasswordUpdated(storedUser._id, storedUser.email)

      if (reqParams.redirect) { return response.redirect(reqParams.redirect) } else { return response.render('Your password was successfully updated! You can now login there: ' + config.urlPrefix) }
...
exports.controller = function (request, getParams, response) {
  ...
  if (request.method.toLowerCase() === 'post') { exports.resetPassword(request, request.body/* postParams */, response) } else { exports[getParams && getParams.uid ? 'renderPasswordPage' : 'renderForgotPage'](request, getParams, response) }
}
