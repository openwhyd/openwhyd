...
exports.controller = function(request, reqParams, response)
{
	...
	if (!reqParams || !reqParams.mail)
		return response.render("You must provide a valid email address."
			+ "<script>window.location.href='/';</script>", null, {'content-type': 'text/html'});
	
	user.fetchByEmail(reqParams.mail, function(u) {
		if (u)
			response.render("You've already registered, please log in using the link below:"
				+ "<script>window.location.href='"+config.urlPrefix+"/login?email="+reqParams.mail+"';</script>");
		else {
			user.fetchInviteByEmail(reqParams.mail, function(u) {
				if (u)
					response.render("Please follow the link of the invite email we sent you"
						+ "<script>window.location.href='"+config.urlPrefix+"/invite/"+u._id+"';</script>");
				else {
					user.fetchEmail(reqParams.mail, function(err, email) {
						...
						if (!email) {
							...
							email.email(reqParams.mail, "whyd registration",
							...
							response.render("Don't forget to check your spam folder!");
					});
				}
			});
		}
