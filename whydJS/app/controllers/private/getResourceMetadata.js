...
exports.handleRequest = function (request, reqParams, response) {
  ...
  try {
    ...
    if (url) { url = url.replace('http://openwhyd.org', '').replace('https://openwhyd.org', '') }

    if (url.startsWith('/u/')) {
      userModel.fetchByUid(url.substr(3), function (user) {
        response.render(!user ? {error: 'not found'} : {
          id: user._id,
          mid: user.mid,
          name: user.name,
          img: user.img,
...
exports.controller = function (request, getParams, response) {
  ...
  { exports.handleRequest(request, request.body/* postParams */, response) } else { exports.handleRequest(request, getParams, response) }
}
