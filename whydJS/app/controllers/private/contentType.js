...
exports.controller = function (request, reqParams, response) {
  ...
  if (url.indexOf('openwhyd.org') > -1 || url.indexOf('localhost:') > -1) {
    if (url.contains('/upload_data/') || url.contains('/uPostedImg/') || url.contains('/uAvatarImg/') || url.contains('/ugTopicImg/')) { return renderResult('image/unknown') }
  }
    ...
    get.ContentType(url, function (err, contentType) {
      if (contentType === 'text/html') {
        get(url, function (err, page) {
          if (page && !err) { renderResult(contentType, page.getTitle(), page.getImages()) } else { handleError(err) }
        })
      } else if (contentType && contentType != 'noContentType') { renderResult(contentType) } else { handleError(err) }
    })
  } catch (err) {
    handleError(err)
  }
