...
function genButton (p) {
  ...
  return [
    ...
    'a{' +
			...
			(btnSize ? 'width:' + (p.width - 2) + 'px;height:' + (p.width - 2) + 'px;' +
				"background: #3B3A39 url('/images/whyd_w" + btnSize + ".png') no-repeat 50% 50%;"
    ...
    '</head>',
    '<body>',
    '<a href="/u/' + p.uId + '" target="_blank">' + (btnSize ? '' : '<div>Subscribe to me on Whyd</div>') + '</a>',
    '</body>',
    '</html>'
  ].join('\n')
}
...
var btnTypes = {
  'profile': function (p, cb) {
    if (!p.uId) { cb({error: 'missing uId parameter'}) } else { cb({html: genButton(p)}) }
  }
}
...
exports.controller = function (request, reqParams, response) {
  ...
  if (!reqParams || !reqParams.type || !btnTypes[reqParams.type]) { return response.badRequest() }

  function render (d) {
    if (d && d.html) { response.renderHTML(d.html) } else {
