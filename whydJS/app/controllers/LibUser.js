...
function renderPlaylists(options, maxNb) {
	...
	if (maxNb) {
		if (playlists.length > maxNb)
			playlists = playlists.slice(0, maxNb); //playlists.length-maxNb, playlists.length);
...
function populateUsers(subscr, options, cb) {
	followModel.fetchSubscriptionArray(options.loggedUser.id, function(mySubscr) {
		...
		for (var i in subscr)
			if (subscrSet[subscr[i].id])
				subscr[i].subscribed = true;
...
function fetchAndRenderPlaylist (options, callback, process) {
	...
	if (!options.playlist)
		callback("meh... this playlist does not exist!");
	else
...
//						ago: uiSnippets.renderTimestamp(new Date() - creation.getTimestamp())
				...
				for(var i in options.showActivity.items)
					options.showActivity.items[i].ago = uiSnippets.renderTimestamp(new Date() - options.showActivity.items[i]._id.getTimestamp());
				process([]);
			});
		});
	}
...
function renderUserLibrary (lib, user) {
	...
	if (user == null)
		return lib.render({errorCode:"USER_NOT_FOUND"});
	...
	if (options.user && options.user.lnk)
		renderUserLinks(options.user.lnk);
	...
	function renderResponse(feed){
		if (options.callback) {
			...
			lib.renderPage(user, null/*sidebarHtml*/, generateMixpanelCode(options) + feed);
	}
