...
var profileTemplateV2 = templateLoader.loadTemplate('app/templates/userProfileV2.html')
var playlistTemplateV2 = templateLoader.loadTemplate('app/templates/userPlaylistV2.html')
...
function fetchStats (options, callback) {
  followModel.countSubscriptions(options.user.id, function (nbSubscriptions) {
    followModel.countSubscribers(options.user.id, function (nbSubscribers) {
      ...
      followModel.get({uId: options.loggedUser.id, tId: options.user.id}, function (err, res) {
...
function fetchNbTracks (options, callback) {
  postModel.countUserPosts(options.user.id, function (nbPosts) {
    options.user.nbTracks = nbPosts > 9999 ? Math.floor(nbPosts / 1000) + 'k' : nbPosts
    callback()
  })
}
...
function fetchActivity (options, cb) {
  activityModel.fetchHistoryFromUidList([options.user.id], {limit: MAX_HISTORY}, function (activities) {
    ...
    for (var i in activities) {
      if (activities[i]) {
        activities[i].ago = uiSnippets.renderTimestamp(new Date() - activities[i]._id.getTimestamp())
        if (activities[i].like) { postsToPopulate.push(mongodb.ObjectId('' + activities[i].like.pId)) }
      }
    }
...
function renderPlaylists (options, maxNb) {
  ...
  if (maxNb) {
    if (playlists.length > maxNb) { playlists = playlists.slice(0, maxNb) } // playlists.length-maxNb, playlists.length);
  ...
  for (var i in playlists) {
    ...
    // playlists[i].url = "/u/" + options.user.id + "/playlist/" + playlists[i].id;
      playlists[i].img = '/img/playlist/' + options.user.id + '_' + playlists[i].id
    }
  }
...
function populateUsers (subscr, options, cb) {
  followModel.fetchSubscriptionArray(options.loggedUser.id, function (mySubscr) {
...
function fetchAndRenderPlaylist (options, callback, process) {
  ...
  if (options.playlistId == 'create') {
    options.playlist = {
      id: 'create',
      name: /* (options.reqParams || {}).name || */ 'Playlist #' + options.user.pl.length
    }
    options.pageTitle = 'new playlist'
  } else {
    options.pageTitle = ((options.playlist || {}).name || 'a playlist') + ' by ' + options.user.name
  }
  if (!options.playlist) { callback('meh... this playlist does not exist!') } else {
    contestModel.fetchByTitle((options.playlist || {}).name, function (contest) {
      var prevId = null
      for (var p = options.user.pl.length - 1; p > -1; --p) {
        var pl = options.user.pl[p]
        if (!pl) continue
        if (pl.id == options.playlistId) {
          if (prevId !== null) { options.prevPageInList = '/u/' + options.uid + '/playlist/' + prevId }
          for (--p; p > -1; --p) {
            if (options.user.pl[p]) {
              options.nextPageInList = '/u/' + options.uid + '/playlist/' + options.user.pl[p].id
              break
            }
          }
          break
      ...
      postModel.fetchPlaylistPosts(options.uid, options.playlistId, options.fetchParams, process)
    })
  }
}
...
function fetchAndRenderProfile (options, callback, process) {
  ...
  if (options.showPlaylists) {
    ...
    postModel.fetchPosts({lov: options.uid}, /* params */null, {after: options.after}, process)
    ...
    followModel.fetchUserSubscriptions(options.loggedUser.id, function (mySubscr) {
      // console.log("mySubscr.subscriptions", mySubscr.subscriptions);
      var mySubscrUidList = snip.objArrayToValueArray(mySubscr.subscriptions, 'id')
      activityController.generateActivityFeed([options.user.id], mySubscrUidList, options, function (result) {
        for (var i in result.recentActivity.items) {
          if (result.recentActivity.items[i].subscriptions) {
            result.recentActivity.items[i].subscribedUsers = result.recentActivity.items[i].subscriptions
        ...
        if (result.hasMore) { options.hasMore = {lastPid: result.hasMore.last_id} } else {
        ...
        for (var i in options.showActivity.items) { options.showActivity.items[i].ago = uiSnippets.renderTimestamp(new Date() - options.showActivity.items[i]._id.getTimestamp()) }
        process([])
      })
    })
...
function fetchAndRender (options, callback) {
  ...
  var process = bareFormats[options.format] ? callback : function (posts) {
    if (!options.format && !options.embedW) {
      if (options.playlistId) { options.pageImage = config.urlPrefix + '/img/playlist/' + options.user.id + '_' + options.playlistId }
  ...
  options.fetchParams = {after: options.after, before: options.before, limit: options.limit}
  if (options.embedW) { options.fetchParams.limit = config.nbTracksPerPlaylistEmbed } else if (options.limit && typeof options.limit !== 'number') {
    if (typeof options.limit === 'string') { options.fetchParams.limit = parseInt(options.limit) } else if (typeof options.limit === 'object' && options.limit.push) { options.fetchParams.limit = parseInt(options.limit.pop()) }
...
function renderUserLibrary (lib, user) {
  ...
  options.pageUrl = options.pageUrl.replace('/' + user.handle, '/u/' + user._id)
  ...
  function renderResponse (feed) {
    if (options.callback) {
      ...
      lib.renderOther(safeCallback + '(' + JSON.stringify(feed) + ')', 'application/javascript')
    } else if (options.format == 'links') {
      lib.renderOther(feed.map(function (p) {
        return config.translateEidToUrl((p || {}).eId)
      }).join('\n'), 'text/text')
    } else if (options.format == 'json') { lib.renderJson(feed) } else if (options.after || options.before) { lib.render({html: feed}) } else { lib.renderPage(user, null/* sidebarHtml */, generateMixpanelCode(options) + feed) }
  }
  ...
  if (!options.after && !options.before) // main tab: tracks (full layout to render, with sidebar)
  { fcts = [fetchPlaylists, /* fetchSubscriptions, */ fetchStats, fetchLikes, fetchNbTracks /* fetchSimilarity, fetchTags */].concat(fcts) }
