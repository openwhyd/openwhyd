...
function handleRequest (p, cb) {
  if (p.ajax && processAjax[p.ajax]) {
    ...
    if (lastUrlWord == 'bookmarklet' || lastUrlWord == 'button') { p.step = 'bookmarklet-legacy' }

    var processor = processStep[p.step]
    if (!processor) { cb({error: 'unknown step'}) } // cb({redirect:"/"});
    else {
...
exports.controller = function (request, getParams, response) {
  ...
  handleRequest(p, function (r) {
    ...
    if (r.redirect) { response.temporaryRedirect(r.redirect) } else if (r.html) { response.renderHTML(r.html) } else { response.renderJSON(r) }
  })
}
