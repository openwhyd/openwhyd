...
function loadTemplates (callback) {
  ...
  tmpContributors = templateLoader.loadTemplate('app/templates/dlgPlContributors.html', callback)
  // callback && callback();
}
...
exports.controller = function (request, reqParams, response) {
  ...
  function render (html) {
    ...
    if (loggedInUser && loggedInUser.id && !reqParams.after && !reqParams.before) { analytics.addVisit(loggedInUser, request.url) }
  }
  ...
  if (request.url.split('?')[0].endsWith('/contributors')) {
    collabModel.fetchPlaylistById(reqParams.id, function (playlist) {
      for (var i in playlist.admin) {
        playlist.admin[i].img = playlist.admin[i].id ? '/img/u/' + playlist.admin[i].id
          : 'http://graph.facebook.com/v2.3/' + playlist.admin[i].fbId + '/picture?type=square'
      }
      function renderTemplate () {
        render(tmpContributors.render({
  ...
  collabModel.fetchPostsByPlaylistId(reqParams.id, {after: reqParams.after, limit: LIMIT + 1}, function (posts) {
    ...
    if (!pageVars.rawFeed) {
      collabModel.fetchPlaylistById(reqParams.id, function (playlist) {
        if (!playlist) { return render('The playlist you were looking for may have vanished!') }

        pageVars.playlist = playlist

        render(mainTemplate.renderWhydPage({
