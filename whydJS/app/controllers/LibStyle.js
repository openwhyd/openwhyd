...
function renderFriendsLibrary (lib, style) {
  ...
  if (!uidList) { return lib.renderPage({}, null, 'hmmm... we have no selection of tracks for this style yet!') }
  ...
  renderFriendsFeed(options, uidList, function (feedHtml) {
    if (options.after || options.before) { lib.render({html: feedHtml}) } else {
      /*
