...
exports.controller = function(request, getParams, response) {
	...
	function render(result) {
		...
		if (result && result.html)
			response.renderHTML(result.html);
		else 
	...
	function joinContest(plC) {
		if (!plC || !plC.uri)
			return render({error: (plC || {}).error || "hmm... are you sure that this contest still exists?"});
		if (loggedUser) {
	...
	if (getParams && getParams.action == "info" && getParams.uri && isAdmin)
		...
		contestModel.save(request.body, /*render*/ function(plC){
			if (!plC || !plC.uri)
				render({error: (plC || {}).error});
			else
				response.redirect("/playlistcontest/" + plC.uri + "/info");
		});
	}
	else
		response.badRequest();
