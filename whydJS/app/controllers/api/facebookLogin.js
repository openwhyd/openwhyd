...
exports.handleRequest = function (request, reqParams, response) {
  ...
  if (!reqParams || !reqParams.fbAccessToken || !reqParams.fbUid) { return renderError('invalid facebook login request') }

  var fbCookieUid = request.getFbUid()
  if (fbCookieUid) {
    console.log('facebook cookie uid:', fbCookieUid)
    if (!reqParams.fbUid || fbCookieUid != reqParams.fbUid) { return renderError('invalid facebook cookie') }
  }
  ...
  facebookModel.fetchMe(reqParams.fbAccessToken, function (fbUser) {
    ...
    if (!fbUser || fbUser.id != reqParams.fbUid) { renderError('facebook session token does not match user id', fbUser) } else if (reqParams.action && reqParams.action == 'link' && loggedUser) {
