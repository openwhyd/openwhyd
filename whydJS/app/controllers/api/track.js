...
var ACTIONS = {
  ...
  'listBySource': function (p, cb) {
    ...
    mongodb.collections['track'].count(function (err, total) {
      ...
      var i = 0
      mongodb.forEach2('track', {fields: {_id: 0, eId: 1, pId: 1, name: 1}}, function (track, next) {
        if (!track) { whenDone() } else {
          if (++i % 100) { console.log('listBySource', i, '/', total) }
          if ((track.eId || '').indexOf('/dz/') == 0) { list.push([track.eId, track.pId, track.name]) }
          next()
        }
      })
    })
  ...
  fetchMetadataConfidence: function (p, cb) {
    ...
    mongodb.collections['track'].count(function (err, count) {
      var i = 0
      cb({total: count})
      mongodb.forEach2('track', {fields: {_id: 0, eId: 1, meta: 1}}, function (track, next) {
        if (!track) { whenDone() } else {
          if (++i % 10) { console.log('fetchMetadataConfidence', i, '/', count, track.eId, track.name) }
  ...
  fetchMetadataForHotTracks: function (p, cb) {
    var countPerConfidence = {}, countPerSource = {}, count = 50
    mongodb.collections['track'].find({}, {fields: {_id: 0, eId: 1, name: 1, meta: 1}, sort: {score: -1}, limit: count}).toArray(function (err, tracks) {
      for (var i in tracks) {
        var track = tracks[i]
        console.log('fetchMetadataForHotTracks', i, '/', count, !!track.meta, track.eId, track.name)
        var eId = track.eId || ''
        if (!eId) { continue }
        countPerSource[eId.substr(1, 2)] = (countPerSource[eId.substr(1, 2)] || 0) + 1
        if (eId.indexOf('/yt/') == 0) {
          var c = (track.meta || {}).c
          countPerConfidence[c] = (countPerConfidence[c] || 0) + 1
        }
      }
      console.log('fetchMetadataForHotTracks, countPerConfidence:', countPerConfidence)
      cb({countPerConfidence: countPerConfidence, countPerSource: countPerSource})
    })
  },
...
exports.controller = function (request, reqParams, response) {
  ...
  if (reqParams._1 && reqParams._2) { p.eId = request.url.split('/api/track')[1].split('?')[0] } else if (isAdmin && reqParams._1 && !reqParams.action) { reqParams.action = reqParams._1 }
