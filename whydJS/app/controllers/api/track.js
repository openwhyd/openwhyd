...
var ACTIONS = {
	refresh: function(p, cb){
		if (!p.eId)
			return cb({error:"missing parameter: eId"});
		...
		mongodb.collections["track"].count(function(err, total){
			...
			mongodb.forEach2("track", {fields:{_id:0,eId:1,pId:1,name:1}}, function (track, next){
				if (!track)
					...
					if (++i % 100)
						console.log("listBySource", i, "/", total);
		...
		mongodb.collections["track"].count(function(err, count){
			...
			mongodb.forEach2("track", {fields:{_id:0,eId:1,meta:1}}, function (track, next){
				if (!track)
					...
					if (++i % 10)
						console.log("fetchMetadataConfidence", i, "/", count, track.eId, track.name);
