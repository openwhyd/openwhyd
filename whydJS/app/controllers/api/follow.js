...
var follow = function (reqParams, dbHandler) {
	switch(reqParams.action) {
		...
		case "insert":
			var obj = {
				uId:reqParams.uId, uNm:reqParams.uNm,
				tId:reqParams.tId, tNm:reqParams.tNm
			};
			if (!obj.uId || !obj.tId)
				return dbHandler()
...
exports.controller = function(request, reqParams, response) {
	...
	if (ranPublicAction(user, reqParams, function(res){ response.renderJSON(res); }))
		return;

	// make sure a registered user is logged, or return an error page
	if (!user)
		return sendResult({error:"please login first"});
