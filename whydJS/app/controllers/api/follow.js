...
exports.follow = function (followerId, followingId, callback) {
  follow({
    ...
    tId: followingId,
    tNm: ('' + followingId).startsWith('/u/') ? (mongodb.usernames[followingId.substr(3)] || {}).name : null
...
exports.isUserFollowing = function (uid, followingMid, handler/*, fbAccessToken */) {
  follow({action: 'get', uId: uid, tId: followingMid}, function (error, result) {
    handler(result)
  })
}
...
function ranPublicAction (loggedUser, reqParams, cb) {
  ...
  if (action) {
    function fetchSubscriptionStatus (res) {
      var uids = snip.objArrayToValueArray(res, 'uId').concat(snip.objArrayToValueArray(res, 'tId'))
      followModel.fetch({uId: loggedUser.id, tId: {$in: uids}}, null, function (subscrStatus) {
        var subscrSet = snip.objArrayToSet(subscrStatus, 'tId', true)
        for (var i in res) { res[i].isSubscribing = subscrSet[res[i].uId || res[i].tId] }
        cb(res)
      })
    }
...
exports.controller = function (request, reqParams, response) {
  ...
  var sendResult = function (error, result) {
    ...
    if (reqParams.redirect) { response.redirect(reqParams.redirect) } else { response.renderJSON(result) }
  }

  var user = request.checkLogin(/* response */)
  if (ranPublicAction(user, reqParams, function (res) { response.renderJSON(res) })) { return }
