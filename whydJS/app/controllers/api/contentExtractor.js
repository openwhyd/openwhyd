...
exports.controller = function (request, reqParams, response) {
  ...
  function renderResult (embeds, allEmbeds) {
    ...
    if (reqParams.title) {
      ...
      for (var i in embeds) {
        if (embeds[i].name) { embeds[i].distance = levenshteinenator(normalize(embeds[i].name), title) }
      }
  ...
  try {
    get(url, function (err, page) {
      if (err)
      // throw err;
      { console.log('contentExtractor.get() error: ', err, err.stack) } else {
        ...
        // 1) extract named embeds
          ...
          console.log('-> extracted embeds', embedRefs.length, ' (skipped', (embeds.length - embedRefs.length), 'no-name embeds)')
          ...
          if (mp3) {
            ...
            for (var i in mp3) {
              embedRefs.push({
                type: 'mp3 file',
                url: mp3[i],
                name: mp3[i].split('/').pop().replace(/\.mp3$/i, '')
              })
            }
          }
