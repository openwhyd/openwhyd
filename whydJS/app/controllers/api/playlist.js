...
exports.actions = {
	...
	setOrder: function(p, cb) {
		if (!p || !p.order || !p.id)
			...
			postModel.setPlaylistOrder(p.uId, p.id, p.order, cb);
	},
...
exports.handlePostRequest = function(request, reqParams, response) {
	...
	if (false == user || !reqParams || !reqParams.action || !exports.actions[reqParams.action])
...
function fetchPlaylist(p, cb){
	p = p || {};
	if (!p.id)
		return cb({error:"missing id parameter"});
	...
	userModel.fetchMulti({_id:{$in:uidList}}, {fields:{name:1,pl:1}}, function(userList) {
		...
		for (var i in userList)
			userSet[""+userList[i]._id] = userList[i];
		// populate plIdSet
		for (var i in plIdSet) {
			var plId = i.split("_");
			var userPl = (userSet[plId[0]] || {}).pl || [];
			for (var j in userPl)
				if (userPl[j].id == plId[1])
					plIdSet[i] = userPl[j];
		}
		...
		(function next(i) {
			...
			postModel.fetchPlaylistPosts(playlists[i].uId, playlists[i].plId, {limit:10, fields:{name:1, eId:1}}, function(posts){
				...
				for (var j in posts) {
					...
					if (artist)
						playlists[i].lastArtists.push(artist);
