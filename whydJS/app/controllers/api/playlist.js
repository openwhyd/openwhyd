...
exports.actions = {
  ...
  setOrder: function (p, cb) {
    if (!p || !p.order || !p.id) { cb({error: 'missing parameters'}) } else { postModel.setPlaylistOrder(p.uId, p.id, p.order, cb) }
  },
...
exports.handlePostRequest = function (request, reqParams, response) {
  ...
  if (user == false || !reqParams || !reqParams.action || !exports.actions[reqParams.action]) { return response.badRequest() }
...
function fetchPlaylist (p, cb) {
  ...
  var playlists = [], // output of the function
    plIdSet = {},
    uidList = []
  var plIds = (typeof p.id === 'object' && p.id.length ? p.id : [p.id]).map(function (plId) {
    var plId = '' + plId
    plIdSet[plId] = {}
    uidList.push(plId.split('_')[0])
    return plId
  })
  userModel.fetchMulti({_id: {$in: uidList}}, {fields: {name: 1, pl: 1}}, function (userList) {
    // populate userSet
    var userSet = {}
    ...
    (function next (i) {
      ...
      if (plId.length != 2) { next(i + 1) } else {
...
function includeTags (playlists, cb) {
  plTagsModel.getTagEngine(function (tagEngine) {
    ...
    (function next (i) {
      ...
      postModel.fetchPlaylistPosts(playlists[i].uId, playlists[i].plId, {limit: 10, fields: {name: 1, eId: 1}}, function (posts) {
        var tagSet = {}
        playlists[i].lastArtists = []
        for (var j in posts) {
          var artist = snip.detectArtistName((posts[j] || {}).name)
          if (artist) { playlists[i].lastArtists.push(artist) }
          (tagEngine.getTagsByEid((posts[j] || {}).eId) || []).map(function (tag) {
...
exports.controller = function (request, getParams, response) {
  ...
  if (request.method.toLowerCase() === 'post') { exports.handlePostRequest(request, request.body, response) } else if (getParams.id) {
