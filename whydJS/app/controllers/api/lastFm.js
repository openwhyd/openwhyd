...
function LastFM(apiKey, apiSecret) {
	...
	this.updateNowPlaying2 = function(post, lastFmSessionKey, cb) {
		...
		if (lastFmSessionKey && splitted.length > 1) {
			...
			if (post.duration)
				scrobbleData.duration = parseInt(post.duration);
	...
	this.scrobble2 = function(trackName, lastFmSessionKey, chosenByUser, timestamp, cb) {
		...
		if (lastFmSessionKey && splitted.length > 1)
			...
			cb({error:"unable to scrobble track"});
	}
}
...
exports.controller = function(request, p, response) {
	...
	if (p.token) {
		function render(message, session) {
			var session = session ? JSON.stringify({sk:session.key, name:uiSnip.htmlEntities(session.name)}) : '';
			console.log("rendering lastfm session for callback", message, session);
			response.renderHTML(message + "<script>window.opener.lastFmCallback("+session+");</script>");
		}
