...
exports.handleRequest = function (request, form, response, ignorePassword) {
  ...
  request.logToConsole('login.handleRequest', {action: form.action, email: form.email, md5: form.md5})
  ...
  function renderRedirect (url, user) {
    ...
    if (!form.ajax) { response.renderHTML(loggingTemplate.htmlRedirect(url)) } else {
  ...
  function renderForm (form) {
    ...
    if (form.ajax) { renderJSON(form) } else { response.renderHTML(loggingTemplate.renderLoginPage(form)) }
  }

  if (form.mail) { // new email given on landing page
    form.mail = emailModel.normalize(form.mail)
    if (!emailModel.validate(form.mail)) { return renderJSON({error: 'Please enter a valid email address'}) }
    userModel.fetchByEmail(form.mail, function (u) {
      if (u) { renderJSON({result: "You've already registered, please log in.", redirect: 'javascript:login();'}) } else {
          ...
          if (u) {
            ...
            userModel.fetchEmail(form.mail, function (err, email) {
              if (!email) {
                ...
                renderJSON({
                  result: "Awesome, we just sent you an email! Please check your spam folder if you don't receive it."
    ...
    userModel[form.email.indexOf('@') > -1 ? 'fetchByEmail' : 'fetchByHandle'](form.email, function (dbUser) {
      if (!dbUser) {
        ...
        /* form.error */ form.ok = 'We just sent you an email to reset your password, wait for it!'
      } else if (!ignorePassword && !form.md5 /* (!form.password || form.password == "password") */) {
        form.error = 'Please enter your password below:'
      } else if (form.action == 'login' && (ignorePassword || dbUser.pwd == form.md5)) {
        console.log('ok, user logged in as: ' + dbUser.name)
        console.log('form.fbUid', form.fbUid)
        if (form.fbUid) {
          userModel.update(dbUser._id, {$set: {
            fbId: form.fbUid,
...
exports.controller = function (request, getParams, response) {
  if (request.method.toLowerCase() === 'post') { exports.handleRequest(request, request.body, response) } else { exports.handleRequest(request, getParams, response) }
}
