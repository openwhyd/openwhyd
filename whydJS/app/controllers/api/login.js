...
exports.handleRequest = function(request, form, response, ignorePassword) {
	form = form || {};
	if (form.password && !form.md5)
		form.md5 = md5(form.password);
	...
	function renderRedirect(url, user) {
		...
		if (!form.ajax)
			response.renderHTML(loggingTemplate.htmlRedirect(url));
		else {
	...
	function renderForm(form) {
		...
		if (form.ajax)
			...
			response.renderHTML(loggingTemplate.renderLoginPage(form));
	}
	...
	if (form.mail) { // new email given on landing page
		...
		userModel.fetchByEmail(form.mail, function(u) {
			if (u)
				renderJSON({result:"You've already registered, please log in.", redirect:"javascript:login();"});
			else {
					...
					if (u)
						...
						userModel.fetchEmail(form.mail, function(err, email) {
							if (!email) {
								...
								renderJSON({result: "Don't forget to check your spam folder!"});
						});
					}
				});
			}
