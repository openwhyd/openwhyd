...
function handleUrl(url, render, raw){
	if (url.indexOf("spotify:track:") == 0)
		...
		render(new Error("URL is not recognized"));
}
...
exports.controller = function(request, reqParams, response) {
	...
	else if (isAdmin && reqParams.q)
		search({name: reqParams.q}, render);
	else if (isAdmin && reqParams._1 == "monitor")
		monitor(render);
	else if (isAdmin && (reqParams.name || reqParams.artistName || reqParams.trackTitle))
