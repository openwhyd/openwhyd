...
function handleUrl (url, render, raw) {
  if (url.indexOf('spotify:track:') == 0) { spotifyApi.fetchTrackMetadata(url, render, raw) } else if (url.indexOf('youtube.com') > -1) { youtubeApi.fetchMetadataFromYoutubePage(url, render) } else if (url.indexOf('soundcloud.com') > -1) { metadataResolver.fetchMetadataForEid('/sc/' + url, render) } else { render(new Error('URL is not recognized')) }
}

function search (trackMetadata, cb) {
  var res = {}
  var extractors = snip.mapToObjArray(metadataResolver.EXTRACTORS, 'name', 'api');
  (function next () {
    if (!extractors.length) { return cb(null, res) }
    var extr = extractors.shift()
    console.log('searching', extr.name, '...')
    metadataResolver.searchBestMatches(extr.api, trackMetadata, function (err, hits) {
...
exports.controller = function (request, reqParams, response) {
  ...
  function render (err, result) {
    ...
    response.render(reqParams.format == 'text' ? JSON.stringify(result, null, 2) : result)
  }

  if (reqParams._1 && reqParams._2) {
    var components = ['', reqParams._1, reqParams._2]
    if (reqParams._3) // soundcloud eIds contain a third component (/sc/user/title)
    { components.push(reqParams._3) }
    metadataResolver.fetchMetadataForEid(components.join('/'), render)
  } else if (isAdmin && reqParams.url) { handleUrl(reqParams.url, render, reqParams.raw) } else if (isAdmin && reqParams.q) { search({name: reqParams.q}, render) } else if (isAdmin && reqParams._1 == 'monitor') { monitor(render) } else if (isAdmin && (reqParams.name || reqParams.artistName || reqParams.trackTitle)) { search(reqParams, render) } else {
    response.badRequest()
  }
}
