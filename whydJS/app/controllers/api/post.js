...
function fetchSubscribedUsers(uidList, uid, cb) {
	followModel.fetch({uId:uid, tId:{$in:uidList}}, null, function(results) {
		...
		for (var i in results)
			uidSet[results[i].tId] = results[i];
		cb(uidSet);
	});
}
...
exports.actions = {
	...
	insert: function (p, callback) {
		...
		function actualInsert() {
			if (p.pId)
				postModel.rePost(p.pId, q, callback);
			else {
				if (p._id) // edit mode
					q._id = p._id;
	...
	toggleLovePost: function(p, callback) {
		postModel.isPostLovedByUid(p.pId, p.uId, function(loved, post) {
			if (!post)
				callback({loved:false, lovers:0}); // to prevent crash when trying to love a repost
			else if (loved)
	...
	incrPlayCounter: function(p, cb) {
		...
		if (p.logData.err)
			postModel.fetchPostById(p.pId, callbackAndLogPlay);
		else
			postModel.incrPlayCounter(p.pId, callbackAndLogPlay);
	},
	scrobble: function(p, cb) {
		...
		postModel.fetchPostById(p.pId, function(r) {
			if (!r)
				return cb && cb({error:"missing track"});
...
exports.handleRequest = function(request, reqParams, response) {
	...
	function resultHandler(res, args) {
		if (!res || res.error)
			console.log(/*reqParams,*/ "api.post." + reqParams.action, " RESULT =>", res);
