...
var publicActions = {
  ...
  reposts: function (p, callback) {
    postModel.fetchPosts({'repost.pId': p.pId}, null, null, function (results) {
      var reposts = []
      if (results) {
        var repostUids = []
...
exports.actions = {
  ...
  insert: function (p, callback) {
    ...
    function actualInsert () {
      if (p.pId) { postModel.rePost(p.pId, q, callback) } else {
        if (p._id) // edit mode
        { q._id = p._id }

        if (p.img && p.img != 'null') { q.img = p.img }

        if (p.src) // source webpage of the content: {id,name} provided by bookmarklet
        { q.src = typeof p.src === 'object' ? p.src : tryJsonParse(p.src) } else if (p['src[id]'] && p['src[name]']) {
          q.src = {
            id: p['src[id]'],
            name: p['src[name]']
          }
    ...
    if (q.pl.collabId) { delete q.pl.id } else if (q.pl.id == 'create') {
  ...
  delete: function (p, callback) {
    postModel.deletePost(p._id, function (result) {
  ...
  toggleLovePost: function (p, callback) {
    postModel.isPostLovedByUid(p.pId, p.uId, function (loved, post) {
      if (!post) { callback({loved: false, lovers: 0}) } // to prevent crash when trying to love a repost
      else if (loved) {
        ...
        postModel.lovePost(p.pId, p.uId, function () {
          callback({loved: true, lovers: (post.lov || []).length + 1, post: post})
        })
      }
    })
  },
  incrPlayCounter: function (p, cb, request) {
    ...
    function callbackAndLogPlay (post) {
      ...
      if (!p.logData.err) {
        lastFm.updateNowPlaying2(post, (mongodb.usernames[p.uId].lastFm || {}).sk, function (res) {
        // console.log("-> last fm response", res);
        })
      }
    }
    if (p.logData.err) { postModel.fetchPostById(p.pId, callbackAndLogPlay) } else { postModel.incrPlayCounter(p.pId, callbackAndLogPlay) }
  },
  scrobble: function (p, cb) {
    ...
    postModel.fetchPostById(p.pId, function (r) {
      ...
      lastFm.scrobble2(r && r.name, lastFmSessionKey, p.uId == r.uId, p.timestamp, function (res) {
        // console.log("-> last fm response", res);
        cb && cb(res)
      })
    })
...
exports.handleRequest = function (request, reqParams, response) {
  ...
  function resultHandler (res, args) {
    if (!res || res.error) { console.log(/* reqParams, */ 'api.post.' + reqParams.action, ' RESULT =>', res) }
  ...
  if (reqParams.action && publicActions[reqParams.action]) { return publicActions[reqParams.action](reqParams, resultHandler) }

  // make sure a registered user is logged, or return an error page
  if (!user || !user.id) return response.badRequest()

  if (reqParams.action && exports.actions[reqParams.action]) { exports.actions[reqParams.action](reqParams, resultHandler, request) } else { response.badRequest() }
}
