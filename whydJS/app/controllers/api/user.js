...
function addUserInfo (userSub, mySub) {
  var mySubIdSet = {}
  if (mySub) {
    for (var i in mySub) { mySubIdSet[mySub[i].id] = true }
  }
...
var publicActions = {
  'subscriptions': function (p, cb) {
    if (!p || !p.id) { cb({error: 'user not found'}) } else {
      followModel.fetchUserSubscriptions(p.id, function (sub) {
        userModel.fetchUserBios(sub.subscriptions, function () {
          if (p.loggedUser && p.loggedUser.id != p.id) {
            followModel.fetchUserSubscriptions(p.loggedUser.id, function (mySub) {
              cb(sub ? addUserInfo(sub.subscriptions, mySub ? mySub.subscriptions : []) : null)
            })
          } else { cb(sub ? addUserInfo(sub.subscriptions, p.loggedUser ? sub.subscriptions : []) : null) }
        })
      })
    }
  },
    ...
    if (!p || !p.id) { cb({error: 'user not found'}) } else {
      followModel.fetchUserSubscriptions(p.id, function (sub) {
        userModel.fetchUserBios(sub.subscribers, function () {
          if (p.loggedUser && p.loggedUser.id != p.id) {
            followModel.fetchUserSubscriptions(p.loggedUser.id, function (mySub) {
              cb(sub ? addUserInfo(sub.subscribers, mySub ? mySub.subscriptions : []) : null)
            })
          } else { cb(sub ? addUserInfo(sub.subscribers, p.loggedUser ? sub.subscriptions : []) : null) }
        })
      })
    }
  },
  'delete': function (p, cb) {
    if (!p || !p.loggedUser || !p.loggedUser.id) { cb({error: 'Please log in first'}) } else {
...
var fieldSetters = {
  ...
  'img': function (p, cb) {
    userModel.fetchByUid(p._id, function (user) {
      ...
      if (p.img.indexOf('blank_user.gif') == -1) { uploadCtr.moveTo(p.img, uploadCtr.config.uAvatarImgDir, actualUpdate) } else { actualUpdate(p.img) }
    })
  },
  'cvrImg': function (p, cb) {
    userModel.fetchByUid(p._id, function (user) {
      ...
      function actualUpdate (newFilename) {
        defaultSetter('cvrImg')({_id: p._id, cvrImg: newFilename || p.cvrImg}, cb)
      }
      if (p.cvrImg.indexOf('blank') == -1) { uploadCtr.moveTo(p.cvrImg, uploadCtr.config.uCoverImgDir, actualUpdate) } else { userModel.update(p._id, {$unset: {cvrImg: 1}}, cb) } // remove cvrImg attribute
    })
  },
  'pwd': function (p, cb) {
  ...
  'email': function (p, cb) {
    ...
    if (!emailModel.validate(p.email)) { cb({error: 'This email address is invalid'}) } else {
      userModel.fetchByEmail(p.email, function (existingUser) {
        if (!existingUser) {
          notifEmails.sendEmailUpdated(p._id, p.email)
          defaultSetter('email')(p, cb)
        } else if ('' + existingUser._id == p._id) // no change
        { cb({email: p.email}) } else { cb({error: 'This email already belongs to another user'}) }
      })
    }
  },
  'pref': function (p, cb) {
    // type each provided pref value accordingly to defaults. "true" boolean was translated to "1"
    for (var i in p.pref) { p.pref[i] = typeof userModel.DEFAULT_PREF[i] === 'boolean' ? (p.pref[i] == 1) : p.pref[i] }
  ...
  'apTok': function (p, cb) {
    if (p.apTok === '') { userModel.clearApTok(p._id, cb) } else { userModel.setApTok(p._id, p.apTok, cb) }
  },
...
function includeTags (user, cb) {
  ...
  plTagsModel.getTagEngine(function (tagEngine) {
    tagEngine.fetchTagsByUid(uId, function (tags) {
      ...
      postModel.fetchPosts({uId: '' + uId}, {fields: {name: 1}}, {limit: 10}, function (posts, b) {
        user.lastArtists = []
        for (var i in posts) {
...
exports.fetchUserData = function (user, cb) {
  ...
  (function next () {
    if (!ops.length) { cb(user) } else { ops.pop().apply(null, [user, next]) }
  })()
}
  ...
  userModel.fetchByUid(uId, function (user) {
    ...
    userModel.fetchPlaylists(user, {}, function (playlists) {
      ...
      (function next () {
        ...
        if (!item) { cb(user) } else {
          if (options[item[0]]) { item[1](user, next) } else { next() }
        }
      })()
    })
  })
...
function fetchUserByIdOrHandle (uidOrHandle, options, cb) {
  function returnUser (u) {
    var uId = (u || {}).id
    if (!uId) { cb({error: 'user not found'}) } else { fetchUserById(uId, options, cb) }
  }
  var u = mongodb.getUserFromId(uidOrHandle) || {}
  if (u.id) { returnUser(u) } else { userModel.fetchByHandle(uidOrHandle, returnUser) }
}
...
function handlePublicRequest (loggedUser, reqParams, localRendering) {
  ...
  if (handler) {
    ...
    fetchUserByIdOrHandle(reqParams.id, reqParams, function (u) {
      ...
      if (reqParams.isSubscr) { tasks.push(function (u, next) { hasSubscribed(loggedUser, u, next) }) }
...
function handleAuthRequest (loggedUser, reqParams, localRendering) {
  ...
  if (loggedUser == false) { return localRendering({error: 'user not logged in'}) }

  reqParams._id = loggedUser._id

  var toUpdate = []
  for (var i in fieldSetters) {
  ...
  if (toUpdate.length) {
    ...
    (function setNextField (prevResult) {
      ...
      if (!toUpdate.length) { localRendering(result) } else {
...
exports.controller = function (request, reqParams, response) {
  ...
  function localRendering (r) {
    if (r) { delete r.pwd }
    if (!r || r.error) { console.log('api.user.' + (reqParams._action || 'controller') + ' ERROR:', (r || {}).error || r) }
    response.renderJSON(reqParams.callback ? snip.renderJsCallback(reqParams.callback, r) : r)
  };
