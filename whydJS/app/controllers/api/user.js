			...
			followModel.fetchUserSubscriptions(p.id, function(sub) {
				userModel.fetchUserBios(sub.subscriptions, function() {
					if (p.loggedUser && p.loggedUser.id != p.id)
						...
						cb(sub ? addUserInfo(sub.subscriptions, p.loggedUser ? sub.subscriptions : []) : null);
				});
			});
	},
			...
			followModel.fetchUserSubscriptions(p.id, function(sub) {
				userModel.fetchUserBios(sub.subscribers, function() {
					if (p.loggedUser && p.loggedUser.id != p.id)
						...
						cb(sub ? addUserInfo(sub.subscribers, p.loggedUser ? sub.subscriptions : []) : null);
				});
			});
	},
...
var fieldSetters = {
	...
	"cvrImg": function(p, cb) {
		userModel.fetchByUid(p._id, function(user) {
			...
			if (p.cvrImg.indexOf("blank") == -1)
				...
				userModel.update(p._id, {$unset:{cvrImg:1}}, cb); // remove cvrImg attribute
		});
	},
	"pwd": function(p, cb) {
		userModel.fetchByUid(p._id, function(item) {
			if (item && item.pwd == userModel.md5(p.oldPwd || "")) {
				...
				cb({error: "Your current password is incorrect"});
		});
	},
		...
		if (!emailModel.validate(p.email))
			...
			userModel.fetchByEmail(p.email, function(existingUser){
				if (!existingUser) {
					...
					cb({error: "This email already belongs to another user"});
			});
	},
	"pref": function(p, cb) {
		// type each provided pref value accordingly to defaults. "true" boolean was translated to "1"
		for (var i in p.pref)
			p.pref[i] = typeof userModel.DEFAULT_PREF[i] == "boolean" ? (p.pref[i] == 1) : p.pref[i];
	...
	"apTok": function(p, cb) {
		if (p.apTok === "")
			userModel.clearApTok(p._id, cb);
		else
			userModel.setApTok(p._id, p.apTok, cb);
	},
...
function fetchUserByIdOrHandle(uidOrHandle, options, cb) {
	function returnUser(u){
		...
		if (!uId)
			...
			fetchUserById(uId, options, cb);
	}
	var u = mongodb.getUserFromId(uidOrHandle) || {};
	if (u.id)
		returnUser(u);
	else
		userModel.fetchByHandle(uidOrHandle, returnUser);
}
	...
	if (handler) {
		...
		fetchUserByIdOrHandle(reqParams.id, reqParams, function(u){
			...
			if (reqParams.isSubscr)
				tasks.push(function(u, next){ hasSubscribed(loggedUser, u, next); });
			if (reqParams.countPosts)
				tasks.push(countUserPosts);
			if (reqParams.countLikes)
				tasks.push(countUserLikes);
			(function next(){
				var fct = tasks.pop();
				fct && fct(u, next);
			})();
		...
		(function setNextField(prevResult) {
			if (prevResult)
				for (var i in prevResult)
					result[i] = prevResult[i] || result[i];
			if (!toUpdate.length)
				localRendering(result);
			else {
...
function handleRequest(loggedUser, reqParams, localRendering) {
	try {
		if (handlePublicRequest(loggedUser, reqParams, localRendering))
			return true;
	}
	catch (e){
...
exports.controller = function(request, reqParams, response) {
	...
	function localRendering(r) {
		if (r)
			delete r.pwd;
		if (!r || r.error)
			console.log("api.user."+(reqParams._action || "controller")+" ERROR:", (r || {}).error || r);
