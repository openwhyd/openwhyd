...
function detectUserId(path) {
	...
	if (splitted.length == 3 && splitted[1] == "u")
		return splitted[2];
	if (splitted.length == 2)
		for (var i in mongodb.usernames)
			if (mongodb.usernames[i].handle && mongodb.usernames[i].handle == splitted[1])
			...
			get(p.url, function(err, page) {
				try {
					...
					if (tmp && tmp.length) {
						...
						if (tmp && tmp.length)
							response.img = parseHtmlAttr(tmp[0], "src");
					}
					...
					mongodb.collections["post"].find({uId:uid}, {limit:9999999, fields:{_id:0, lov:1, nbP:1}}, function (err, cursor) {
						cursor.each(function(err, f) {
							if (!f) {
								...
								if (f.lov)
									userList[i].nbLikes += f.lov.length;
								if (f.nbP)
									userList[i].nbPlays += f.nbP;
							}
						});
					});
				});
