...
function detectUserId (path) {
  ...
  if (splitted.length == 2) {
    for (var i in mongodb.usernames) {
      if (mongodb.usernames[i].handle && mongodb.usernames[i].handle == splitted[1]) { return '' + mongodb.usernames[i]._id }
    }
  }
}
...
var processData = {
    ...
    try {
      get(p.url, function (err, page) {
        try {
          if (err) { throw (err) }
          var response = {
            title: parseHtmlText(parseHtmlText(page.find(createHtmlElemRegEx('h2'))[0]))
          }
  ...
  'featured': function (p, render) {
    followModel.fetchUserSubscriptions((p.loggedUser || {}).id, function (userSub) {
      var subscribed = snip.objArrayToSet(userSub.subscriptions || [], 'id', true)
      mongodb.collections['featured'].find({}, {limit: 20, sort: {_id: -1}}, function (err, cursor) {
        cursor.toArray(function (err, posts) {
          for (var i in posts) {
            posts[i].subscribed = subscribed[posts[i].uId]
            if (posts[i].img) { posts[i].img = posts[i].img.replace('http:', '') }
          }
          render({posts: posts})
        })
      })
  ...
  'ranking': function (p, render) {
    ...
    function fetchNextUserStats (cb, i) {
      var i = i || 0
      if (i == userList.length) { cb() } else {
        var uid = '' + userList[i].id
        console.log('fetchNextUserStats', i, uid)
        mongodb.collections['follow'].count({tId: uid}, function (err, nbSubscribers) {
          userList[i].nbSubscribers = nbSubscribers
          // TODO: this request is heavy (e.g. on uid = 518b5a447e91c862b2adea1a)
          mongodb.collections['post'].find({uId: uid}, {limit: 9999999, fields: {_id: 0, lov: 1, nbP: 1}}, function (err, cursor) {
            cursor.each(function (err, f) {
              if (!f) {
                mongodb.collections['post'].count({'repost.uId': uid}, function (err, nbAdds) {
    ...
    var steps = [
      function (cb) {
        ...
        followModel.fetchSubscriptionHistory(options, function (act) {
          ...
          console.log('=> date of last activity', act[act.length - 1]._id.getTimestamp())
          for (var i in act) { incrementUserCounter('' + act[i].tId, 'nbNewSubscribers') }
          cb()
        })
      },
      /** /
      ...
      function (cb) {
        ...
        if (userList.length > MAX_TRENDING) { userList = userList.slice(0, MAX_TRENDING) }
      ...
      function (cb) {
        ...
        fetchSubscriptions(p, function (subscr) {
          if (subscr && subscr.subscriptions) {
            var subscr = snip.arrayToSet(snip.objArrayToValueArray(subscr.subscriptions, 'id'))
            for (var i in userList) { userList[i].subscribed = subscr[userList[i].id] }
          }
          cb()
        })
      },
  ...
  'recommendedUsers': function (p, render) {
    ...
    function renderUsers (users) {
      ...
      if (users.length > MAX_RECOM_USERS) { users = users.slice(0, MAX_RECOM_USERS) }
      for (var i in users) {
        console.log(users[i].name, 'posted', (users[i].posted || []).length, 'liked', (users[i].liked || []).length, 'liker', (users[i].liker || []).length)
    ...
    followModel.fetchUserSubscriptions(uId, function (userSub) {
      var options = {excludeUids: snip.objArrayToValueArray(userSub.subscriptions || [], 'id')}
...
exports.handleRequest = function (request, reqParams, response) {
  ...
  function render (result) {
    ...
    /* else */ if (result.html) { response.render(result.html, null, {'content-type': 'text/html'}) } else { response.render(result) }
  }
      ...
      templateLoader.loadTemplate(TEMPLATE_FILE, function (template) {
        render({html: mainTemplate.renderWhydPage({
          ...
          content: template.render({
            ...
            isAdmin: reqParams.loggedUser ? request.isUserAdmin(reqParams.loggedUser) : false
          })
        })})
