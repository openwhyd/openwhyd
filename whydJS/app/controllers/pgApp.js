...
function sendHeariamEmail(p) {
	userModel.fetchByUid(p.loggedUser.id, function(user){
		...
		emailModel.email(
			user.email,
			"Thank you for connecting Whyd to Hear I Am",
			[
				'Hey '+user.name+'!',
...
function sendHeariamAdminEmail(p) {
	emailModel.email(
		ADMIN_EMAIL,
		"new whyd user connected to heariam",
...
function getUserFromWid(wid, cb) {
	...
	if (user)
		...
		userModel.fetchByHandle(wid, cb)
}
...
function handleRequest(p, cb) {
	...
	if (!app)
		cb({content: "no such app..."});
	else if (p.action == "signup")
		renderSignupPage({ app: app, loggedUser: p.loggedUser, ref: p.ref, email: p.email }, cb);
	else if (p.action == "connected" && p.appId == "heariam") { // callback from heariam
...
exports.controller = function(request, reqParams, response) {
	...
	for (var f in reqParams)
		FIELDS[f] && (p[f] = reqParams[f]);
	...
	function render(data) {
		...
		if (data.html) {
			...
			response.render(data.text || data.json || data.error);
	}
