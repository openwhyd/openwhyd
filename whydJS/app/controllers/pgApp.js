...
function sendWelcomeEmail (p, cb) {
  userModel.fetchByUid(p.loggedUser.id, function (user) {
    console.log('app sending WHYD welcome email', user.email, p.sender.id, p.sender.name)
...
function sendHeariamEmail (p) {
  userModel.fetchByUid(p.loggedUser.id, function (user) {
    user = user || {}
    console.log('app sending HEARIAM welcome email', user.email, p.sender.id, p.sender.name)
    if (!user.email) { return console.error('ERROR: user not found in sendHeariamEmail') }
    emailModel.email(
      user.email,
...
function getUserFromWid (wid, cb) {
  ...
  if (user) { cb(user) } else { userModel.fetchByHandle(wid, cb) }
}

function handleRequest (p, cb) {
  p = p || {}
  var app = apps[p.appId]
  if (!app) { cb({content: 'no such app...'}) } else if (p.action == 'signup') { renderSignupPage({ app: app, loggedUser: p.loggedUser, ref: p.ref, email: p.email }, cb) } else if (p.action == 'connected' && p.appId == 'heariam') { // callback from heariam
    ...
    if (p.action == 'signedup' && p.loggedUser) { sendWelcomeEmail({ loggedUser: p.loggedUser, sender: app }) }
