	...
	plTagsModel.getTagEngine(function(tagEngine){
		mongodb.forEach2("post", {q:p.q, after:p.after, before:p.before, sort:[['_id','desc']]}, function(post, next) {
			if (!next || posts.length >= p.limit)
				...
				if (tags && tags.length && tags[0].id == genre)
					posts.push(post);
				next();
			}
		});
...
function fetchGenreStream(p, cb) {
	...
	trackModel.fetchPostsByGenre(p.genre, {limit: LIMIT+1}, function(posts) {
		...
		if (hasMore)
			posts = posts.slice(0, LIMIT);
		for (var i in posts) {
			if (posts[i].rankIncr < 0)
				posts[i].cssClass = "rankingUp";
			else if (posts[i].rankIncr > 0)
				posts[i].cssClass = "rankingDown";
			if (p.loggedUser)
				posts[i].isLoved = snip.arrayHas(posts[i].lov, ""+p.loggedUser.id);
		}
		cb({
...
exports.controller = function(request, reqParams, response) {
	...
	function render(data) {
		...
		if (data.html) {
			...
			response.render(data.json || data.error);
	}
	...
	if (!genre)
		render({error: "hmm... we don't know that genre..."});
	else if (tab == "hot") {
		...
		fetchGenreLatest(fetchingParams, function(r) {
			...
			if (reqParams.after)
				render({html:template.render(renderingParams)});
			else
				renderPage(template.render(renderingParams));
		});
	}
