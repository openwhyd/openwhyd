...
var template = require('../templates/templateLoader.js').loadTemplate('app/templates/pgGenre.html')
...
function fetchAllPostsByGenre (genre, p, cb) {
  ...
  plTagsModel.getTagEngine(function (tagEngine) {
    mongodb.forEach2('post', {q: p.q, after: p.after, before: p.before, sort: [['_id', 'desc']]}, function (post, next) {
      if (!next || posts.length >= p.limit) { cb(posts) } else {
...
function fetchGenreLatest (p, cb) {
  ...
  fetchAllPostsByGenre(p.genre, {q: {repost: {$exists: false}}, after: p.after/*, before: p.before */, limit: LIMIT + 1}, function (posts) {
    var hasMore = posts && posts.length > LIMIT
    if (hasMore) { posts = posts.slice(0, LIMIT) }
    if (p.loggedUser) {
      for (var i in posts) { posts[i].isLoved = snip.arrayHas(posts[i].lov, '' + p.loggedUser.id) }
    }
    cb({
...
function fetchGenreStream (p, cb) {
  ...
  trackModel.fetchPostsByGenre(p.genre, {limit: LIMIT + 1}, function (posts) {
    ...
    var hasMore = (posts && posts.length > LIMIT) ? { skip: firstIndex + LIMIT } : null // to do before renderPosts
    if (hasMore) { posts = posts.slice(0, LIMIT) }
    for (var i in posts) {
      if (posts[i].rankIncr < 0) { posts[i].cssClass = 'rankingUp' } else if (posts[i].rankIncr > 0) { posts[i].cssClass = 'rankingDown' }
      if (p.loggedUser) { posts[i].isLoved = snip.arrayHas(posts[i].lov, '' + p.loggedUser.id) }
    }
    cb({
      hasMore: hasMore,
      posts: posts
...
function populateUserListData (tagEngine, loggedUid, users, cb) {
  followModel.fetchSubscriptionSet(loggedUid, function (subscrSet) {
    (function next (i) {
      if (--i < 0) { cb(users) } else if (!users[i]) {
...
exports.controller = function (request, reqParams, response) {
  ...
  if (loggedInUser && loggedInUser.id) { loggedInUser.isAdmin = request.isUserAdmin(loggedInUser) }
  ...
  if (!genre) { render({error: "hmm... we don't know that genre..."}) } else if (tab == 'hot') {
    ...
    fetchGenreLatest(fetchingParams, function (r) {
      ...
      if (reqParams.after) { render({html: template.render(renderingParams)}) } else { renderPage(template.render(renderingParams)) }
    })
  } else if (tab == 'people') {
