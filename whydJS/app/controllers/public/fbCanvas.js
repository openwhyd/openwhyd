	...
	var checkInvite = function (reqId) {
		userModel.fetchInviteByFbRequestId(reqId, function (invite) {
			...
			if (invite && invite.fbRequestIds) {
				...
				var registrationPage = invitePage.renderInvitePage(sender, null/*request.getUser()*/,
					invite._id, null, "", reqIds);
				response.render(registrationPage, null, {'content-type': 'text/html'});
			}
			else
				console.log("request id not found: ", reqId);
			if (--remaining == 0) {
				console.log("invitation token not found => redirect to /");
				//response.render("Your invite code is invalid or expired. Please go to http://whyd.com/ to request an invite.");
				response.redirect("/");
			}
		});
	};
