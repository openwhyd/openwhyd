...
function renderError (request, getParams, response, errorMsg) {
  ...
  response[getParams.ajax == 'iframe' ? 'renderWrappedJSON' : 'renderJSON'](json)
}
...
exports.registerInvitedUser = function (request, user, response) {
  request.logToConsole('register.registerInvitedUser', user ? {
  ...
  if (ENFORCE_GENUINE_SIGNUP_FROM_IOS || user.iRf !== 'iPhoneApp') {
    ...
    if (!user.sTk || (typeof user.sTk !== 'string')) { return error(request, user, response, 'Invalid sTk') }
    ...
    if (!genuineToken) { return error(request, user, response, 'Non genuine signup request') }
  }

  if (!user.name) { return error(request, user, response, 'Please enter your name') }

  if (!user.password) { return error(request, user, response, 'Please enter a password') }

  if (!user.email) { return error(request, user, response, 'Please enter your email') }

  if (checkInvites && !user.inviteCode) { return error(request, user, response, 'Invalid invite code') }

  user.email = emailModel.normalize(user.email)

  if (!emailModel.validate(user.email)) { return error(request, user, response, 'This email address is invalid') }

  if (user.name == 'Your Name' || user.name.trim() == '') { return error(request, user, response, "Don't you have a name, dude ?") }

  if (user.fbId && isNaN('' + user.fbId)) { return error(request, user, response, 'Invalid Facebook id') }

  if (user.password == 'password') { return error(request, user, response, 'You have to set your password first') } else if (user.password.length < 4 || user.password.length > 32) { return error(request, user, response, 'Your password must be between 4 and 32 characters') }
  ...
  function registerUser () {
    userModel.fetchByEmail(user.email, function (item) {
      if (item) { return error(request, user, response, 'This email address is already registered on whyd') }
  ...
  function afterSave (storedUser) {
    if (!storedUser) { return error(request, user, response, 'Oops, your registration failed... Please try again!') }

    var inviteSender = user.iBy ? request.getUserFromId(user.iBy) : null

    if (user.fbRequest) { userModel.removeInviteByFbRequestIds(user.fbRequest) } else { userModel.removeInvite(user.inviteCode) }
    ...
    function loginAndRedirectTo (url) {
      ...
      if (user.ajax) {
        ...
        function renderJSON () {
          response[user.ajax == 'iframe' ? 'renderWrappedJSON' : 'renderJSON'](json)
        }
        if (user.includeUser) {
    ...
    // connect users
    if (user.iBy) {
      var inviteSender = {id: user.iBy, name: request.getUserNameFromId(user.iBy)}
    ...
    if (user.plContest) {
      userModel.createPlaylist(storedUser._id, user.plContest.title, function (playlist) {
    ...
    inviteController.checkInviteCode(request, user, response, function () {
      if (user.plC) {
        contestModel.fetchById(user.plC, function (plContest) {
          if (!plContest) { console.error('plcontest not found when trying to signup') } else { user.plContest = plContest }
          registerUser()
        })
      } else { registerUser() }
    })
...
exports.controller = function (request, getParams, response) {
  ...
  { exports.registerInvitedUser(request, request.body, response) } else { inviteController.renderRegisterPage(request, getParams, response) }
}
