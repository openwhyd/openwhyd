...
function follow (user, userToFollow, ctx) {
	followModel.add({
		uId:user.id, uNm:user.name,
		tId:userToFollow.id, tNm:userToFollow.name,
		ctx: ctx
	}, function(){
...
exports.registerInvitedUser = function(request, user, response) {
	...
	if (user.password == "password")
		return error(request, user, response, "You have to set your password first");
	else if (user.password.length < 4 || user.password.length > 32)
		return error(request, user, response, "Your password must be between 4 and 32 characters");
	...
	function registerUser() {
		userModel.fetchByEmail(user.email, function(item) {
			if (item)
				return error(request, user, response, "This email address is already registered on whyd");
	...
	function afterSave(storedUser) {
		if (!storedUser)
			return error(request, user, response, "Oops, your registration failed... Please try again!");

		var inviteSender = user.iBy ? request.getUserFromId(user.iBy) : null;
		
		if (user.fbRequest)
			userModel.removeInviteByFbRequestIds(user.fbRequest);
		else
		...
		function loginAndRedirectTo(url) {
			...
			if (user.ajax) {
				...
				response.renderHTML(htmlRedirect(url));
		}
...
exports.controller = function(request, getParams, response) {
	...
	if (request.method.toLowerCase() === 'post') // sent by (new) register form
		...
		inviteController.renderRegisterPage(request, getParams, response);
};
