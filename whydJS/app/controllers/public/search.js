...
var fetchDataByType = {
	...
	"post": function(hits, cb) {
		...
		postModel.fetchPosts({_id:{$in:idList}}, {}, {limit: MAX_RESULTS}, function(items){
			...
			for (var i in items)
				if (items[i])
					itemSet[""+items[i]._id] = items[i];
			items = [];
			for (var i in idList)
				items.push(itemSet[""+idList[i]]);
			cb(items);
		});
	},
	...
	"playlist": function(hits, cb) {
		...
		for (var i in playlists)
			playlists[i].idParts = (""+(playlists[i]._id || playlists[i].id)).split("_");
		function next(i) {
          ...
          delete playlists[i].author.email
...
exports.controller = function(request, reqParams, response, error) {
	...
	function renderSearchPage(q, cb) {
		fetchSearchPage(request.getUid(), q, function(results) {
			...
			if (reqParams.format == "json")
				cb({q: q, results: results});
			else
				template.renderSearchPage(results, reqParams, cb);
		});
	}
	...
	function renderResultsBox(q, format, cb) {
		fetchResultsPerType({q:q}, function(resultsPerType) {
			...
			if (format == "html")
				template.renderResultBox(q, resultsPerType, cb);
			else
				cb({q:q, results:resultsPerType});
		});
	}
	...
	// tracks posted by other users (from add track dialog)
		...
		fetchOtherTracks((reqParams.loggedUser || {}).id, reqParams.q, renderJSON); // prevent crash
	...
	// main search (triggered from header bar)
		...
		renderSearchPage(reqParams.q, render);

	// main search page (default case)
	else
		template.renderSearchPage(null, reqParams, renderHTML);
}
