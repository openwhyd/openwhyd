...
function removeEmptyItems (posts) {
  var finalPosts = []
  for (var i in posts) {
    if (posts[i]) { finalPosts.push(posts[i]) }
  }
  return finalPosts
}
...
var fetchDataByType = {
  ...
  'post': function (hits, cb) {
    ...
    postModel.fetchPosts({_id: {$in: idList}}, {}, {limit: MAX_RESULTS}, function (items) {
  ...
  'playlist': function (hits, cb) {
    ...
    var result = []
    for (var i in playlists) { playlists[i].idParts = ('' + (playlists[i]._id || playlists[i].id)).split('_') }
    function next (i) {
      ...
      postModel.countPlaylistPosts(pl.idParts[0], pl.idParts[1], function (c) {
        if (!c) { next(i + 1) } else {
          ...
          postModel.fetchPlaylistPosts(pl.idParts[0], pl.idParts[1], {limit: MAX_PLAYLIST_THUMBS - 1}, function (posts) {
            for (var j in posts) {
              if (posts[j].img) { posts[j].img = posts[j].img.replace('http:', '') }
            }
...
var sortByType = {
  ...
  'post': function (items, myUid, followedUids) {
    for (var i in items) {
      ...
      items[i].score = 0 + 2 * (items[i].uId == myUid) + items[i].isSubscribed + (items[i].nbLikes > 1)
    }
  ...
  'user': function (items, myUid, followedUids) {
    for (var i in items) {
      ...
      items[i].score = 0 + !!items[i].img + items[i].isSubscribed + !!items[i].lastTrack
    }
    items.sort(sortByDecreasingScore)
    return items
  },
  'playlist': function (items, myUid, followedUids) {
    for (var i in items) {
      ...
      items[i].score = 0 + (2 * (items[i].idParts[0] == myUid) + items[i].isSubscribed + 1) * items[i].nbTracks
    }
    items.sort(sortByDecreasingScore)
    return items
  }
...
function processPosts (results, params, cb) {
  ...
  fetchDataByType['post'](results, function (posts) {
    ...
    if (params.uid) { followModel.fetchSubscriptionSet(params.uid, populateSortAndReturn) } else { populateSortAndReturn({}) }
  })
}
...
exports.controller = function (request, reqParams, response, error) {
  function renderSearchPage (q, cb) {
    fetchSearchPage(request.getUid(), q, function (results) {
      ...
      if (reqParams.format == 'json') { cb({q: q, results: results}) } else { template.renderSearchPage(results, reqParams, cb) }
    })
  }
  ...
  function renderResultsBox (q, format, cb) {
    fetchResultsPerType({q: q}, function (resultsPerType) {
      ...
      if (format == 'html') { template.renderResultBox(q, resultsPerType, cb) } else { cb({q: q, results: resultsPerType}) }
    })
  }
    ...
    if (!uid) {
      ...
      // console.log("myResults", myResults);
        ...
        // console.log("theirResults", myResults);
          processPosts((theirResults || {}).hits, {uid: uid}, function (theirPosts) {
  ...
  function renderFilteredFeed (uid, q, cb) {
    searchModel.query({q: q, uId: uid}, function (results) { // uid is provided => only posts results will be returned
      fetchDataByType['post']((results || {}).hits || [], function (posts) {
        if (reqParams.format == 'html') { template.renderPosts(posts, reqParams.loggedUser, cb) } else { cb({q: q, uId: uid, results: posts}) }
      })
    })
  }
  ...
  function fetchQuickUsers (uid, q, cb) {
    followModel.fetchSubscriptionSet(uid, function (followed) {
      var sorted = []
      searchModel.query({_type: 'user', q: reqParams.q, limit: MAX_NB_MENTION_SUGGESTIONS}, function (users) {
        var hits = (users || {}).hits || []
  ...
  if (reqParams.q && reqParams.uid) { renderFilteredFeed(reqParams.uid, reqParams.q, render) }

  // mentions (for comments)
  else if (reqParams.q && reqParams.context == 'mention') { fetchQuickUsers((reqParams.loggedUser || {}).id, reqParams.q, renderJSON) }

  // tracks posted by other users (from add track dialog)
  else if (reqParams.q && reqParams.context == 'addTrack') { fetchOtherTracks((reqParams.loggedUser || {}).id, reqParams.q, renderJSON) } // prevent crash

  // combination of tracks posted by me and other users (from /mobile web interface)
  else if (reqParams.q && reqParams.context == 'quick') { fetchQuickResults((reqParams.loggedUser || {}).id, reqParams.q, renderJSON) }

  // quick/autocomplete search (from header bar)
  else if (reqParams.q && reqParams.context == 'header') { renderResultsBox(reqParams.q, reqParams.format, render) }
