...
var tabParams = ['showPlaylists', 'showLikes', 'showActivity', 'showSubscribers', 'showSubscriptions']
var paramsToInclude = ['after', 'before', 'limit', 'playlistId', 'embedW', 'format', 'pageUrl', 'callback'].concat(tabParams)
...
function LibraryController (reqParams, render) {
  ...
  for (var i in paramsToInclude) { this.options[paramsToInclude[i]] = reqParams[paramsToInclude[i]] }
  if (typeof this.options.limit === 'string') { this.options.limit = parseInt(this.options.limit) }
...
exports.controller = function (request, reqParams, response) {
  ...
  if (loggedInUser && loggedInUser.id) { reqParams.loggedUser.isAdmin = request.isUserAdmin(loggedInUser) }
  ...
  function render (data, mimeType) {
    if (mimeType) { return response.render(data, null, {'content-type': mimeType}) }
    data = data || {error: 'Nothing to render! Please send the URL of this page to ' + process.appParams.feedbackEmail}
    if (data.errorCode) {
      // response.renderHTML(errorTemplate.renderErrorCode(data.errorCode));
      errorTemplate.renderErrorResponse(data, response, reqParams.format, loggedInUser)
    } else if (data.error) {
    ...
    } else if (typeof data === 'object') { response.renderJSON(data.json || data) } else { response.render(data.error) }
  }

  var lib = new LibraryController(reqParams, render)

  function redirectTo (path) {
    var paramsObj = {}, paramsToKeep = ['after', 'before', 'embedW', 'format', 'limit', 'callback']
    for (var i in paramsToKeep) {
      if (reqParams[paramsToKeep[i]]) { paramsObj[paramsToKeep[i]] = reqParams[paramsToKeep[i]] }
    }
  ...
  if (path == '/' || request.url.indexOf('/stream') > -1 || request.url.indexOf('/welcome') > -1) {
    if (loggedInUser && loggedInUser.id) { return renderFriendsLibrary(lib) } else if (reqParams.format == 'json') { return render({errorCode: 'REQ_LOGIN'}) } else {
      loggingTemplate.renderLandingPage(loggedInUser, reqParams, function (html) {
    ...
    if (request.checkLogin(response, reqParams.format)) {
      userModel.fetchByUid(loggedInUser.id, function (user) {
        if (!user) { render({errorCode: 'USER_NOT_FOUND'}) } else { redirectTo(path.replace('/me', user.handle ? '/' + user.handle : '/u/' + reqParams.id)) }
      })
    }
  } else if (path == '/all') { return renderAllLibrary(lib) } else if (reqParams.style) {
    ...
    if (reqParams.handle) {
      userModel.fetchByHandle(reqParams.handle, function (user) {
        if (user) { redirectTo(path.replace('/' + reqParams.handle, '/u/' + user._id)) } else { render({errorCode: 'USER_NOT_FOUND'}) }
      })
    ...
    if (!mongodb.isObjectId(reqParams.id)) { return render({errorCode: 'USER_NOT_FOUND'}) }
    userModel.fetchByUid(reqParams.id, function (user) {
      if (!user) { render({errorCode: 'USER_NOT_FOUND'}) } else if (user.handle && !reqParams.embedW) { redirectTo(path.replace('/u/' + reqParams.id, '/' + user.handle)) } else { renderUserLibrary(lib, user) }
    })
