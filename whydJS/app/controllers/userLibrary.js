...
exports.controller = function(request, reqParams, response) {
	...
	function render(data, mimeType) {
		...
		if (data.errorCode) {
			...
			response.renderJSON(data.json || data);
		else
			response.render(data.error);
	}
	...
	if (path == "/" || request.url.indexOf("/stream") > -1 || request.url.indexOf("/welcome") > -1) {
		...
		if (request.checkLogin(response, reqParams.format))
			userModel.fetchByUid(loggedInUser.id, function(user){
				if (!user)
					render({errorCode: "USER_NOT_FOUND"});
				else
					redirectTo(path.replace("/me", user.handle ? "/"+user.handle : "/u/"+reqParams.id));
			});
	}
	else if (path == "/all")
