...
function countOccurences(array) {
	var count = {};
	for (var j in array)
		count[array[j]] = (count[array[j]] || 0) + 1;
...
function gatherTrackAnalytics(cb){
		...
		mongodb.collections["post"].find({}, {batchSize:1000, fields:{_id: 0, eId:1, lov:1, name:1}}, function(err, cursor){
			cursor.each(function(err, track) {
				...
				if (!track) // we're done
					...
					console.error("warning: null track eid");
				else {
	...
	if (cachedTrackAnalytics)
		cb(cachedTrackAnalytics);
	else
			...
			if (!track)
				...
				if (track.name) {
					...
					if (artistName) {
						...
						console.log("unrecognized track artist: ", track.name);
				}
				else
					console.error("warning: missing track name");
			}
		});
	}
	if (cachedArtistAnalytics)
		cb(cachedArtistAnalytics);
	else
		...
		(function next() {
			...
			recomUsersFct(user.id, function(allUsers){
				...
				for (var i in allUsers)
					if (!allUsers[i].subscribed)
						recomUsers.push(allUsers[i]);
				...
				for (var i=table.length-1; i>0; --i) {
					...
					for (var j=0; j<nbClasses; ++j)
						if (nbRecoms >= j)
							...
							break; // => jump to next user of the recom table
				}
				cb(classes);
			});
		}
