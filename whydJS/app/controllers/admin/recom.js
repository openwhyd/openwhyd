...
function gatherTrackAnalytics (cb) {
  ...
  function generateMap (cb) {
    var nb = 0
    var eIds = {}
    mongodb.collections['post'].find({}, {batchSize: 1000, fields: {_id: 0, eId: 1, lov: 1, name: 1}}, function (err, cursor) {
      cursor.each(function (err, track) {
        ++nb
        if (!track) // we're done
        { cb(eIds, nb) } else if (!track.eId) { console.error('warning: null track eid') } else {
          eIds[track.eId] = eIds[track.eId] || {name: track.name, posts: 0, likes: 0}
  ...
  if (cachedTrackAnalytics) { cb(cachedTrackAnalytics) } else {
    generateMap(function (eIds, nb) {
      console.log('cached', nb, 'posts')
      for (var eId in eIds) { eIds[eId].interacted = eIds[eId].posts + eIds[eId].likes }
...
function gatherArtistAnalytics (cb) {
  ...
  function generateMap (cb) {
    var nb = 0
    var artists = {}
    postModel.forEachPost({}, {batchSize: 1000, fields: {_id: 0, lov: 1, name: 1}}, function (track) {
      if (!track) { cb(artists, nb) } else {
        ++nb
        if (track.name) {
          var artistName = snip.detectArtistName(snip.cleanTrackName(track.name))
          if (artistName) {
            var artist = snip.normalizeArtistName(artistName)
            artists[artist] = artists[artist] || {name: /* track.name */artistName, posts: 0, likes: 0}
  ...
  if (cachedArtistAnalytics) { cb(cachedArtistAnalytics) } else {
    generateMap(function (map, nb) {
      console.log('cached', nb, 'posts')
      for (var artist in map) { map[artist].interacted = map[artist].posts + map[artist].likes }
...
function RecomEvaluator (recomUsersFct) {
  ...
  function buildEvalTable (cb) {
    ...
    function addUserRecoms (res) {
      var recom = [res.user.id, res.user.name, res.recomUsers.length, res.minScore, res.maxScore]
  ...
  function gatherEvalTable (cb) {
    if (cachedEvalTable) { cb(cachedEvalTable) } else { buildEvalTable(cb) }
  }
  return {
    genTsv: function (cb) {
      gatherEvalTable(function (table) {
        ...
        console.log('generated recommendations for ', nbUsersWithMatches, '/', totalUsers, 'users')
    ...
    countClasses: function (nbClasses, cb) {
      gatherEvalTable(function (table) {
        console.log('generated recommendations for ', nbUsersWithMatches, '/', totalUsers, 'users')
        var classes = [/* {gte:4, c:0}, {gte:3, c:0}, {gte:2, c:0}, {gte:1, c:0}, {gte:0, c:0} */]
        for (var j = 0; j < nbClasses; ++j) { classes.push(0) }
        for (var i = table.length - 1; i > 0; --i) {
          console.log(table[i])
          var nbRecoms = table[i][2]
          for (var j = 0; j < nbClasses; ++j) {
            if (nbRecoms >= j) { ++classes[j] } else { break }
          } // => jump to next user of the recom table
        }
        cb(classes)
      })
    }
...
var fileGenerators = {
  ...
  'bandsToUsers.json': function (p, cb) {
    ...
    recomModel.matchingEngine.fetchUsersByArtists(bands, null, function (users) {
      var users = snip.mapToObjArray(users).map(function (user) {
        ...
        user.artists = artists.map(function (artist) {
  ...
  'bandsToUsers.tsv': function (p, cb) {
    fileGenerators['bandsToUsers.json'](p, function (users) {
      ...
      cb({tsv: (new snip.DataTable()).fromArray(users.map(function (user) {
  ...
  'tracks.txt': function (p, cb) {
    gatherTrackAnalytics(function (map) {
      var trackNames = []
      for (var eId in map) {
        var cleaned = snip.detectTrackFields(snip.cleanTrackName(map[eId].name))
        if (cleaned /*! = map[eId].name */) { trackNames.push(cleaned + ' <- ' + map[eId].name) }
      }
      cb(trackNames.join('\n'))
    })
  },
  ...
  'tracks.json': function (p, cb) {
    gatherTrackAnalytics(function (eIds, nb) {
      ...
      var postedTracks = snip.objArrayToValueArray(eIds, 'posts').sort(snip.descSort)
      var likedTracks = snip.objArrayToValueArray(eIds, 'likes').sort(snip.descSort)
      var interactedTracks = snip.objArrayToValueArray(eIds, 'interacted').sort(snip.descSort)
      cb({
        quartiles: {
  ...
  'artists.json': function (p, cb) {
    gatherArtistAnalytics(function (map, nb) {
      ...
      var interacted = snip.objArrayToValueArray(map, 'interacted').sort(snip.descSort)
