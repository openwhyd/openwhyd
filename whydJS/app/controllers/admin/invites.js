...
var inviteUser = function (email, handler) {
	...
	userModel.inviteUser(email, function(storedUser){
		if (storedUser)
			notifEmails.sendAcceptedInvite(storedUser);
		handler(storedUser);
	});
	return true;
};
...
function renderUserList (users, title, actionNames)
{
	...
	for (var i in users)
	{
		...
		if (title == "invites")
			var link = "/invite/"+u._id;
		else
			var link = u.name ? "/u/"+u._id : (/*u.email ? "mailto:" + u.email :*/ null);
		
		if (u.fbRequestIds)
			u.fbInvites = u.fbRequestIds.join ? u.fbRequestIds.length + " fb invites" : "1 fb invite";
			...
			fetchUsers("invite", function(err, invites) {
				fetchUsers("email", function(err, requests) {
					for (var i in requests)
						requests[i] = {_id:requests[i]._id, email:requests[i]._id, date:requests[i].date};
