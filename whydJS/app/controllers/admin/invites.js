...
function renderUserList (users, title, actionNames) {
  ...
  if (users && users.length) {
    ...
    userList += '<p style="position:relative;top:-5px;color:gray;">' +
			'<input type="checkbox" onClick="toggle(\'' + fieldName + '\', \'' + title + '\', this)" />toggle all</p>'
  }
  ...
  for (var i in users) {
    ...
    var img = u.img || (u.fbId ? '//graph.facebook.com/v2.3/' + u.fbId + '/picture?type=square' : null)

    if (title == 'invites') { var link = '/invite/' + u._id } else { var link = u.name ? '/u/' + u._id : (/* u.email ? "mailto:" + u.email : */ null) }

    if (u.fbRequestIds) { u.fbInvites = u.fbRequestIds.join ? u.fbRequestIds.length + ' fb invites' : '1 fb invite' }

    userList += '<p style="clear:both;">' +
				(actionNames ? '<input type="checkbox" name="' + fieldName + '" value="' + userId + '" />' : '') +
				(img ? '<img src="' + img + '" width="24px" height="24px" style="float:left;"/>&nbsp;' : '') +
				(link ? '<a href="' + link + '">' : '') +
					(u.fbInvites || u.name || u.email) +
				(link ? '</a>' : '') +
				(u.fbId ? '&nbsp;(<a href="//www.facebook.com/profile.php?id=' + u.fbId + '">FB</a>)' : '') +
				'<small>' + (u.name ? '<br/>&nbsp;' + u.email : '') +
				(u.iBy ? '<br/>&nbsp;invited by ' + (mongodb.usernames[u.iBy] || {}).name : '') +
				(u.iPo ? '<br/>&nbsp;from post #<a href="/c/' + u.iPo + '">' + u.iPo + '</a>' : '') +
				(u.iPg ? '<br/>&nbsp;from page <a href="' + u.iPg + '">' + u.iPg + '</a>' : '') +
				(u.iRf ? '<br/>&nbsp;via referrer <a href="' + u.iRf + '">' + u.iRf + '</a>' : '') +
				' (' + date + ')</small>' +
				'</p>'
  }
  ...
  if (actionNames) {
    ...
    for (var i in actionNames) {
      userList += '<input type="submit" name="action" value="' + actionNames[i] + '"' +
				' onclick="return confirm(\'Your are going to ' + actionNames[i] + ' the following user(s):\\n\' + getSelectedCheckbox(form.email) + \'\\nAre you sure?\')" />'
    }
...
function renderTemplate (requests, invites, users, reqParams) {
  ...
  var out = [
    ...
    '► <a href="?' + (new Date()).getTime() + '">REFRESH</a> (please use this link instead of browser\'s refresh)',
    '<p>Invite by email: ',
    '<form method="post">',
    '<input name="email">',
...
exports.handleRequest = function (request, reqParams, response) {
  ...
  if (!user /* || !(user.fbId == "510739408" || user.fbId == "577922742") */) { return }

  var fetchAndRender = function () {
    fetchUsers('user', function (err, users) {
      fetchUsers('invite', function (err, invites) {
        fetchUsers('email', function (err, requests) {
          for (var i in requests) { requests[i] = {_id: requests[i]._id, email: requests[i]._id, date: requests[i].date} }

          response.render(renderTemplate(requests, invites, users, reqParams), null, {'content-type': 'text/html'})
  ...
  if (reqParams.action && reqParams.title) {
    ...
    } else if (reqParams.title == 'invites' && reqParams.action == 'delete' && emails.length) {
