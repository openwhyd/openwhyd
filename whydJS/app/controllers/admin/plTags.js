		...
		fileGenerators[name](p, function(items){
			...
			for(var i in items) {
				...
				for(var u in items[i])
					lines.push("    " + util.inspect(items[i][u]).replace(/\n/g, " ").replace(/[ ]+/g, " ") + ",");
				lines.push("  ],");
			}
			lines.push("}");
			cb(lines.join('\n'));
	...
	**/
	"bestUsersForTags.json": function(p, cb) {
		...
		plTagsModel.getTagEngine(function(tagEngine){
			cb(tagEngine.getUsersByTags(tags).map(function(user){
				user.tags = (tagEngine.getBestTagsByUid(user.id) || []).slice(0,10).map(function(tag){
					return tag.id + " (" + tag.c + ")";
				});
				return user;
			}));
	...
	"nbTaggedTracks": function(p, cb) {
		plTagsModel.getTagEngine(function(tagEngine){
			var nb = 0, total = 0, eids = {}, eidToTags = tagEngine.eidToTags;
			mongodb.forEach("track", {fields:{_id:0, eId:1}}, function(post){
				if (eids[post.eId])
					return;
				eids[post.eId] = true;
				total++;
				if ((eidToTags[post.eId] || []).length)
					++nb;
			}, function(){
				cb("tagged tracks (thanks to playlists): " + nb + " / " + total + " = " + (100*nb/total) + "%");				
			});
		});
	},
	...
	"nbTracksInPlaylists": function(p, cb) {
		mongodb.collections["post"].count({}, function(err, nbTracks){
			mongodb.collections["post"].count({"pl.id": {$exists: true}}, function(err, nbTracksInPlaylists){
				cb("number of tracks in playlists: " + nbTracksInPlaylists + " / " + nbTracks
