...
function listBestContributors(cb) {
	var users = {};
	mongodb.forEach("post", {fields:{_id:0, uId:1, uNm:1}}, function (f) {
		users[f.uId] = users[f.uId] || {id:f.uId, name:f.uNm, c:0};
		users[f.uId].c++;
	}, function(){
		cb(snip.mapToObjArray(users).sort(snip.makeFieldSort("c", snip.descSort)));
	});
}

function listTracksFromBookmarklet(p, cb) {
	var bkPosts = [];
	mongodb.forEach("post", {q:{"src.id":{$exists:true}, "repost":{$exists:false}}, fields:{"_id":1, uNm:1, "src.id":1}}, function(post){
		if (post && post.src.id)
			bkPosts.push({
				pId: ""+post._id,
				uNm: post.uNm,
...
function listBookmarkletSources(p,cb) {
	listTracksFromBookmarklet(p, function(bkPosts){
		...
		for (var i in sources)
			sources[i].u = Object.keys(sources[i].u).length;
