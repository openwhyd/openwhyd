  ...
  'rename': function (p, cb) {
    if (p._id && p.name) {
      userModel.renameUser(p._id, p.name, function (result) {
        result = result || {}
        if (result.error) { console.log('error:', result.error) } else { result.message = 'The user name has been set to ' + p.name }
        cb(result)
      })
...
function renderItem (item) {
  ...
  return {
    ...
    info: [
      ...
      'Registered on ' + date +
				(item.fbId ? ', using <a href="https://www.facebook.com/profile.php?id=' + item.fbId + '">FB</a>' : ''),
      (item.iBy ? 'invited by ' + snip.htmlEntities((mongodb.usernames[item.iBy] || {}).name) : '') +
...
exports.handleRequest = function (request, reqParams, response) {
  ...
  function renderResult (result) {
    ...
    var html = result.html || ((result.error ? '<h1>error</h1><p>' + result.error + '</p>'
  ...
  if (reqParams.action && handlers[reqParams.action]) { handlers[reqParams.action](reqParams, renderResult) } else {
    userModel.fetchMulti({}, {sort: [['_id', 'desc']], limit: 600}, function (users) {
