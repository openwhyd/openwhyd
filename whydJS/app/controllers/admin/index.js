...
function refreshIndex(type, cb, preprocess) {
	...
	function flush(cb){
		searchModel.indexBulk(bulkDocs, function(r){
			r = r || {};
			if (r.errors || r.error)
				console.error("[ERR] (BULK) admin.index.refreshIndex, searchModel.index: " + (r.errors || r.error));
	...
	refreshPlaylistIndex: function(cb) {
		refreshIndex("playlist", cb, function(user, nextUser, index){
			function nextPlaylist() {
				var p = user.pl.pop();
				if (!p)
					nextUser(); // no more playlists for this user => process next user
				else {
		...
		(function nextUser(){
			cursor.nextObject(function(err, user){
				if (!user)
					...
					++result.dbUsers;
					if (user.pl)
						result.dbPlaylists += user.pl.length;
