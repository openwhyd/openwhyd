...
function refreshIndex (type, cb, preprocess) {
  var BULK_SIZE = 1000, bulkDocs = [], fetched = 0, indexed = 0
  console.log('Indexing ' + type + 's ...')
  function flush (cb) {
    searchModel.indexBulk(bulkDocs, function (r) {
      r = r || {}
      if (r.errors || r.error) { console.error('[ERR] (BULK) admin.index.refreshIndex, searchModel.index: ' + (r.errors || r.error)) }
  ...
  function index (doc, cb) {
    ...
    if (bulkDocs.length >= BULK_SIZE) { flush(cb) } else { cb() }
  }
  var process = !preprocess ? index : function (obj, fetchAndProcessNextObject) {
  ...
  mongodb.collections[indexCol[type]].find({}, options, function (err, cursor) {
    (function next () {
      cursor.nextObject(function (err, u) {
        if (err) { console.log('[ERR] admin.index.refreshIndex, db.nextObject: ', err) }
        if (u != null) {
          ++fetched
          process(u, function () {
            if (fetched % 100 == 0) { console.warn('=> last (BULK) indexed document: ', u._id) }
            setTimeout(next /*, 100 */)
          })
        } else {
          flush(function () {
  ...
  refreshPlaylistIndex: function (cb) {
    refreshIndex('playlist', cb, function (user, nextUser, index) {
      function nextPlaylist () {
        var p = user.pl.pop()
        if (!p) { nextUser() } // no more playlists for this user => process next user
        else {
      ...
      if (!user || !user.pl || !user.pl.length) { nextUser() } else { nextPlaylist() }
    })
  }
}
...
function countDbUsersAndPlaylists (cb) {
  ...
  mongodb.collections[indexCol['user']].find({}, {fields: {pl: 1}}, function (err, cursor) {
    (function nextUser () {
      cursor.nextObject(function (err, user) {
        if (!user) { cb(result) } else {
          ++result.dbUsers
