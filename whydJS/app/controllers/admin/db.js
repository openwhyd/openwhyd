...
function listMissingUsers (uids, cb) {
  var users = []
  mongodb.forEach('user', {q: {_id: {$nin: uids}}, fields: {_id: 1, name: 1}}, function (user) {
    users.push(user)
  }, cb, users)
}
...
var fileGenerators = {
  ...
  'find.json': function (p, cb) {
    ...
    delete p.col
    if (!col) { cb({error: 'invalid col parameter'}) } else {
      ...
      col.find(p, {limit: limit}, function (error, cursor) {
        if (error) { cb({error: error}) } else {
          cursor.toArray(function (error, items) {
            if (error) { cb({error: error}) } else { cb(items) }
          })
        }
      })
    }
