exports.buildController = function (params) {
  return function (request, reqParams, response) {
    ...
    function render (res) {
      if (!res) { response.badRequest() } else if (res.tsv) { response.render(res.tsv, null, {'content-type': 'text/tsv'}) } else if (res.csv) { response.render(res.csv, null, {'content-type': 'text/csv'}) } else if (res.html) { response.renderHTML(res.html) } else { response.render(res) }
    }
