...
exports.controller = function(request, reqParams, response)
{
	...
	try {
		if (!templateFile)
			notifTemplates.generateRegWelcomeAsync(user, {name:"coco", id:"7"}, function(email){
				response.renderHTML(email.bodyHtml);
			});
		else
			templateLoader.loadTemplate("app/emails/"+templateFile+".html", function(templateHtml) {
