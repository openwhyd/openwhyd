...
exports.controller = function (request, getParams, response) {
  ...
  function render (r) {
    ...
    if (r.redirect) { response.redirect(r.redirect) } else if (r.html) { response.renderHTML(r.html) } else { response.renderJSON(r) }
  ...
  if (isPost) {
    ...
    function onDone (err, user) {
      if (user && user.consent) { console.log('user id', p.loggedUser.id, 'consented to gdpr notice =>', user.consent) }
