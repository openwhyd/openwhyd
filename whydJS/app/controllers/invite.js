...
var submitInvites = function(request, reqParams, response) {
	...
	if (!loggedUser || !reqParams)
		response.badRequest();
	else if (reqParams.email && reqParams.email.join && reqParams.email.length) { // === invite by email
		var successEmails = [];
...
exports.controller = function(request, reqParams, response, error) {
	...
	else if (request.method.toLowerCase() === 'delete' && reqParams.inviteCode && request.checkLogin())
	...
	else if (reqParams.inviteCode || (/*config.landingStream &&*/ request.url.startsWith("/signup")))
		exports.renderRegisterPage(request, reqParams, response, error);
	else
		renderInviteForm(request, reqParams, response);
}
