...
(function loadTemplates (callback) {
  template = templateLoader.loadTemplate('app/templates/hotTracks.html', callback)
})()
...
exports.controller = function (request, reqParams, response) {
  ...
  reqParams.limit = (reqParams.limit ? parseInt(reqParams.limit) : 0) || config.nbPostsPerNewsfeedPage
  ...
  function renderHotTracks (posts) {
    ...
    if (loggedInUser.id) {
      for (var i in posts) { posts[i].isLoved = snip.arrayHas(posts[i].lov, '' + loggedInUser.id) }
    }
    if (reqParams.format == 'json') {
      ...
      postsTemplate.renderPostsAsync(posts, {loggedUser: loggedInUser}, function (postsHtml) {
        var pageVars = {
