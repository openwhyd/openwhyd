...
exports.TestLogger = function () {
  ...
  this.log = function () {
    for (var i in arguments) {
      if (arguments[i] instanceof Object || arguments[i] instanceof Array) { arguments[i] = util.inspect(arguments[i]) }
    }
...
exports.ServerTestRunner = function () {
  ...
  function makeTestSet (tests) {
    return function (p, ee, cb) {
      ...
      Object.keys(tests).map(function (testId) {
        ...
        if (typeof testFct !== 'function') { console.log(testId + ' is not a test => skipping') } else {
  ...
  this.run = function (testId, p, render) {
    ...
    if (!test) {
      return render(null, Object.keys(tests).map(function (testId) {
        return '/test/' + testId
      }).join('\n'))
    }
