...
exports.forEachFileLine = function(fileName, lineHandler) {
	...
	fs.createReadStream(fileName).addListener('data', function(data){
...
var regexUrl = /(\b(https?|ftp|file):\/\/([-A-Z0-9+&@#$*'()%?=~_|!:,.;]*)[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;
...
var timeScales = [{"minute(s)":60},{"hour(s)":60},{"day(s)":24},{"month(s)":30},{"year(s)":12}];

exports.renderTimestamp = function(timestamp) {
	var t = (timestamp / 1000), lastScale = "second(s)";
	for (i in timeScales)
	{
		var scaleTitle
...
exports.MONTHS_SHORT = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

exports.renderShortMonthYear = function(t) {
	var t = new Date(t);
	var sameYear = false; //(new Date()).getFullYear() == t.getFullYear();
	return exports.MONTHS_SHORT[t.getMonth()] + (sameYear ? "" : " " + t.getFullYear());
}

exports.padNumber = function(str, n) {
	var ret = ""+str;
	while (ret.length < n) // pad with leading zeroes
		ret = '0' + ret;
    return ret;
}
...
exports.renderJsCallback = function(fctName, obj) {
	return exports.sanitizeJsIdentifier(fctName) + "("
...
exports.normalizeArtistName = function(artistName) {
	return exports.removeAccents(artistName.trim().toLowerCase())
...
exports.values = function(set) {
	var list = [];
	for (var i in set)
		if (set[i]) // TODO: remove this line
			list.push(set[i]);
	return list;
}
...
exports.arrayToSet = function(array, value) {
	var set = {};
	for (var i in array)
		if (array[i]) // TODO: remove this line
...
exports.removeDuplicates = function(array, keyFieldName) {
	if (keyFieldName)
		return exports.mapToObjArray(exports.objArrayToSet(array, keyFieldName), keyFieldName);
	else
		return Object.keys(exports.arrayToSet(array));
}

exports.objArrayToValueArray = function(array, attr) {
	var list = [];
	for (var i in array)
		if (array[i] && array[i][attr]) // TODO: fix this line, cf line 217
...
exports.getMapFieldNames = function(map /*, firstField*/) {
	var fieldNames = [], fieldSet = {};
	for (var i in map) {
		for (var f in map[i])
			if (!fieldSet[f]) { // TODO: check this line
				fieldNames.push(f);
				fieldSet[f] = true;
			}
	}
...
exports.excludeKeys = function(map, keySet) {
	var map = map || [], keySet = keySet || {}, res = [];
	for (var k in map)
		if (!keySet[k]) // TODO: check this line
			res[k] = map[k];
	return res;
}
...
exports.checkMistypedFields = function(obj, fieldTypeSet) {
	...
	function Error(error, f){
		return {field: f, type: typeof obj[f], expected: fieldTypeSet[f], error: error + ": " + f};
	}
	for (var f in fieldTypeSet)
		if (fieldTypeSet[f]) {
			if (!obj.hasOwnProperty(f))
				return Error("missing field", f);
			else if (typeof obj[f] !== fieldTypeSet[f] && (fieldTypeSet[f] !== "array" || !obj[f].splice))
				return Error("mistyped field", f);
		}
		else if (obj.hasOwnProperty(f))
			return Error("forbidden field", f);
}
...
exports.DataTable = function() {
	...
	self.fromMap = function(map, header) {
		...
		for (var i in map) {
			...
			for (var f in self.header)
				line.push(map[i][self.header[f]]);
	...
	self.toHtml = function() {
		...
		return [ "<!DOCTYPE html>", "<html>", "<head>", "<meta charset=\"utf-8\"/>", "</head>",
			"<body>", "<p>" + this.table.length + " items</p>", "<table>",
			"<thead style='background:#ccc;'>", table.shift(), "</thead>",
			"<tbody>" ].concat(table).concat([ "</tbody", "</table>", "</body>", "</html>" ]).join("\n");
	}
...
exports.httpSetDomain = function(regex, options){
	if (!options)
		delete httpDomains[regex];
	else
		httpDomains[regex] = [regex, options];
}
...
exports.httpGetDomain = function(domain){
	for (var i in httpDomains)
		if (httpDomains[i][0].test(domain))
			return httpDomains[i][1];
}

function _httpRequest(options, callback){
	var data = "", body = options.body;
	if (body) // => don't forget to add Content-Type and Content-Length headers in that case
		delete options.body;
...
exports.httpRequest = function (url, options, callback) {
	var urlObj = urlModule.parse(url), options = options || {};
	...
	if (domainOpts.queue){
		...
		domainOpts.queue.push(_httpRequest.bind(null, options, function(){
...
exports.checkParams = function(obj, mandatorySet, optionalSet){
	...
	obj = obj || {};
	for (var fieldName in mandatorySet)
		storeIfValid(fieldName, mandatorySet);
...
exports.Worker = function(options){
	options = options || {};
	var jobs = 0, interval = null;
	function addJob(job){
		console.log("added job: " + job);
		++jobs;
		if (!interval)
			interval = setInterval(function(){console.log("("+jobs+" jobs)");}, options.statusDisplayInterval || 5000);
	}
		...
		this.wrapCallback = function(callback){
			return function(){
				if (self && self.done) {
					...
					console.log("warning: intercepted callback from terminated job " + self);
			};
		}
		this.timeout = setTimeout(function(){
			console.log("job " + self + " was still running 1 minute after launch => destroying");
			console.warn("destroyed a job that was still running 1 minute after launch");
			removeJob(self);
