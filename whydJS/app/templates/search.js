...
var makeLink = {
	"track": function(item) { return "/c/" + item._id; }, // in algolia index only
	"post": function(item) { return "/c/" + item._id; },
	"user": function(item) { return "/u/" + item._id; },
	"playlist": function(item) { return "/u/" + item._id.replace("_", "/playlist/"); }
};

var makeImg = {
	"track": function(item) { return item.img; }, // in algolia index only
	"post": function(item) { return "/img/post/" + item._id; },
	"user": function(item) { return "/img/u/" + item._id; },
	"playlist": function(item) { return "/img/playlist/" + item._id; }
};
...
exports.renderPosts = function(posts, loggedUser, cb) {
	postsTemplate.renderPostsAsync(posts, {
...
exports.renderSearchPage = function(results, reqParams, cb) {
	...
	if (results) {
		...
		if (results.posts)
			exports.renderPosts(results.posts, reqParams.loggedUser, function(postsHtml){
