...
exports.refreshTemplates = function (callback) {
  pageTemplate = templateLoader.loadTemplate('app/templates/invitePage.html', callback)
}
...
exports.renderSignupPage = function (p) {
  ...
  if (p.redirect && p.redirect[0] == '/') { params.redirect = config.urlPrefix + p.redirect }

  return pageTemplate.render(params)
}

exports.renderInvitePage = function (sender, loggedUser, inviteCode, iPo, email, fbRequest, plC, redirect) {
  var params = {}
  ...
  if (redirect) { params.redirect = (redirect[0] == '/' ? config.urlPrefix : '') + redirect }
