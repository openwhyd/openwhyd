...
var FEEDBACK_FOOTER = "P.S  : We'd love your feedback! We're all ears at " + FEEDBACK_EMAIL + ", and you can follow @whyd on twitter.";
var FEEDBACK_FOOTER_HTML = 'P.S  : We\'d love your feedback! We\'re all ears at <a href="mailto:' + FEEDBACK_EMAIL + '" style="color:#3177AF;text-decoration:underline;">' + FEEDBACK_EMAIL + '</a>, and you can follow <a href="http://twitter.com/whyd" style="color:#3177AF;text-decoration:underline;">@whyd</a> on twitter.';

var MAX_HOT_TRACKS = 3, MAX_RECOM_USERS = 4, MAX_BIO_LENGTH = 72; //  for RegWelcomeAsync
...
var renderTemplateFile = (function (){
	for (var i in TEMPLATES)
		TEMPLATES[i].template = templateLoader.loadTemplate(TEMPLATE_PATH + TEMPLATES[i].file);
	return function(templateName, p){
		var p = p || {};
		for(var i in TEMPLATE_DEFAULTS)
			if (p[i] == undefined)
				p[i] = TEMPLATE_DEFAULTS[i];
...
exports.generateWaitingList = function () { // called from models/notifEmails.js wrappers
	...
	return {
		...
		bodyText: [
			...
			'In the meantime, follow us on Twitter (http://twitter.com/whyd) '
			+ 'and like us on Facebook (http://facebook.com/usewhyd) so you can read interviews '
			+ 'with music curators, listen to cool playlists, and travel to far away lands.',
			"Never stop jamming - the Whyd team",
			urlPrefix
		].join("\n\n"),
		bodyHtml: [
			'<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">',
			...
			'</head>',
			'<body>',
				'<p>Hey! Thank you for your interest in <a href="http://whyd.com/">Whyd</a>!',
				'	Our community of music lovers is growing by the minute. ',
				...
				'<p>In the meantime, follow us on <a href="http://twitter.com/whyd">Twitter</a>',
				'	and like us on <a href="http://facebook.com/usewhyd">Facebook</a>',
				'	so you can read interviews with music curators, listen to cool playlists, and travel',
				'	to far away lands.</p>',
				'<p>Never stop jamming - the Whyd team</p>',
			'</body>',
			'</html>'
		].join("\n")
	} 
...
exports.generateAcceptedInvite = function(user) { // called from controllers/admin/invites.js
	...
	return renderEmailTemplate("Your invitation to Whyd", [
		...
		"the Whyd team",
		//FEEDBACK_FOOTER
		"PS: We'd love your feedback! In fact we need it to continue to develop Whyd to be the simplest and most enjoyable way for music lovers to access the wealth of music available online. We're all ears (or, eyes, actually) at " + FEEDBACK_EMAIL,
	]);
}
...
exports.generateRegWelcome = function (user, inviteSender) {
	return renderEmailTemplate("Welcome to Whyd, "+user.name+"!", [
		...
		"- Install the " + renderLink("bookmarklet/extension", "/button") + " so you can add great music directly to your profile from anywhere on the web;",
		"- Make your profile beautiful by customizing your cover photo and playlist covers, you can even make a special URL like " + renderLink("whyd.com/tony", "/tony") + ", that way it's easier for people to remember. Plus, anyone can listen to your profile, they don't even have to be Whyd members!",
		"- " + renderLink("Invite your friends", "/invite") + " and people you would like to share music with, after all it's a social network for music lovers! People you invite are automatically connected to you.",
		(inviteSender ? "By the way, take a look at the stream of your friend " + renderLink(inviteSender.name, "/u/" + (inviteSender._id || inviteSender.id)) : ""),
		"Never stop jamming!",
		" - Tony",
		//FEEDBACK_FOOTER
		...
		"Follow us " + renderLink("@whyd on Twitter", "http://twitter.com/whyd") + " and definitely Like " + renderLink("our Facebook page!", "http://facebook.com/usewhyd")
	]);
};
...
exports.generateRegWelcomeAsync = function (user, inviteSender, cb) {
	(function (cb) {
		userModel.fetchByUid(user.id, function(user){
			...
			trackModel.fetchPostsByGenres(genres, {limit: MAX_HOT_TRACKS}, function(hotPosts) {
				plTagsModel.getTagEngine(function(tagEngine){
					var hotUsers = tagEngine.getUsersByTags(genres).slice(0, MAX_RECOM_USERS);
					if (inviteSender)
						hotUsers.unshift(inviteSender);
					userModel.fetchUserBios(hotUsers, function(){
						for (var i in hotUsers)
							if (((hotUsers[i] || {}).bio || "").length > MAX_BIO_LENGTH)
								hotUsers[i].bio = hotUsers[i].bio.substr(0, MAX_BIO_LENGTH) + "...";
...
exports.generateInviteBy = function (senderName, inviteId, message) {
	...
	return {
		...
		bodyText: (message ? message + "\n\n--\n\n" : "") + [
			...
			"Whyd is the place to keep, play, and share all of the internet's music from sites like Soundcloud, Vimeo, and YouTube.",
			"Whyd is invite only, so here is your personal invite: " + inviteUrl,
			"Never stop jamming,",
 			senderName
			//FEEDBACK_FOOTER
		].join("\n\n")
...
exports.generateInviteAccepted = function (user) {
	return {
		...
		bodyText: "Great news! Your friend "+user.name+" just accepted your invitation to whyd!\n\n"
			...
			+ "Thanks for adding interesting people to the whyd community!\n\n"
			+ "We look forward to listening to your next amazing tracks on whyd: "+urlPrefix+"\n\n"
			+ "Greetings from the whyd team!\n\n"
			+ FEEDBACK_FOOTER
	}
};
...
exports.generatePasswordReset = function (user, options) {
	options = options || {};
	var href = urlPrefix + "/password?uid=" + user.id + "&resetCode=" + options.resetCode;
	if (options.redirect)
		href += "&redirect=" + encodeURIComponent(options.redirect);
	return {
		subject: "whyd password",
		bodyText: "If you want to set a new password to your whyd account, open this link:\n\n"
			+ href + "\n\n"
			+ "Greetings from the whyd team!\n\n"
			+ FEEDBACK_FOOTER
	}
...
exports.generatePasswordUpdated = function (user) {
	return {
		subject: "your whyd password was successfully updated!",
...
exports.generateEmailUpdated = function (user) {
	return {
		subject: "your whyd email address was successfully updated!",
		bodyText: [
...
exports.generateNotifDigest = function (p) { // {user, subscriptions, repostedTrackSet, likersPerPost, digestFrequency}
	p = p || {};
	p.notifType = "digest";
	return (new NotifDigest(p)).renderNotifEmailObj("Your " + p.digestFrequency + " digest");
};
...
exports.generateComment = function (post, comment) {
	return renderEmailTemplate(comment.uNm + " commented on your track", [
		...
		renderTemplateFile("comment", {
			unsubUrl: config.urlPrefix + "/api/unsubscribe?type=emCom&uId=" + post.uId,
...
exports.generateMention = function (mentionedUid, post, comment) {
	return renderEmailTemplate(comment.uNm + " mentioned you on a track", [
		...
		renderTemplateFile("mention", {
			unsubUrl: config.urlPrefix + "/api/unsubscribe?type=emMen&uId=" + mentionedUid,
			user: mongodb.getUserFromId(""+comment.uId),
			track: post
		})
	]);
...
exports.generateCommentReply = function (post, comment, repliedUid) {
	return renderEmailTemplate(comment.uNm + " replied to one of your comments", [
		...
		renderTemplateFile("commentReply", {
			unsubUrl: config.urlPrefix + "/api/unsubscribe?type=emRep&uId=" + repliedUid,
			user: mongodb.getUserFromId(""+comment.uId),
			track: post
		})
	]);
...
exports.generateFbFriendJoinedWhyd = function(newUser) {
	return {
		...
		bodyText: [
			"Hooray!",
			"Your friend " + newUser.name + " just joined whyd !",
