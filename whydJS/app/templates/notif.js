...
var FEEDBACK_FOOTER = "P.S  : We'd love your feedback! We're all ears at " + FEEDBACK_EMAIL + ', and you can follow @open_whyd on twitter.'
var FEEDBACK_FOOTER_HTML = 'P.S  : We\'d love your feedback! We\'re all ears at <a href="mailto:' + FEEDBACK_EMAIL + '" style="color:#3177AF;text-decoration:underline;">' + FEEDBACK_EMAIL + '</a>, and you can follow <a href="http://twitter.com/open_whyd" style="color:#3177AF;text-decoration:underline;">@open_whyd</a> on twitter.'
...
var renderTemplateFile = (function () {
  for (var i in TEMPLATES) { TEMPLATES[i].template = templateLoader.loadTemplate(TEMPLATE_PATH + TEMPLATES[i].file) }
...
exports.generateRegWelcome = function (user, inviteSender) {
  return renderEmailTemplate('Welcome to Openwhyd, ' + user.name + '!', [
    ...
    '- Make your profile beautiful by customizing your cover photo and playlist covers, you can even make a special URL like ' + renderLink('openwhyd.org/tony', '/tony') + ", that way it's easier for people to remember. Plus, anyone can listen to your profile, they don't even have to be Openwhyd members!",
    ...
    'Follow us ' + renderLink('@open_whyd on Twitter', 'http://twitter.com/open_whyd') + ' and definitely Like ' + renderLink('our Facebook page!', 'http://facebook.com/openwhyd')
  ])
}
        ...
        plTagsModel.getTagEngine(function (tagEngine) {
          ...
          userModel.fetchUserBios(hotUsers, function () {
            for (var i in hotUsers) {
              if (((hotUsers[i] || {}).bio || '').length > MAX_BIO_LENGTH) { hotUsers[i].bio = hotUsers[i].bio.substr(0, MAX_BIO_LENGTH) + '...' }
            }
...
exports.generatePasswordReset = function (user, options) {
  ...
  if (options.redirect) { href += '&redirect=' + encodeURIComponent(options.redirect) }
  return {
