...
function makeMetaHead (options) {
  ...
  if (options.pageTitle) { meta.push('<meta property="og:title" content="' + uiSnippets.htmlEntities(options.ogTitle || options.pageTitle) + '" />') }
  if (options.pageUrl) { meta.push('<meta property="og:url" content="' + uiSnippets.htmlEntities(options.pageUrl) + '" />') }
  return meta
}
...
exports.renderWhydFrame = function (html, params) {
  ...
  if (params.pageUrl && params.pageUrl.indexOf('/') == 0) { params.pageUrl = config.urlPrefix + params.pageUrl }
  ...
  if (params.title) { out.push('    <title>' + uiSnippets.htmlEntities(params.title) + '</title>') }

  for (var i in params.css) { out.push('    <link href="' + render.urlPrefix + '/css/' + params.css[i] + includeSuffix + '" rel="stylesheet" type="text/css" />') }
  ...
  if (!params.loggedUser || !params.loggedUser.id) { params.bodyClass = (params.bodyClass || '') + ' visitor' }
