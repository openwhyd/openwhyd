...
function prepareFeedVars(posts, options) {
	...
	if (!options.before)
		feedVars.hasMore = options.hasMore; // prevent from showing the button when a newly added track is requested
	...
	if (options.playlist) {
		...
		if (feedVars.playlist.name)
			feedVars.playlist._js_name = snip.sanitizeJsStringInHtml(feedVars.playlist.name);
	...
	if (options.user && options.user.id) {
		...
		if (feedVars.user.bio)
			feedVars.user.renderedBio = snip.replaceURLWithFullHTMLLinks(snip.htmlEntities(feedVars.user.bio)).replace(/\n\n/g, "\n").replace(/\n/g, "<br/>");
...
exports.renderFeedAsync = function (posts, options, callback) {
	postsTemplate.renderPostsAsync(posts, options, function (postsHtml) {
		...
		if (format == "json")
			...
			callback((options.customFeedTemplate || templates[format] || templates["html"]).render(feedVars));
