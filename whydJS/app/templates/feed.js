...
function loadTemplates (callback) {
  templates['html'] = templateLoader.loadTemplate('app/templates/feed.html', function () {
    templates['embed'] = templateLoader.loadTemplate('app/templates/feedEmbed.html', function () {
      templates['embedv2'] = templateLoader.loadTemplate('app/templates/feedEmbedV2.html', function () {
...
function prepareFeedVars (posts, options) {
  options.bodyClass = (options.bodyClass || '') + (options.ownProfile ? ' ownProfile' : '')
  ...
  var feedVars = {
    ...
    emptyFeed: posts.length == 0 && !options.before ? { ownProfile: options.ownProfile } : null,
    ...
    inviteAdEmailPlaceholder: FUNNY_EMAILS[Math.floor(FUNNY_EMAILS.length * Math.random())]
  ...
  if (posts.length && !options.after) {
    feedVars.hasLess = {
      firstPid: (options.playlist && posts[0].order != null ? parseInt(posts[0].order) : 0) || posts[0]._id
    }
  }

  if (options.playlist) {
    feedVars.playlist = options.playlist || {}
    if (feedVars.playlist.name) { feedVars.playlist._js_name = snip.sanitizeJsStringInHtml(feedVars.playlist.name) }
    feedVars.playlist.url = config.urlPrefix + '/u/' + options.user.id + '/playlist/' + feedVars.playlist.id
    feedVars.playlist.urlEncoded = encodeURIComponent(feedVars.playlist.url)
    feedVars.playlist.urlEncodedTweet = encodeURIComponent('♫ ' + feedVars.playlist.name /* + ' ' + feedVars.playlist.url */)
    feedVars.playlist.nbTracks = feedVars.playlist.nbTracks || (posts ? posts.length : 'no')
    feedVars.trackOrderUrl = '/u/' + options.user.id + '/playlist/' + feedVars.playlist.id + '/edit'
  ...
  if (options.user && options.user.id) {
    ...
    if (feedVars.user.bio) { feedVars.user.renderedBio = snip.replaceURLWithFullHTMLLinks(snip.htmlEntities(feedVars.user.bio)).replace(/\n\n/g, '\n').replace(/\n/g, '<br/>') }
    if (feedVars.user.name) { feedVars.user._js_name = snip.sanitizeJsStringInHtml(feedVars.user.name) }
  }

  if (options.embedW) {
    feedVars.firstTrack = posts && posts.length > 0 ? posts[0] : {
      id: '',
      img: '',
      url: 'javascript:{}'
    }
    if (!feedVars.firstTrack.img) { feedVars.firstTrack.img = '/images/cover-track.png' }
    options.title = (options.playlist || {}).name + ' by ' + (options.user || {}).name + ' - whyd'
  }
...
exports.renderFeedAsync = function (posts, options, callback) {
  postsTemplate.renderPostsAsync(posts, options, function (postsHtml) {
    ...
    var format = options.format ? options.format.toLowerCase() : (options.embedW ? 'embed' : 'html')
    // var t = (options.embedW ? templateEmbed[options.format || "oldEmbed"] : null) || template;
    if (format == 'json') { callback(postsHtml) } else { callback((options.customFeedTemplate || templates[format] || templates['html']).render(feedVars)) }
...
exports.renderFeedEmbed = function (feedHtml, options) {
  ...
  options.css = options.css || []
  options.nocss = true
  options.css.push((options.format || '').toLowerCase() == 'embedv2' ? 'feedEmbedV2.css' : 'feedEmbed.css')
