...
var i18n = {
	"EN": {
		...
		c5: [
			{h: "You", ul: [
				{li: "You create a playlist contest with the name of your site/brand/project/event, giving people a starting point to build a playlist, and choose a date that you’d like the contest to end (optimal time is two weeks)."},
				{li: "You copy and paste the rules into a post on your site, or other announcements via social media."},
				{li: "You support your audience by getting them excited to participate!"},
			...
			{h: "Whyd", ul: [
				{li: "Whyd identifies the playlist and adds the necessary features to enable clear voting."},
				{li: "Whyd will feature the contest and push it out to all users across the site, plus provide support on social media and individual invitations to key users."}
			]}
		],
	...
	"FR": {
		...
		c5: [
			{h: "Vous", ul: [
				{li: "Vous créez un concours de playlist avec le nom de votre site/marque/projet/évènement pour donner aux gens un point de départ pour construire une playlist. Puis, vous sélectionnez une date de fin de concours (le période optimale est deux semaines)"},
				{li: "Copiez et collez les régles dans un article/post sur votre site et media sociaux."},
			...
			{h: "Whyd", ul: [
				{li: "Whyd identifie les playlists officielles et ajoute les boutons de vote"},
				{li: "Whyd présente le concours, le partage sur les media sociaux, diffuse sur la home du site, et envoie des invitations spéciales à ses membres clés."}
			]}
		],
...
for (var i in defaultI18n)
	i18n["FR"][i] = i18n["FR"][i] || defaultI18n[i];
...
exports.renderInfoPage = function(plC) {
	var safe = {};
	for (var i in plC)
		safe[i] = uiSnippets.htmlEntities(plC[i]);
	return [
...
exports.renderListPage = function(list) {
	...
	for (var i in list) {
		...
		for (var j in list[i].playlists) {
			...
			plHtml += '<li><a href="/u/' + plId[0] + '/playlist/' + plId[1] + '">'
					+ uiSnippets.htmlEntities((mongodb.usernames[plId[0]] || {}).name) + '</a></li>';
		}
		html.push('<li>'
			+ '<a href="'+config.urlPrefix+'/playlistcontest/'+list[i].uri+'/info">'
			+ uiSnippets.htmlEntities(list[i].title) + '</a>'
			+ ', ' + (list[i].playlists || []).length + ' candidates:'
			+ '<ul>' + plHtml + '</ul></li>');
	}
