...
function cleanInfoArray(info) {
	...
	for (var i in info)
		if (info[i])
			result.push(/*uiSnippets.htmlEntities*/(info[i]));
	return result;
}
...
exports.AdminLists = function() {
	...
	var renderList = function (items, title, actionNames, formParams) {
		...
		for (var i in items) {
			...
			html.push('<p>'
				+ (u.img ? '<img src="'+uiSnippets.htmlEntities(u.img)+'" />' : '')
				+ "<div>"
				+ (actionNames ? '<input type="checkbox" name="_id" value="' + (u._id || u.id) + '" />' : '')
				+ (u.href ? '<a href="' + u.href + '">' : '')
					+ '<span class="itemName">' + uiSnippets.htmlEntities(u.name) + '</span>'
				+ (u.href ? '</a>' : '')
				+ (u.nameSuffix ? "&nbsp;" + u.nameSuffix : '')
				+ (info ? '<br/>&nbsp;<small>' + /*u.*/info.join("<br/>&nbsp;") + '</small>': '')
				+ "</div>"
				+ '</p>');
		}
		html = html.join('\n');
		...
		if (actionNames) {
			if (formParams)
				for(var i in formParams)
					if (formParams[i]) // prevent null values
						html += '<input type="hidden" name="'+i+'" value="' + formParams[i] + '" />';
			var buttons = '';
			for(var i in actionNames)
				buttons += '<input type="submit" name="action" value="' + actionNames[i] + '"'
						+ ' onclick="return confirm(\'Your are going to ' + actionNames[i] + ' the following items(s):\\n\' + getSelectedCheckbox(form._id) + \'\\nAre you sure?\')" />';

			html =  '<form method="post">' + buttons + html + buttons + '</form>';
		}

		return '<div class="userList">' + (title ? '<h2>'+title+'</h2>' : '') + html + '</div>';
	}
	...
	var renderWideList = function (items, title, actionNames, formParams) {
		...
		return '<div class="wideList">' + (title ? '<h2>'+title+'</h2>' : '') + html + '</div>';
	}
	...
	return {
		...
		renderPage: function (params) {
			...
			if (params.css)
				params.css.unshift("admin.css")
			else
				params.css = ["admin.css"];
