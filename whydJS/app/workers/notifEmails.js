...
function oneAtATime(fct, msg) {
	...
	return function() {
		if (running)
			console.error(msg || "WARNING: [OneAtATime] fct is trying to start before last call has ended");
		else {
			running = true;
			fct(function() {
				running = false;
...
if (DIGEST_INTERVAL < 0)
	console.log("[DIGEST WORKER] config.digestInterval is NULL or NEGATIVE => digest worker is disabled");
else if (!timer) {
