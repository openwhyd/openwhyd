...
var DAY = 0, HOUR = 8, MIN = 0 // activate every sunday at 8am

function check () {
  var now = new Date()
  if (now.getDay() == DAY && now.getHours() == HOUR && now.getMinutes() == MIN) {
    console.log('[HOT SNAPSHOT WORKER] ===', now, '=> refreshing hot track trends...')
    trackModel.snapshotTrackScores(function (r) {
      console.log('[HOT SNAPSHOT WORKER] snapshotTrackScores => ', r || {ok: 'done'})
    })
  }
};
