...
(function parseHandlesFromRouteFile(routeFile){
	...
	snip.forEachFileLine(routeFile, function(line){
		if (typeof line != "string")
			return console.log("=> Parsed", nb, "handles from:", routeFile /*, " : ", USERNAME_RESERVED*/);
...
(function parseHandlesFromTextFile(fileName){
	...
	snip.forEachFileLine(fileName, function(line){
		if (typeof line != "string")
			return console.log("=> Parsed", nb, "handles from:", fileName /*, " : ", USERNAME_RESERVED*/);
...
function processUserPref(user) {
	...
	for (var i in defaultPref)
		user.pref[i] = user.pref[i] === undefined || user.pref[i] === null ? defaultPref[i] : // default is better than null/undefined value
			(typeof defaultPref[i] == "boolean" ? !!user.pref[i] : user.pref[i]); // type existing values accordingly to defaults
	return user;
}
...
exports.save = function(user, handler) {
	...
	delete user._id;
	if (user.name)
		user.n = exports.normalizeName(user.name);
	...
	if (criteria._id) {
		...
		fetch(criteria, function(err, user) {
			if (err)
				console.error(err);
			if (!user)
				return handler && handler({error: "user not found"});
			// delete playlists from index
			if (user.pl)
				(function next() {
					if (!user.pl.length)
						console.log("done deleting user playlists");
					else {
						var pl = (user.pl.pop() || {});
			...
			mongodb.collections["user"].remove(criteria, function (err, item) {
				if (err)
					...
					console.log("removed users", criteria);
...
exports.getEmailNotifsFreq = function(user) {
	...
	for (var i in user.pref)
		if (i.indexOf("em") == 0)
			freq = Math.max(freq, user.pref[i]);
	return freq;
}
...
exports.setPref = function(uId, pref, handler) {
	...
	if (!pref)
		handler({error:"Invalid preferences"});
	else {
		...
		if (emailFreq > 0) {
			...
			exports.update(uId, {$set:cleanPref, $unset:{"pref.prevEN":1, "pref.nextEN":1}}, handler);
	}
}
	...
	exports.fetchByFbUid(fbId, function(user){
		if (user) {
			if (user._id != uId) {
				...
				cb && cb({error: "This Facebook account is already associated to this user"});
		}
		else {
...
exports.setTwitterId = function(uId, twId, twTok, twSec, cb) {
	...
	if (!uId)
	...
	else if (!twId) // disconnect twitter account
		...
		fetch({twId:twId}, function(err, user){
			if (user && user._id != uId)
				cb && cb({error: "This Twitter account is already associated to another user"});
			else
...
exports.setHandle = function(uId, username, handler) {
	...
	if (!username)
		return res({error:"You must enter a username"});
	var username = (""+username).trim().toLowerCase();
	if (username.length < USERNAME_MIN_LENGTH)
		return res({error:"Must be at least " + USERNAME_MIN_LENGTH + " characters long"});
	if (username.length > USERNAME_MAX_LENGTH)
		return res({error:"Must be less than " + USERNAME_MAX_LENGTH + " characters long"});
	if (!USERNAME_REGEX.test(username))
		return res({error:"Special characters are not allowed"});
	if (!isNaN(username))
		return res({error:"Numbers are not allowed"});
		...
		exports.fetchMulti({_id:{$in:uidList}}, {fields:attrSet}, function(userList) {
			...
			if (userList)
				for (var i in userList)
					uidSet[""+userList[i]._id] = userList[i];
			for (var i in subList)
