...
var USERNAME_RESERVED = { // blacklist of user urls
  'index.html': true,
  'null': true,
  'undefined': true,
  'whyd': true,
...
(function parseHandlesFromRouteFile (routeFile) {
  ...
  snip.forEachFileLine(routeFile, function (line) {
    if (typeof line !== 'string') { return console.log('=> Parsed', nb, 'handles from:', routeFile /*, " : ", USERNAME_RESERVED */) }
...
(function parseHandlesFromTextFile (fileName) {
  ...
  snip.forEachFileLine(fileName, function (line) {
    if (typeof line !== 'string') { return console.log('=> Parsed', nb, 'handles from:', fileName /*, " : ", USERNAME_RESERVED */) }
...
exports.normalizeName = function (origName) {
  return origName.toLowerCase().replace(/[àáâãäåāąă]/g, 'a').replace(/[çćčĉċ]/g, 'c').replace(/[ďđð]/g, 'd').replace(/[èéêëēęěĕė]/g, 'e').replace(/[ƒſ]/g, 'f').replace(/[ĝğġģ]/g, 'g').replace(/[ĥħ]/g, 'h').replace(/[ìíîïīĩĭįı]/g, 'i').replace(/[ĳĵ]/g, 'j').replace(/[ķĸ]/g, 'k').replace(/[łľĺļŀ]/g, 'l').replace(/[ñńňņŉŋ]/g, 'n').replace(/[òóôõöøōőŏœ]/g, 'o').replace(/[Þþ]/g, 'p').replace(/[ŕřŗ]/g, 'r').replace(/[śšşŝș]/g, 's').replace(/[ťţŧț]/g, 't').replace(/[ùúûüūůűŭũų]/g, 'u').replace(/[ŵ]/g, 'w').replace(/[ýÿŷ]/g, 'y').replace(/[žżź]/g, 'z').replace(/[æ]/g, 'ae').replace(/[ÀÁÂÃÄÅĀĄĂ]/g, 'A').replace(/[ÇĆČĈĊ]/g, 'C').replace(/[ĎĐÐ]/g, 'D').replace(/[ÈÉÊËĒĘĚĔĖ]/g, 'E').replace(/[ĜĞĠĢ]/g, 'G').replace(/[ĤĦ]/g, 'H').replace(/[ÌÍÎÏĪĨĬĮİ]/g, 'I').replace(/[Ĵ]/g, 'J').replace(/[Ķ]/g, 'K').replace(/[ŁĽĹĻĿ]/g, 'L').replace(/[ÑŃŇŅŊ]/g, 'N').replace(/[ÒÓÔÕÖØŌŐŎ]/g, 'O').replace(/[ŔŘŖ]/g, 'R').replace(/[ŚŠŞŜȘ]/g, 'S').replace(/[ÙÚÛÜŪŮŰŬŨŲ]/g, 'U').replace(/[Ŵ]/g, 'W').replace(/[ÝŶŸ]/g, 'Y').replace(/[ŹŽŻ]/g, 'Z').replace(/[ß]/g, 'ss').replace(/\W+/g, ' ')
}
...
function processUserPref (user) {
  ...
  for (var i in defaultPref) {
    user.pref[i] = user.pref[i] === undefined || user.pref[i] === null ? defaultPref[i] // default is better than null/undefined value
      : (typeof defaultPref[i] === 'boolean' ? !!user.pref[i] : user.pref[i])
  } // type existing values accordingly to defaults
  return user
}
...
function processUsers (list) {
  for (var i in list) {
    if (list[i]) { processUserPref(list[i]) }
  }
}
...
exports.fetchAll = function (handler) {
  mongodb.collections['user'].find({}, {sort: [['_id', 'desc']]}, function (err, cursor) {
...
exports.fetchMulti = function (q, options, handler) {
  if (q._id && typeof q._id === 'string') { q._id = ObjectId(q._id) }
  if (q._id && typeof q._id === 'object') // $in:[]
  {
    for (var i in q._id) {
      if (i == '$in') {
        for (var j in q._id['$in']) { q._id['$in'][j] = ObjectId('' + q._id['$in'][j]) }
      }
    }
  }
...
exports.fetchByUid =
exports.model = function (uid, handler) {
...
exports.updateAndFetch = function (criteria, update, opts, cb) {
  ...
  mongodb.collections['user'].update(criteria, /* {$set: */update/* } */, opts || {}, function (err, item) {
...
exports.update = function (uid, update, handler) {
  exports.updateAndFetch({'_id': ObjectId('' + uid)}, update, null, function (err, user) {
...
exports.save = function (user, handler) {
  ...
  var criteria = (uid ? {_id: typeof uid === 'string' ? ObjectId(uid) : uid}
    : {email: emailModel.normalize(user.email)})
  var user = user
  delete user._id
  if (user.name) { user.n = exports.normalizeName(user.name) }
  console.log('models.user.save : ', Object.keys(user), '=> criteria: ', criteria)
  mongodb.collections['user'].update(criteria, {$set: user}, {upsert: true}, function (err, item) {
...
exports.delete = function (criteria, handler) {
  ...
  if (criteria._id) {
    ...
    fetch(criteria, function (err, user) {
      ...
      if (user.pl) {
        (function next () {
          if (!user.pl.length) { console.log('done deleting user playlists') } else {
            var pl = (user.pl.pop() || {})
            console.log('deleting user playlists', user._id + '_' + pl.id, pl.name)
      ...
      mongodb.collections['user'].remove(criteria, function (err, item) {
        if (err) { console.error(err) } else { console.log('removed users', criteria) }
...
exports.fetchEmail = function (email, callback) {
  mongodb.collections['email'].findOne({_id: emailModel.normalize(email)}, callback)
}
...
exports.storeEmail = function (email) {
  mongodb.collections['email'].save({_id: emailModel.normalize(email), date: new Date()}, {w: 0})
}

// invites

exports.fetchInvite = function (inviteCode, handler) {
  mongodb.collections['invite'].findOne({_id: ObjectId(inviteCode)}, function (err, user) {
    handler(user)
  })
}

exports.fetchInviteByEmail = function (email, handler) {
  mongodb.collections['invite'].findOne({email: emailModel.normalize(email)}, function (err, user) {
    handler(user)
  })
}
...
function insertInvite (obj, handler) {
  ...
  } else if (obj.fbId) { criteria = { fbId: obj.fbId } } else {
    console.log('WARNING: email or fbId must be specified')
    return handler && handler()
  }

  mongodb.collections['invite'].update(criteria, {$set: obj}, {upsert: true, multi: true}, function (err, item) {
    if (err) console.log(err)
...
exports.inviteFbUserBy = function (fbId, senderId, handler) { // formerly inviteToJoinConversation()
...
exports.removeInviteByEmail = function (emailList, handler) {
  mongodb.collections['invite'].remove({email: {$in: emailList}}, function (err, item) {
...
exports.hasPlaylistName = function (user, name) {
  ...
  if (!user && user.id != null) { console.error('null user provided in user.hasPlaylist') } else if (user.pl && user.pl.length) {
    for (var i in user.pl) {
      if (user.pl[i].name == name) { return user.pl[i] }
    }
  }
  return false
}
    ...
    exports.save(user, function () {
      ...
      if (user.fbTok && user.pref && user.pref.ogCPl) {
        try {
          ...
          fbModel.graphApiRequest(user.fbTok, '/me/music.playlists', params, function (result) {
          // console.log("opengraph playlist =>", result);
            if (result && result.id) {
              exports.setPlaylist(uId, pl.id, {fbId: result.id}, function (r) {
  ...
  fetch({_id: '' + uId}, function (err, user) {
    ...
    if (found) {
      exports.save(user, function () {
        ...
        if (upd.name) {
          console.log('updating playlist name in index and corresponding tracks...')
...
exports.deletePlaylist = function (uId, plId, handler) {
  ...
  fetch({_id: uId}, function (err, user) {
    ...
    postModel.unsetPlaylist(uId, plId, function () {
      user.pl = newPl
      exports.save(user, function () {
        console.log('deleted playlist (and updated corresponding tracks):', plId)
...
exports.fetchEmailNotifsToSend = function (now, cb) {
  ...
  if (!TESTING_DIGEST) { criteria['pref.nextEN'] = {$lte: msToDigestTimestamp(now)} } // next email notif date
...
exports.incrementNotificationCounter = function (uId, handler) {
  ...
  mongodb.collections['user'].update({_id: ObjectId('' + uId)}, {$inc: {'pref.pendEN': 1}}, function (err, item) {
    if (err) console.error(err)
...
exports.setPref = function (uId, pref, handler) {
  ...
  if (!pref) { handler({error: 'Invalid preferences'}) } else {
    var cleanPref = {}, emailFreq = 0
    for (var i in defaultPref) {
      if (pref[i] !== undefined && pref[i] !== null) { // only clean provided pref values
        cleanPref['pref.' + i] = (typeof defaultPref[i] === 'boolean' ? !!pref[i] : pref[i]) // type each value accordingly to defaults
        if (i.indexOf('em') == 0) // enabled email notification => set next notification date
    ...
    if (emailFreq > 0) {
      ...
      cleanPref['pref.nextEN'] = msToDigestTimestamp(now + daysToMillisecs(emailFreq))
      exports.update(uId, {$set: cleanPref}, handler)
    } else { exports.update(uId, {$set: cleanPref, $unset: {'pref.prevEN': 1, 'pref.nextEN': 1}}, handler) }
  }
}
...
exports.setApTok = function (uId, apTok, cb) {
  ...
  if (apTok.length != 64) { return cb({error: 'invalid apple push notification device token'}) }
  ...
  mongodb.collections['user'].update({'apTok.tok': apTok}, {$unset: {apTok: ''}}, {multi: 1}, function () {
    mongodb.collections['user'].update({_id: ObjectId('' + uId)}, {$set: {apTok: [newApTok]}}, handleResult)
  })
}

exports.clearApTok = function (uId, cb) {
  mongodb.collections['user'].update({_id: ObjectId('' + uId)}, {$unset: {apTok: ''}}, function (err, success) {
    cb && cb((err || !success) ? {error: err} : {ok: success})
  })
}
...
exports.setFbId = function (uId, fbId, cb, fbTok) {
  ...
  exports.fetchByFbUid(fbId, function (user) {
    if (user) {
      if (user._id != uId) {
        console.error('[WARNING] This Facebook account is already associated to another user:', uId, fbId, user._id)
        cb && cb({error: 'This Facebook account is already associated to another user'})
      } else { cb && cb({error: 'This Facebook account is already associated to this user'}) }
    } else {
...
exports.setTwitterId = function (uId, twId, twTok, twSec, cb) {
  ...
  if (!uId) { return cb && cb({error: 'invalid parameters'}) } else if (!twId) // disconnect twitter account
  {
    mongodb.collections['user'].update({_id: ObjectId('' + uId)}, {$unset: {twId: '', twTok: '', twSec: ''}}, function (err, success) {
      cb && cb((err || !success) ? {error: err} : {ok: success})
    })
  } else if (!twTok || !twSec) { return cb && cb({error: 'invalid parameters'}) } else {
    fetch({twId: twId}, function (err, user) {
      if (user && user._id != uId) { cb && cb({error: 'This Twitter account is already associated to another user'}) } else { exports.save({ _id: uId, twId: twId, twTok: twTok, twSec: twSec }, cb) }
    })
  }
}
...
exports.setHandle = function (uId, username, handler) {
  ...
  if (username.length < USERNAME_MIN_LENGTH) { return res({error: 'Must be at least ' + USERNAME_MIN_LENGTH + ' characters long'}) }
  if (username.length > USERNAME_MAX_LENGTH) { return res({error: 'Must be less than ' + USERNAME_MAX_LENGTH + ' characters long'}) }
  if (!USERNAME_REGEX.test(username)) { return res({error: 'Special characters are not allowed'}) }
  if (!isNaN(username)) { return res({error: 'Numbers are not allowed'}) }
  if (USERNAME_RESERVED[username]) { return res({error: 'This username is reserved'}) }

  exports.fetchByHandle(username, function (user) {
    if (user && '' + user._id != '' + uId) { res({error: 'This username is taken by another user'}) } else {
      fetch({_id: uId}, function (err, user) {
...
exports.renameUser = function (uid, name, callback) {
  ...
  if (!user) { callback({error: 'renameUser error: user not found'}) } else if (oldName == name) { callback({}) } // nothing to do
  else {
    (function next () {
      var col
      ...
      col.count({$or: [{uId: uid}, {tId: uid}]}, function (err, count) {
        ...
        col.update({uId: uid/*, uNm: oldName */}, {$set: {uNm: name}}, {multi: true}, function (err, result) {
          if (err) console.log('err', err)
          // console.log("-> updated to ", result);
          col.update({tId: uid/*, tNm: oldName */}, {$set: {tNm: name}}, {multi: true}, function (err, result) {
            if (err) console.log('err', err)
            // console.log("-> updated to ", result);
            next()
          })
...
exports.fetchUserFields = function (subList, attrToCopy, cb) {
  var uidList = []
  if (subList) {
    for (var i in subList) { uidList.push(subList[i].id) }
  }
  if (uidList.length) {
    var attrSet = snip.arrayToSet(attrToCopy, 1)
    exports.fetchMulti({_id: {$in: uidList}}, {fields: attrSet}, function (userList) {
      var uidSet = {}
      ...
      for (var i in subList) {
        for (var j in attrToCopy) {
          if (subList[i] && uidSet[subList[i].id]) { subList[i][attrToCopy[j]] = uidSet[subList[i].id][attrToCopy[j]] }
        }
      }
      cb(subList)
    })
...
exports.fetchUserFieldsIfNeeded = function (userList, fields, cb) {
  for (var u in userList) {
    for (var f in fields) {
      if (!userList[u].hasOwnProperty(fields[f])) { return exports.fetchUserFields(userList, fields, cb) }
    }
  }
  cb(userFields)
}
