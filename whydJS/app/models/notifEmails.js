...
function sendEmail (toUid, emailObj, cb) {
  ...
  if (!to || !emailObj) { cb && cb({error: 'invalid call to sendEmail()'}) } else { emailModel.email(to.email, emailObj.subject, emailObj.bodyText, emailObj.bodyHtml, to.name, cb) }
}
...
exports.sendRegWelcomeAsync = function (storedUser, inviteSender, cb) {
  notifTemplate.generateRegWelcomeAsync(storedUser, inviteSender, function (email) {
    emailModel.email(storedUser.email, email.subject, email.bodyText, email.bodyHtml, undefined, cb)
  })
}
...
exports.sendInviteAccepted = function (senderId, storedUser, cb) {
  if (getUserPrefs(senderId)['emAcc'] != -1) { sendEmail(senderId, notifTemplate.generateInviteAccepted(storedUser)/*, cb */) }
  // else
...
exports.askAccountDeletion = function (uId, uNm) {
  var text = (uNm || 'A user') + ' requested deletion of their profile: ' + process.appParams.urlPrefix + '/u/' + uId
  emailModel.email(process.appParams.feedbackEmail, '[' + process.appParams.urlPrefix + '] user account deletion request', text)
}
...
exports.sendPasswordReset = function (uid, resetCode, redirect) {
  var temp = notifTemplate.generatePasswordReset({id: uid}, {resetCode: resetCode, redirect: redirect})
...
function submitNotif (recipient, type, immediateNotifHandler, noNotifHandler) {
  if (!recipient) { return console.error('NULL recipient in models/notifEmails.js/submitNotif()') }
  ...
  } else if (immediateNotifHandler) // recipient.pref[type] == 0 (call handler to send immediate email)
...
exports.sendRepost = function (reposter, post, postAuthor /* Email */) {
  submitNotif(postAuthor, 'emAdd', function () {
    ...
    emailModel.email(postAuthor.email/* Email */, temp.subject, temp.bodyText, temp.bodyHtml)
  })
}
...
exports.sendSubscribedToUser = function (subscriber, selectedUser, cb) {
  submitNotif(selectedUser, 'emSub', function () {
    var temp = notifTemplate.generateSubscribedToUser(subscriber, selectedUser._id)
    emailModel.email(selectedUser.email, temp.subject, temp.bodyText, temp.bodyHtml, undefined, cb)
  }, cb)
}
...
exports.sendLike = function (user, post, postAuthor) {
  submitNotif(postAuthor, 'emLik', function () {
    if (postAuthor.pref && postAuthor.pref['emLik'] == 0) { // user explicitely chose to receive immediate notifications
      var temp = notifTemplate.generateLike(user, post, postAuthor)
      emailModel.email(postAuthor.email, temp.subject, temp.bodyText, temp.bodyHtml)
    }
  })
}
...
exports.sendComment = function (post, comment, cb) {
  if (getUserPrefs(post.uId)['emCom'] == -1) { cb && cb({warn: 'no email notification will be sent (disabled by user)'}) } else { sendEmail(post.uId, notifTemplate.generateComment(post, comment), cb) }
}

// 6) "XXX mentioned you"
exports.sendMention = function (mentionedUid, post, comment, cb) {
  if (getUserPrefs(post.uId)['emMen'] == -1) { cb && cb({warn: 'no email notification will be sent (disabled by user)'}) } else { sendEmail(mentionedUid, notifTemplate.generateMention(mentionedUid, post, comment), cb) }
}

// 7) "XXX replied to your comment"
exports.sendCommentReply = function (post, comment, repliedUid, cb) {
  if (getUserPrefs(post.uId)['emRep'] == -1) { cb && cb({warn: 'no email notification will be sent (disabled by user)'}) } else { sendEmail(repliedUid, notifTemplate.generateCommentReply(post, comment, repliedUid), cb) }
}
