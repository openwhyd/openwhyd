...
function sendEmail(toUid, emailObj, cb){
	...
	if (!to || !emailObj)
		cb && cb({error:"invalid call to sendEmail()"});
	else
		emailModel.email(to.email, emailObj.subject, emailObj.bodyText, emailObj.bodyHtml, to.name, cb);
}
...
exports.sendInviteAccepted = function (senderId, storedUser, cb) {
	if (getUserPrefs(senderId)["emAcc"] != -1)
		sendEmail(senderId, notifTemplate.generateInviteAccepted(storedUser)/*, cb*/);
	//else
...
function submitNotif(recipient, type, immediateNotifHandler, noNotifHandler) {
	if (!recipient)
		return console.error("NULL recipient in models/notifEmails.js/submitNotif()");
...
exports.sendComment = function (post, comment, cb) {
	if (getUserPrefs(post.uId)["emCom"] == -1)
		...
		sendEmail(post.uId, notifTemplate.generateComment(post, comment), cb);
};
...
exports.sendMention = function (mentionedUid, post, comment, cb) {
	if (getUserPrefs(post.uId)["emMen"] == -1)
		...
		sendEmail(mentionedUid, notifTemplate.generateMention(mentionedUid, post, comment), cb);
};
