...
exports.fetchLast = function(p, cb) {
	p = p || {};
	if (!p.pId)
		cb({error: "missing field: pId"});
	else
		getCol().findOne({pId: ""+p.pId}, {sort:[["_id", "desc"]]}, combineResult(cb));
}
...
exports.fetch = function(q, p, cb) {
	...
	if (q.pId)
		q.pId = {$in: (q.pId.push ? q.pId : [q.pId]).map(/*mongodb.ObjectId*/stringify)};
