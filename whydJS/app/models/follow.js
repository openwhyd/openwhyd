...
function transformSubscriptionArray(results) {
	if (results)
		for (var i in results)
			results[i] = {id: (""+results[i].tId).replace("/u/",""), name: results[i].tNm};
	return results;
}
...
function transformSubscriberArray(results) {
	if (results)
		for (var i in results)
			results[i] = {id: /*"/u/" +*/ ""+results[i].uId, name: results[i].uNm};
	return results;
}
...
exports.add = function(followObj, dbHandler) {
	...
	collection.update(req, followObj, {upsert: true}, function (err, result) {  // to avoid duplicates
		if (err)
			dbHandler(err, result);
		else
			collection.findOne(req, dbHandler);
	});
}
