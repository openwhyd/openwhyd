...
function transformSubscriptionArray (results) {
  if (results) {
    for (var i in results) { results[i] = {id: ('' + results[i].tId).replace('/u/', ''), name: results[i].tNm} }
  }
  return results
}
...
function transformSubscriberArray (results) {
  if (results) {
    for (var i in results) { results[i] = {id: /* "/u/" + */ '' + results[i].uId, name: results[i].uNm} }
  }
  return results
}
...
function fetchArray (query, options, callback) {
  var query = query || {}
  if (typeof query.uId === 'string' && query.uId.indexOf('/u/') == 0) { console.error('warning: found uId with /u/ prefix') }
  if (typeof query.tId === 'string' && query.tId.indexOf('/u/') == 0) { console.error('warning: found tId with /u/ prefix') }
  if ((query.uId || {}).generationTime) // is ObjectId
  { query.uId = '' + query.uId }
  if ((query.tId || {}).generationTime) // is ObjectId
  { query.tId = '' + query.tId }
  // console.log("follow.fetchArray", query, "...");
  mongodb.collections[COLNAME].find(query, options || {}, function (err, cursor) {
...
exports.add = function (followObj, dbHandler) {
  var req = {
    uId: followObj.uId, tId: followObj.tId
  }
  var collection = mongodb.collections[COLNAME]
  // collection.save(obj, dbHandler);
  collection.update(req, followObj, {upsert: true}, function (err, result) { // to avoid duplicates
    if (err) { dbHandler(err, result) } else { collection.findOne(req, dbHandler) }
  })
}
...
exports.fetchUserSubscriptions = function (uid, callback) {
  ...
  fetchArray({uId: uid}, {sort: [['_id', 'desc']]}, function (subscriptions) {
    result.subscriptions = transformSubscriptionArray(subscriptions)
    fetchArray({tId: uid}, {sort: [['_id', 'desc']]}, function (subscribers) {
...
exports.fetchSubscriptionSet = function (uid, callback) {
  fetchArray({uId: uid}, {fields: {_id: 0, tId: 1}}, function (subscriptions) {
...
exports.fetchSubscriptionHistory = function (options, callback) {
  options = options || {}
  var q = {}
  if (options.fromUId) { q.uId = {$in: options.fromUId} } else if (options.toUId) {
    q.uId = {$ne: '' + options.toUId}
    q.tId = '' + options.toUId
  }
  if (typeof options.excludeTids === 'object') { q.tId = {$nin: options.excludeTids} }
  if (typeof options.excludeCtx === 'string') { q.ctx = {$nin: [options.excludeCtx]} }
  if (options.after) { q._id = {$lt: ObjectId('' + options.after)} } else if (options.until) { q._id = {$gt: mongodb.ObjectId(mongodb.dateToHexObjectId(options.until))} }
  fetchArray(q, {sort: [['_id', 'desc']], limit: options.limit || HISTORY_LIMIT}, callback)
}
