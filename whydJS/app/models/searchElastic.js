...
exports.query = function(q, handler) {
	...
	if (q.uId)
		filter.push({ term: { "uId":q.uId }});
	if (q.excludeUid)
		filter.push({ not: { term: { "uId":q.excludeUid }}});

	if (q._type)
		filter.push({ term: { "_type": q._type }});
	...
	*/
	...
	if (q.uId)
		query.bool.should.push({term:{ "text": q.q }});
...
exports.indexTyped = function(type, item, handler) {
	...
	if (!type || !INDEX_FIELDS[type])
		...
		var doc = { _type: type };
		for (var f in INDEX_FIELDS[type])
			doc[f] = item[f];
