...
function getJson (pathSuffix, cb) {
  return http.request(buildReq('GET', pathSuffix), function (res) {
    var json = ''
...
function postJson (data, cb) {
  ...
  if (data._bulkStr) {
    pathSuffix += '/_bulk'
    data = data._bulkStr
    console.log('search.postJson BULK', pathSuffix, ':', data.length, 'bytes...')
  } else {
  ...
  return http.request(buildReq('POST', pathSuffix), function (res) {
    var json = ''
    res.addListener('data', function (chunk) {
      json += chunk.toString()
    })
...
exports.deleteAllDocs = function (type, cb) {
  ...
  return http.request(buildReq('DELETE', '/' + type), function (res) {
...
exports.deleteDoc = function (type, id, cb) {
  ...
  return http.request(buildReq('DELETE', '/' + type + '/' + id), function (res) {
    cb && res.addListener('end', cb)
  })
    .addListener('error', function (err) {
      console.error('elasticsearch socket error: ', err)
...
exports.index = function (doc, handler) {
  ...
  postJson(doc, function (response) {
    console.log('models.search.index() => ', (response || {}).ok ? 'OK' : 'ERROR')
...
exports.indexBulk = function (docs, cb) {
  ...
  var bulkStr = ''
  for (var i in docs) {
    var u = docs[i], meta = {}, data = {}
    if (!u) {
      console.warn('ignoring empty doc from being indexed in BULK')
      continue
    }
    for (var field in u) {
      if (field[0] == '_') { meta[field] = u[field] } else { data[field] = u[field] }
    }
    bulkStr += JSON.stringify({'index': meta}) + '\n' + JSON.stringify(data) + '\n'
  }
...
exports.indexTyped = function (type, item, handler) {
  ...
  if (!type || !INDEX_FIELDS[type]) { handler || logToConsole({error: 'indexTyped: unknown type'}) } else if (!item || !item._id || !item.name) { handler || logToConsole({error: 'indexTyped: missing parameters'}) } else {
    var doc = { _type: type }
...
exports.init = function () {
  console.log('models.search: initializing ElasticSearch index:', INDEX_HOST + ':' + INDEX_PORT + '/' + INDEX_NAME, '...')
  try {
