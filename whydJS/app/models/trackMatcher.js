...
function getBestTitle (track, targetName) {
  var comb = [
    ...
    return [ combinedName, snip.getLevenshteinDistance(combinedName, targetName) ]
  })
...
function getTrackTitle (track) {
  ...
  assert.ok(track.name || richMeta, 'no title found for track: ' + JSON.stringify(track))
  return track.artistName && track.trackTitle ? [track.artistName, track.trackTitle].join(' - ') : track.name
}
...
Dist.prototype.toString = function (prefix) {
  var prefix = prefix || ''
  return [
    prefix + 'track: ' + this.targetName + ' (' + this.targetDuration + ' seconds)',
    prefix + 'hit  : ' + this.hitTrackName + ' (' + this.hitTrackDuration + ' seconds)',
    prefix + '=> name distance: ' + this.nameDistance +
		       ' + duration distance: ' + this.durationDistance +
		       (this.hasOwnProperty('confidence') ? ' => confidence: ' + this.confidence : '')
  ].join('\n')
}
...
function TrackMatcher (track) {
  ...
  this.evalDistances = function (hit) {
    ...
    if (richTrack && richHit) {
      dist.nameDistance = snip.getLevenshteinDistance(track.artistName, hit.artistName) +
      ...
      dist.nameDistance = snip.getLevenshteinDistance(dist.targetName, dist.hitTrackName)
    }
    // 2) compare names
  ...
  this.evalConfidence = function (hit, dist) {
    ...
    dist.durationDistance = dist.durationDistance || this.DURATION_TOLERANCE / 2
    ...
    var nameConfidence = 1 + (1 - Math.pow(2, dist.nameDistance)) / this.NAME_TOLERANCE
    var durationConfidence = 1 - dist.durationDistance / this.DURATION_TOLERANCE
    dist.confidence = Math.min(Math.max((nameConfidence + durationConfidence) / 2, 0.0), 1.0)
    return dist
  }
  return this
}
