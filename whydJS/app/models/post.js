...
function processAdvQuery (query, params, options) {
  ...
  if (options.before != null) {
    if (mongodb.isObjectId(options.before)) { query._id = {$gt: ObjectId('' + options.before)} } else if (!isNaN(parseInt(options.before))) // if (params.sort == playlistSort.sort)
    { query.order = {$lt: parseInt(options.before)} }
  }
  if (options.after != null) {
    if (mongodb.isObjectId(options.after)) { query._id = {$lt: ObjectId('' + options.after)} } else if (!isNaN(parseInt(options.after))) // if (params.sort == playlistSort.sort)
...
exports.fetchPosts = function (query, params, options, handler) {
  ...
  mongodb.collections['post'].find(query, params, function (err, cursor) {
    ...
    cursor.toArray(function (err, results) {
      if (err) { console.error('model.fetchPosts ERROR on query', query, 'with params', params, ':', err) }
      results = results || []
...
function setPostLove (collection, pId, uId, state, handler) {
  ...
  collection.update({_id: ObjectId('' + pId)}, update, function (err, res) {
    ...
    collection.findOne({_id: ObjectId('' + pId)}, function (err, post) {
      ...
      if (state) { activityModel.addLikeByPost(post, {id: uId, name: mongodb.getUserNameFromId(uId)}) } else { activityModel.removeLike(pId, uId) }
    })
  })
}
...
exports.savePost = function (postObj, handler) {
  ...
  if (postObj.pl && typeof postObj.pl.id !== 'number') { postObj.pl.id = parseInt('' + postObj.pl.id) }
  if (pId) {
    delete postObj._id
...
exports.rePost = function (pId, repostObj, handler) {
  ...
  exports.fetchPostById(pId, function (postObj) {
    ...
    for (var i in fieldsToCopy) {
      if (/* repostObj[i] == null && */ postObj[i] != null) { repostObj[i] = postObj[i] }
    }
    ...
    if (repostObj.pl && typeof repostObj.pl.id !== 'number') { repostObj.pl.id = parseInt('' + repostObj.pl.id) }
  ...
  exports.fetchPostById(pId, function (postObj) {
    if (postObj) {
      collection.remove(q, function (error, result) {
        ...
        handler(result)
        if (postObj.repost) { collection.update({_id: ObjectId('' + postObj.repost.pId)}, {$inc: {nbR: -1}}, {w: 0}) }
...
exports.fetchPlaylistPosts = function (uId, plId, options, handler) {
  ...
  } else { exports.fetchPosts({'pl.collabId': {$in: ['' + plId, ObjectId('' + plId)]}}, playlistSort, options, handler) }
}
...
exports.countPlaylistPosts = function (uId, plId, handler) {
  ...
  if (uId) { db['post'].count({'uId': uId, 'pl.id': parseInt(plId)/* {$in:[parseInt(plId), ""+plId]} */}, handle) } else { db['post'].count({'pl.collabId': {$in: ['' + plId, ObjectId('' + plId)]}}, handle) }
}
...
exports.setPlaylistOrder = function (uId, plId, order, handler) {
  ...
  function next (err) {
    ...
    if (!order.length) { handler({ok: 1}) } else {
      var post = {
