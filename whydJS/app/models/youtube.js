...
function extractMetadataFromYoutubePage (html, cb) {
  ...
  if (found) { console.log('Parsing metadata from Youtube page...') } else {
  ...
  htmlDom.parseHtmlDom(html, function (doc) {
    ...
    for (var i in meta) {
      ...
      if (/Purchase/.test(type)) { metadata.trackTitle = data.getChildren()[0].getText().replace(/ \($/, '') } else if (/Artist/.test(type)) { metadata.artistName = data.getText() }
...
exports.fetchMetadataFromYoutubePage = function (url, cb) {
  scrape(url, function (err, res) {
    if (err || !(res || {}).html) { cb(err, res) } else { extractMetadataFromYoutubePage(res.html, cb) }
  })
}
...
function searchYoutubeTracks (q, cb) {
  var url = 'http://gdata.youtube.com/feeds/api/videos?v=2&alt=jsonc&first-index=0&max-results=5&q=' + encodeURIComponent(q)
  return snip.httpRequestJSON(url, null, cb)
}

exports.searchTracks = function (trackMetadata, cb, raw) {
  return searchYoutubeTracks(translateOutgoingQueryParams(trackMetadata).q, function (err, res) {
    if (err || raw) { cb(err, res) } else {
      cb(err, {
        items: (((res || {}).data || {}).items || []).map(exports.translateTrack)
      })
    }
  })
}
...
exports.fetchTrackMetadata = function (trackId, cb, raw) {
  ...
  var url = 'https://gdata.youtube.com/feeds/api/videos/' + trackId + '?v=2&alt=jsonc'
  return snip.httpRequestJSON(url, null, function (err, res) {
    if (err || raw) { return cb(err, res) }
    var data = (res || {}).data
    if (!data) { cb(new Error('invalid youtube reponse: ' + JSON.stringify(res))) } else {
      var url = 'http://www.youtube.com/watch?v=' + ('' + trackId).replace(/[^a-z0-9\_\-]/gi, '')
