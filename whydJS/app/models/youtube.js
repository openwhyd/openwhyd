...
function extractMetadataFromYoutubePage(html, cb) {
	...
	if (found)
		console.log("Parsing metadata from Youtube page...");
	else {
	...
	htmlDom.parseHtmlDom(html, function(doc){
		...
		for(var i in meta){
			...
			if (/Purchase/.test(type))
				metadata.trackTitle = data.getChildren()[0].getText().replace(/ \($/, "");
			else if (/Artist/.test(type))
				metadata.artistName = data.getText();
...
exports.fetchMetadataFromYoutubePage = function(url, cb){
	scrape(url, function(err, res){
		if (err || !(res || {}).html)
			...
			extractMetadataFromYoutubePage(res.html, cb);
	});
}
...
exports.fetchTrackMetadata = function(trackId, cb, raw){
	...
	return snip.httpRequestJSON(url, null, function(err, res) {
		...
		var data = (res || {}).data;
		if (!data)
			cb(new Error("invalid youtube reponse: " + JSON.stringify(res)));
		else {
