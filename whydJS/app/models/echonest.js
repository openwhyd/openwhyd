...
function queryEchonest (p, cb) {
  ...
  p.bucket = ['audio_summary', 'tracks', 'id:deezer', 'id:spotify-WW'/*, "id:musicbrainz", "id:discogs" */]
  var url = 'http://developer.echonest.com/api/v4/song/search?' + querystring.stringify(p)
...
function translateOutgoingQueryParams (p) {
  p = p || {}
  var res = p.artistName && p.trackTitle ? snip.filterFields(p, SEARCH_FIELD_MAPPING) : { combined: p.name }
  if (p.duration) {
...
exports.translateTrack = function (track) {
  ...
  if (track.tracks && track.tracks.length) {
    ...
    if (track.tracks[0].album_name) {
      ...
      if (track.tracks[0].album_date) { res.albumYear = track.tracks[0].album_date.substr(0, 4) }
    }
    // 2) foreign ids
    var tracks = track.tracks
    res.foreignIds = {}
    for (var i in tracks) {
      if (!res.foreignIds[tracks[i].catalog]) { res.foreignIds[tracks[i].catalog] = tracks[i].foreign_id }
    }
  }
  // 3) duration
  if (track.audio_summary && track.audio_summary.duration) { res.duration = track.audio_summary.duration }
  return res
}
...
function searchTracks (p, cb, raw) {
  queryEchonest(translateOutgoingQueryParams(p), function (err, res) {
    if (err || raw) { cb(err, res) } else {
      cb(null, {
        items: (((res || {}).response || {}).songs || []).map(exports.translateTrack)
      })
    }
  })
}
...
exports.fetchTrackMetadata = function (trackId, cb, raw) {
  ...
  var url = 'http://developer.echonest.com/api/v4/track/profile?' + querystring.stringify(p)
  // console.log("url", url);
  return snip.httpRequestJSON(url, null, function (err, res) {
    // console.log("EN", err, res)
    if (err || raw) { return cb(err, res) }
    var data = (res || {}).response
    if (!data) { cb(new Error("no data was found in echonest's reponse")) } else {
