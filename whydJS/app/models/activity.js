...
exports.add = function(d, callback) {
	if (d && d.like && d.like.pId)
		d.like.pId = ""+d.like.pId;
...
exports.fetchHistoryFromUidList = function(uidList, options, callback) {
	...
	function whenDone(activities) {
		...
		console.log("=> reduced to", activities.length, "activities, hasmore:", hasMore);
...
exports.addLikeByPost = function(post, liker, callback) {
	if (post)
		exports.add({id:liker.id, name:liker.name, like:{pId:post._id, id:post.uId, name:post.uNm}}, callback);
	else
		callback && callback({error:"post not found"});
}
