...
exports.fetchPostsByGenre = function (genre, p, cb) {
  if (!genre) { exports.fetchPosts(p, cb) } else { exports.fetchPostsByGenres([genre], p, cb) }
}
...
function translateTrackToConciseMetadata (track) {
  ...
  if (typeof track.mappings === 'object') {
    ...
    for (var sourceId in track.mappings) {
      ...
      if (alt.id != trackId && alt.c >= MINIMUM_CONFIDENCE) { concise.alt.push('/' + sourceId + '/' + (alt.uri || alt.id)) }
    }
  }
  return concise
}
...
function flattenObjectProperties (obj, path, res) {
  ...
  for (var f in obj) {
    path.push(f)
    if (typeof obj[f] === 'object' && !obj[f].splice) { flattenObjectProperties(obj[f], path, res) } else { res[path.join('.')] = obj[f] }
    path.pop()
  }
  return res
}
...
exports.updateAndPopulateMetadata = function (eId, cb, force) {
  ...
  exports.fetchTrackByEid(eId, function (track) {
    ...
    exports.fetchConciseMetadataForEid(eId, function (finalMeta) {
      ...
      if (finalMeta.error) {
        if (finalMeta.error.indexOf('404 - Not Found') == -1) { console.error('updateAndPopulateMetadata', finalMeta.error || 'null error in updateAndPopulateMetadata()') }
        cb && cb(finalMeta)
        return
      }
      if (finalMeta.meta) {
