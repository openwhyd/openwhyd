...
exports.query = function (q, cb) {
  ...
  var q = q || {}
  var queue
  if (q._type) { queue = [q._type] } else if (q.uId) { queue = ['post'] } else { queue = ['user', 'track', 'post', 'playlist'] } // Object.keys(searchByType);
  delete q._type;
  (function next () {
    var type = queue.pop()
    if (!type) { cb({q: q.q, hits: hits}) } else {
...
function indexTypedDocs (type, items, callback) {
  ...
  if (!type || !INDEX_NAME_BY_TYPE[type]) {
    ...
    getIndex(INDEX_NAME_BY_TYPE[type]).addObjects(docs, function (err, content) {
      if (err) {
        console.error('algolia error when indexing ' + items.length + ' ' + type + ' items => ' +
				  err.toString())
      } else {
