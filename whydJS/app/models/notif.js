...
function pushToMobile(code, toUser, text, payload){
	...
	if (!(toUser || {}).pref)
		return console.error("push notif prefs not found for user: ", initialToUser);
	if (parseInt(""+toUser.pref["mn"+code]) > -1) {
		if (toUser.apTok)
			pushToMobileTokens(toUser, text, payload);
		else
...
function pushToMobiles(code, toUsers, text, payload){
	for (var i in toUsers)
		pushToMobile(code, toUsers[i], text, payload);
}
...
function invalidateUserNotifsCache(uId) {
	if (uId.splice)
		for (var i in uId)
			delete exports.userNotifsCache[""+uId[i]];
	else
		delete exports.userNotifsCache[""+uId]; // => force fetch on next request
}
	...
	db["notif"].find({uId:uId}, {sort:['t','desc']}, function (err, cursor) {
		cursor.toArray(function(err, results) {
			...
			for (var i in results) {
				var n = 0;
				if ((""+results[i]._id).endsWith("/loves"))
					n = results[i].n;
				else
					for (var j in results[i].uId)
						if (results[i].uId[j] == uId)
							n++;
...
exports.getUserNotifs = function (uid, handler) {
	...
	if (cachedNotifs)
		...
		exports.fetchUserNotifs(uid, handler);
};
...
exports.love = function (loverUid, post) {
	...
	db["notif"].update({_id:post._id+"/loves"}, {
		$set: { eId:post.eId, name:post.name,
			t:Math.round((new Date()).getTime()/1000),
		...
		col.findOne(criteria, function(err, res) {
			if (res) {
				if (!res.lov || res.lov.length == 0 || res.n < 1)
					...
					col.update(criteria, {$set:{uIdLast:res.lov[res.lov.length-1]}}, {w:0});
