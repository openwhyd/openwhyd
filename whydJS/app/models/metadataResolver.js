...
function appendTrackMappings (track, cb) {
  ...
  if (!track.metadata.hasOwnProperty('confidence')) { track.metadata.confidence = 1 }
...
exports.fetchMetadataForEid = function (eId, callback) {
  ...
  if (!api || !api.fetchTrackMetadata) {
    ...
    if (IGNORED_EXTRACTORS[match.sourceId]) { return cb(null, track) } else { return cb(new Error('unknown source id: ' + match.sourceId)) }
  }
  api.fetchTrackMetadata(match.contentUri, function (err, metadata) {
    track.metadata = metadata
    if (err || !metadata || (!metadata.name && !metadata.trackTitle)) { return cb(err, track) }
