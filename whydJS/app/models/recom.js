...
exports.matchingEngine = (function () {
  ...
  function addPost (track) {
    if (track && track.name && track.uId) {
      ...
      if (artist) {
        ...
        (usersByArtist[artist] = usersByArtist[artist] || []).push([track.uId, true]);
        (artistsByUser[track.uId] = artistsByUser[track.uId] || []).push([artist, true])
      }
    }
  }
  ...
  function populateIndex (cb) {
    ...
    if (config.recomPopulation) {
      console.log('recom.matchingEngine.populateIndex: fetching posted and liked artists...')
      postModel.forEachPost({}, {batchSize: 1000, fields: {_id: 0, uId: 1, uNm: 1, name: 1}}, function (track) {
        if (!track) {
          console.log('recom.matchingEngine.populateIndex: done indexing! => score of most popular artist', mostPopular)
          whenDone()
        } else { addPost(track) }
      })
    } else {
  ...
  function waitForIndex (cb) {
    if (ready) { cb() } else if (populating) { whenReady.push(cb) } else { populateIndex(cb) }
  }

  var printMemUsage = !TRACK_MEMORY_USAGE ? function () {} : function () {
    console.log('RECOM MEMORY USAGE...')
    console.log('RECOM MEMORY USAGE, artists:', sizeof(artists))
    console.log('RECOM MEMORY USAGE, usersByArtist:', sizeof(usersByArtist))
    console.log('RECOM MEMORY USAGE, artistsByUser:', sizeof(artistsByUser))
  }
      ...
      waitForIndex(function () {
        ...
        for (var artist in myArtists) {
          ...
          for (var u in users) {
            ...
            var recomUser = recomUsers[userId] = recomUsers[userId] || {id: userId, name: mongodb.usernames[userId].name};
...
exports.computeUserSimilarity = function (uId1, uId2, cb) {
  exports.matchingEngine.fetchCommonArtistsForTwoUsers(uId1, uId2, function (common) {
    // console.log("common", common);
    var nbArtists = (exports.matchingEngine.getArtistsByUser(uId1) || []).length / 2
    common = snip.mapToObjArray(common, 'id', 'c').sort(function (a, b) {
      return b.c - a.c
    }).map(function (artist) {
      return {
        id: artist.id,
  ...
  exports.matchingEngine.fetchCommonArtists(uId, options, function (users) {
    ...
    for (var i in users) {
      ...
      for (var j in bestArtists) {
        ...
        if (artist && artist.name) { users[i].artistNames.push(bestArtists[j].name) }
      }
      users[i].score = (
