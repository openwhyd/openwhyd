...
exports.parseDom = function (dom) {
  ...
  SimpleDomNode.prototype.getElementsByTagName = function (tagName) {
    ...
    for (var i in nodes) {
      ...
      if (nodes[i].children) { results = results.concat(node.getElementsByTagName(tagName)) }
    }
    return results
  }
  SimpleDomNode.prototype.getElementsByClassName = function (className) {
    ...
    for (var i in nodes) {
      ...
      if (nodes[i].children) { results = results.concat(node.getElementsByClassName(className)) }
    }
    return results
  }
