...
exports.save = function (p, render) {
  ...
  if (!p.uId && !mongodb.usernames[p.uId]) { return render({error: 'organizer not found'}) }
  var plC = {
    title: p.title,
    uId: p.uId,
    url: p.url,
  ...
  mongodb.collections['plContest'].find({}, {sort: [['_id', 'desc']]}, function (err, cursor) {
    cursor.toArray(function (err, contests) {
      ...
      (function fetchNextContextPlaylists () {
        if (--i < 0) { cb(contests || {error: err}) } else {
          ...
          searchModel.query({q: contest.title}, function (r) {
            ...
            var hits = (r || {}).hits || []
            for (var j in hits) {
              if (hits[j]._type == 'playlist' && hits[j].name == contest.title) { contest.playlists.push(hits[j]._id) }
            }
