...
function searchSoundcloudTracks (q, cb) {
  var url = 'http://api.soundcloud.com/tracks.json?client_id=' + CLIENT_ID + '&q=' + encodeURIComponent(q)
...
exports.searchTracks = function (trackMetadata, cb, raw) {
  return searchSoundcloudTracks(translateOutgoingQueryParams(trackMetadata).q, function (err, res) {
    if (err || raw) { cb(err, res) } else if (res && !res.map) {
 			// e.g. res = { errors: [ { error_message: '503 - Service Unavailable' } ] }
 			var ERR_MSG = 'invalid result from searchSoundcloudTracks'
      if (res.errors) { ERR_MSG += ': ' + res.errors[0].error_message || JSON.stringify(res) }
...
function fetchMetadataFromUrl (url, cb, raw) {
  ...
  snip.httpRequestJSON(url, {}, function (err, res) {
    // console.log("=> ", err, res);
    if (err || raw) { cb(err, res) } else if (res && (!res.id || !res.title)) {
      // soundcloud api error or redirection
      if (res.location) { fetchMetadataFromUrl(res.location, cb, raw) } else { cb(new Error('Unrecognized response from soundcloud: ' + JSON.stringify(res))) }
...
function ensureScUrl (url) {
  url = url.split('#')[0]
  if (url.indexOf('//') == -1) { return 'http://' + ('soundcloud.com/' + url).replace(/\/\//g, '/') } else if (url.indexOf('//') == 0) { return 'http:' + url } else { return url }
}

exports.fetchTrackMetadata = function (trackId, cb, raw) {
  assert.ok(trackId, 'trackId is null')
  var trackId = '' + trackId
  var url = 'https://api.soundcloud.com' + ('' + parseInt(trackId) === trackId
    ? '/tracks/' + encodeURIComponent(trackId) + '.json?client_id=' + CLIENT_ID
    : '/resolve.json?client_id=' + CLIENT_ID + '&url=' + encodeURIComponent(ensureScUrl(trackId)))
  console.log('URL', url, '...')
