...
for (var i in exports.ORDERED_GENRES)
	exports.ORDERED_GENRES[i].id = exports.ORDERED_GENRES[i].name.toLowerCase().replace("&", "n").replace(" ", "-");
...
exports.tagEngine = new (function TagEngine(){
	...
	function appendUrl(tagObj) {
		if (tagObj.id)
			tagObj.url = "/genre/" + tagObj.id.replace(/\s+/g, "-");
		return tagObj;
	}
		...
		if (self.eidToTags)
			...
			var interval = setInterval(function(){
				if (self.eidToTags) {
					...
					console.log("still waiting for tag index to be ready...");
			}, 1000);
	}
	...
	self.getUsersByTags = function(tags) {
		...
		*/
		...
		function scoreUserAgainstTags(uId) {
			...
			if (!user || !user.img || user.img.indexOf("blank") != -1 || !userTags || !userTags.length)
				return 0;
			...
			var steps = [
				...
				(function(){	
					...
					for (var i=0; i<nbTopTags; ++i)
						if (tagSet[userTags[i]])
							score += Math.pow(2, tags.length-i-1);
