...
for (var i in exports.ORDERED_GENRES) { exports.ORDERED_GENRES[i].id = exports.ORDERED_GENRES[i].name.toLowerCase().replace('&', 'n').replace(' ', '-') }

var GENRES_WITH_SYNONYMS = {
  'Reggae': ['reggae', 'dancehall', 'roots', 'ska', 'rocksteady', 'ragga'],
  'Latin': ['latin', 'salsa', 'caliente', 'paso doble', 'meringue', 'bossa nova', 'reggaeton', 'flamenco'],
  'Punk': ['punk', 'screamo', 'post-hardcore', 'straight edge'],
  'Metal': ['metal'],
  'Electro': ['electro', 'electronica', 'electronique', 'dance', 'techno', 'beats', 'idm'],
  'Blues': ['blues'],
  ...
  'Folk': ['folk', 'blue grass', 'bluegrass', 'blue-grass', 'acoustic', 'country', 'americana', 'celtic', 'acoustic'],
  'Pop': ['pop', 'variety', 'variétés']
}

var tagSynonyms = {}
for (var tag in GENRES_WITH_SYNONYMS) {
  for (var syn in GENRES_WITH_SYNONYMS[tag]) { tagSynonyms[GENRES_WITH_SYNONYMS[tag][syn]] = tag }
}

exports.extractGenreTags = function (plName) {
  var tags = []
  if (plName) {
    snip.removeAccents(plName.toLowerCase()).split(/[^\-a-z 1-3\&]+/g).map(function (phrase) {
...
exports.tagEngine = new function TagEngine () {
  ...
  var initFcts = [
    function /* importTagsFromPl */(cb) {
      ...
      var nbPl = 0, nbPlWithTags = 0, nbUsersWithPl = 0
      mongodb.forEach('user', {q: {'pl': {$exists: true}}, fields: {_id: 1, pl: 1}}, function (user) {
        ++nbUsersWithPl
    ...
    function /* buildTagIndex */(cb) {
      ...
      mongodb.forEach('post', {q: {'pl.id': {$exists: true}}, fields: {_id: 0, pl: 1, eId: 1, uId: 1}}, function (post) {
        var tags = self.plIdToTags['' + post.uId + '_' + post.pl.id]
        if (tags && tags.length) { eidToTags[post.eId] = (eidToTags[post.eId] || []).concat(tags) }
      }, function () {
    ...
    function /* buildUserIndex */(cb) {
      ...
      var query = {
        q: {_id: {$gt: mongodb.ObjectId(mongodb.dateToHexObjectId(new Date(t0 - TWO_WEEKS)))}}, // posted within the last two weeks
        fields: {_id: 0, eId: 1, uId: 1},
        sort: [['_id', 'desc']]
      }
      mongodb.forEach('post', query, function (post) {
        if (post.uId) {
          ++self.totalPosts
          var userTagSet = (self.uidToTagSet[post.uId] = self.uidToTagSet[post.uId] || {
  ...
  self.init = function (cb) {
    ...
    (function next () {
      if (initFcts.length) { initFcts.shift()(next) } else {
        self.initializing = false
        cb && cb(self)
      }
    })()
  ...
  self.waitForIndex = function (cb) {
    if (self.eidToTags) { cb(self) } else if (!self.initializing) { self.init(cb) } else {
    ...
    if (self.eidToTags && post && post.eId && post.uId) {
      ...
      if ((post.pl || {}).id != undefined) {
        ...
        if (tags && tags.length) {
          ...
          self.eidToTags[post.eId] = (self.eidToTags[post.eId] || []).concat(tags)
  ...
  self.getUsersByTags = function (tags) {
    if (!tags || !tags.length) // prevent reduce() from crashing because of empty array
    { return [] }
    ...
    function scoreUserAgainstTags (uId) {
      ...
      var user = mongodb.getUserFromId(uId)
      var userTags = snip.objArrayToValueArray(self.getBestTagsByUid(uId) || [], 'id') // this tagset is sorted by descending tag count
      if (!user || !user.img || user.img.indexOf('blank') != -1 || !userTags || !userTags.length) { return 0 }
  ...
  self.fetchTagsByUid = function (uId, cb) {
    if (!self.eidToTags) { cb() } else { cb(self.getBestTagsByUid(uId)) }
  }
}()
