...
exports.topicNameSearch = function(q, resultHandler, mongodbModel, timeoutDuration, maxCycles, limit) {
		...
		usercol.find(query, {limit:limitUsers}, function(err, cursor) {
			cursor.each(function(err, item) {
				if (item != null) {
					...
					console.log("user search result: " + item.fbId + " : " + item.name + ", " + item.img);
				}
			});
			...
			mongodb('topic', function(err, collection) {
				...
				collection.find(query, {sort:[['s','desc']],limit:limitExact}, function(err, cursor) {
					...
					var handleNextResult = function(err, item) {
						...
						if (item != null)
						{
							console.log("search exact result: (" + item.s + ") " + item._id + " : " + item.name /* + ", " + item.t*/);
							exactResults.push(item);
							if (--remaining > 0)
								cursor.nextObject(handleNextResult); //recursive call for next objects
						}
						else
							cursor.queryRun = false;
					};
				...
				collection.find(query, {/*sort:[['$natural','asc']],*/limit:limit}, function(err, cursor) {	
					var remaining = limit;
					...
					var handleNextResult = function(err, item) {
						...
						if (item != null)
						{
							console.log("search quick result: (" + item.s + ") " + item._id + " : " + item.name /* + ", " + item.t*/);
				...
				var renderer = function () {
					...
					if (exactResults.length > 0 && quickResults.length > 0)
						for (var j in quickResults)
						{
							...
							if (false == found)
								results.push(quickResults[j]);
						}
					else
						results = results.concat(quickResults);
					...
					if (results.length == 0)
					{
						...
						if (cycles-- == 0)
							resultHandler([], q); //response.render("");
