...
exports.remove = function (q, cb) {
  db['collabPl'].remove(q, function (error, result) {
    if (error) { console.error('collabPl.remove() error: ', error, error.stack) } else { console.log('=> removed collabPl:', result) }
    if (cb) { cb(result) }
  })
}
...
exports.save = function (playlist, cb) {
  ...
  // TODO: update() is deprecated => use updateOne() (new api)
  {
    db['collabPl'].update({_id: mongodb.ObjectId('' + playlist._id)}, {$set: playlist}, /* {upsert:true}, */ function (error, result) {
      if (error) { console.error('collabPl.save() error: ', error, error.stack) } else { console.log('=> saved collabPl:', result) }
      if (cb) { cb(result) }
    })
  } else
...
exports.fetchPlaylists = function (q, params, handler) {
  db['collabPl'].find(q, params, function (err, cursor) {
    ...
    cursor.toArray(function (err, playlists) {
      if (err) { console.error(err) } else { console.log('=> fetched', (playlists || []).length, 'playlists from collabPl') }
      handler(playlists)
    })
  })
}
...
exports.fetchPlaylistsByUid = function (uId, handler) {
  exports.fetchPlaylists({members: {$in: ['' + uId, mongodb.ObjectId('' + uId)]}}, {}, handler)
}
