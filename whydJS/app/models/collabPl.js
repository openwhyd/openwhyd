...
exports.remove = function(q, cb) {
	db["collabPl"].remove(q, function(error, result) {
		if (error)
			console.error("collabPl.remove() error: ", error, error.stack);
		else
			console.log("=> removed collabPl:", result);
		if (cb)
			cb(result);
	});
}
	...
	if (!playlist) {
		...
		db["collabPl"].update({_id: mongodb.ObjectId(""+playlist._id)}, {$set:playlist}, /*{upsert:true},*/ function(error, result) {
			if (error)
				...
				console.log("=> saved collabPl:", result);
			if (cb)
				cb(result);
		});
	else
	...
	db["collabPl"].find(q, params, function(err, cursor) {
		...
		cursor.toArray(function(err, playlists){
			if (err)
				...
				console.log("=> fetched", (playlists || []).length, "playlists from collabPl");
			handler(playlists);
		});
	});
};
