...
function fetchTrackInfo (trackId, cb) {
  return snip.httpRequestJSON('http://api.deezer.com/track/' + encodeURIComponent(trackId), null, function (err, res) {
...
exports.fetchTrackMetadata = function (trackId, cb, raw) {
  ...
  return snip.httpRequestJSON('http://api.deezer.com/track/' + encodeURIComponent(trackId), null, function (err, res) {
    if (err || raw) { cb(err, res) } else { cb(null, exports.translateTrack(res)) }
  })
}
