...
exports.EventManager = my.Class({
  ...
  constructor: function () {
    ...
    if (arguments.length > 0) {
      ...
      this.authorized = {}
      for (var i = 0; i < events.length; i++) { this.authorized[events[i]] = true }
    }
  },
  ...
  on: function (event, listener, a2) {
    if (arguments.length === 1) { return this.bindings[event] }

    if (this.authorized && !this.authorized[event]) { throw new Error('my.utils.EventManager.on: unauthorized <event=' + event + '>') }
    ...
    if (binding) {
      if (typeof a2 === 'number') {
        ...
        if ('runCount' in a2) { binding.runCount = a2.runCount }
        if ('eventObject' in a2) { binding.eventObject = a2.eventObject }
        if ('context' in a2) { binding.context = a2.context }
      }
    } else {
    ...
    if (bindings) {
      if (listener) {
        for (i = 0; i < bindings.length; i++) {
          if (bindings[i].listener === listener) { break }
        }
        if (i < bindings.length) { bindings.splice(i, 1) }
        if (bindings.length === 0) { delete this.bindings[event] }
      } else {
  ...
  fire: function (event, eventObject, context) {
    ...
    if (bindings) {
      for (i = 0; i < bindings.length; i++) {
        ...
        if (obj && obj.constructor === Array) { binding.listener.apply(ctx, obj) } else { binding.listener.call(ctx, obj) }
        binding.runCount--
      }
      for (i = bindings.length - 1; i >= 0; i--) {
        if (bindings[i].runCount == 0) { bindings.splice(i, 1) }
      }
