...
if (gmVersion) {
  ...
  if (imVersion) {
    console.log('[my.img] detected', imVersion, '=> using it for image manipulation')
  } else {
    console.error('[my.img] ERROR: please install graphicsmagick or imagemagick for image manipulation.')
  }
}
  ...
  http.get({host: imgUrl.host, path: imgUrl.pathname, port: 80}, function (res) {
    ...
    res.on('end', function () {
      ...
      fs.writeFile(imgOutput, data, 'binary', function (err) {
        if (err) {
          if (errorListener) { errorListener(err) } else { console.error(err) }
...
if (USE_GRAPHICS_MAGICK) {
  ...
  exports.makeThumb = function (imgPath, thumbOutput, width, height, endListener) {
	  ...
	  if (width && height) { exec('convert ' + imgPath + ' -resize ' + width + 'x' + height + ' ' + thumbOutput, execCallback) } else if (width) { exec('convert ' + imgPath + ' -resize ' + width + ' ' + thumbOutput, execCallback) } else if (height) { exec('convert ' + imgPath + ' -resize x' + height + ' ' + thumbOutput, execCallback) }
  }
}
