...
function SessionMongo (options) {
  ...
  mongodb.connect(db, {w: 0}, function (err, db) { // write concern is disabled
    if (err) throw err
  ...
  if (options.cleanEvery === 'week') { options.cleanEvery = 7 * 24 * 60 * 60 * 1000 }
  ...
  if (typeof options.cleanEvery === 'number') {
    setTimeout(function () {
      if (self.collection) { self.collection.remove({'cookie.expires': {$lte: Date.now()}}, {w: 0}) }
    }, options.cleanEvery)
  }
}
