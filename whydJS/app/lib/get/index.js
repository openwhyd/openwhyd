...
function ContentEmbedWrapper () {
  ...
  var fakeRequest = function () {
    ...
    for (var i in arguments) {
      if (typeof arguments[i] === 'string' && arguments[i].indexOf('//') > -1) { url = arguments[i]./* replace("alt=jsonc", "alt=json"). */replace(/[&?]callback=\?/, '') } else if (typeof arguments[i] === 'function') { callback = arguments[i] }
    }
...
Page.prototype.getImages = function () {
  ...
  for (i = 0, len = imgs.length; i < len; i++) {
    ...
    if (img.substr(0, 7) !== 'http://') {
      if (base) { img = path.normalize(base + '/' + img) } else {
...
Page.prototype.getMp3s = function () {
  ...
  for (i = 0, len = mp3s.length; i < len; i++) {
    ...
    if (mp3.substr(0, 7) !== 'http://') {
      if (base) { mp3 = path.normalize(base + '/' + mp3) } else {
