...
const map = makeMapWith(renderDate, function mapTemplate() {
  ...
  const failed = this.err ? 1 : 0;
  const val = { total: 1, total_err: failed };
  const getPlayerId = (eId) => (/^\/(\w\w)\//.exec(eId) || [])[1];
  const isUrl = (eId) => eId.substr(0, 8).indexOf('//') !== -1;
  const playerId = getPlayerId(this.eId) || (isUrl(this.eId) ? 'fi' : 'xx');
  val[playerId] = 1;
